...
#include "unknown/Support/ARMTargetParser.h"
...
{
    return StringSwitch<StringRef>(HWDiv).Case("thumb,arm", "arm,thumb").Default(HWDiv);
}
...
unsigned
ARM::parseArchVersion(StringRef Arch)
{
    Arch = getCanonicalArchName(Arch);
...
bool
ARM::getFPUFeatures(unsigned FPUKind, std::vector<StringRef> &Features)
{
...
unsigned
ARM::getDefaultFPU(StringRef CPU, ARM::ArchKind AK)
{
    if (CPU == "generic")
...
#include "unknown/Support/ARMTargetParser.def"
        .Default(ARM::FK_INVALID);
}

unsigned
ARM::getDefaultExtensions(StringRef CPU, ARM::ArchKind AK)
{
    if (CPU == "generic")
...
#define ARM_CPU_NAME(NAME, ID, DEFAULT_FPU, IS_DEFAULT, DEFAULT_EXT) \
    .Case(NAME, ARCHNames[static_cast<unsigned>(ArchKind::ID)].ArchBaseExtensions | DEFAULT_EXT)
#include "unknown/Support/ARMTargetParser.def"
        .Default(ARM::AEK_INVALID);
}

bool
ARM::getHWDivFeatures(unsigned HWDivKind, std::vector<StringRef> &Features)
{
...
bool
ARM::getExtensionFeatures(unsigned Extensions, std::vector<StringRef> &Features)
{
...
unsigned
ARM::getArchAttr(ARM::ArchKind AK)
{
...
unsigned
ARM::parseHWDiv(StringRef HWDiv)
{
...
unsigned
ARM::parseArchExt(StringRef ArchExt)
{
...
void
ARM::fillValidCPUArchList(SmallVectorImpl<StringRef> &Values)
{
...
StringRef
ARM::computeDefaultTargetABI(const Triple &TT, StringRef CPU)
{
    StringRef ArchName = CPU.empty() ? TT.getArchName() : getArchName(parseCPUArch(CPU));

    if (TT.isOSBinFormatMachO())
    {
        if (TT.getEnvironment() == Triple::EABI || TT.getOS() == Triple::UnknownOS ||
