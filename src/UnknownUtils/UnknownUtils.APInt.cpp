...
getDigit(char cdigit, uint8_t radix)
{
    unsigned r;
...
void
APInt::initSlowCase(uint64_t val, bool isSigned)
{
...
void
APInt::initSlowCase(const APInt &that)
{
...
void
APInt::initFromArray(ArrayRef<uint64_t> bigVal)
{
...
APInt::APInt(unsigned numBits, unsigned numWords, const uint64_t bigVal[]) : BitWidth(numBits)
{
    initFromArray(makeArrayRef(bigVal, numWords));
}

APInt::APInt(unsigned numbits, StringRef Str, uint8_t radix) : BitWidth(numbits)
{
    assert(BitWidth && "Bitwidth too small");
...
void
APInt::reallocate(unsigned NewBitWidth)
{
...
void
APInt::Profile(FoldingSetNodeID &ID) const
{
...
bool
APInt::EqualSlowCase(const APInt &RHS) const
{
...
void
APInt::setBitsSlowCase(unsigned loBit, unsigned hiBit)
{
...
void
APInt::flipBit(unsigned bitPosition)
{
...
void
APInt::insertBits(const APInt &subBits, unsigned bitPosition)
{
    unsigned subBitWidth = subBits.getBitWidth();
    assert(0 < subBitWidth && (subBitWidth + bitPosition) <= BitWidth && "Illegal bit insertion");
    ...
    if (loBit == 0)
    {
        ...
        memcpy(U.pVal + loWord, subBits.getRawData(), numWholeSubWords * APINT_WORD_SIZE);
...
APInt::extractBits(unsigned numBits, unsigned bitPosition) const
{
    ...
    assert(bitPosition < BitWidth && (numBits + bitPosition) <= BitWidth && "Illegal bit extraction");
    ...
    for (unsigned word = 0; word < NumDstWords; ++word)
    {
        ...
        uint64_t w1 = (loWord + word + 1) < NumSrcWords ? U.pVal[loWord + word + 1] : 0;
...
unsigned
APInt::getBitsNeeded(StringRef str, uint8_t radix)
{
    ...
    // bits in that case.
    unsigned sufficient = radix == 10 ? (slen == 1 ? 4 : slen * 64 / 18) : (slen == 1 ? 7 : slen * 16 / 3);
...
hash_code
unknown::hash_value(const APInt &Arg)
{
    if (Arg.isSingleWord())
...
bool
APInt::isSplat(unsigned SplatSizeInBits) const
{
    assert(getBitWidth() % SplatSizeInBits == 0 && "SplatSizeInBits must divide width!");
...
APInt
APInt::getSplat(unsigned NewLen, const APInt &V)
{
...
unsigned
APInt::countLeadingZerosSlowCase() const
{
    unsigned Count = 0;
    for (int i = getNumWords() - 1; i >= 0; --i)
    {
        ...
        {
            Count += unknown::countLeadingZeros(V);
            break;
        }
    }
...
unsigned
APInt::countLeadingOnesSlowCase() const
{
    ...
    int i = getNumWords() - 1;
    unsigned Count = unknown::countLeadingOnes(U.pVal[i] << shift);
    if (Count == highWordBits)
    {
        for (i--; i >= 0; --i)
        {
            ...
            {
                Count += unknown::countLeadingOnes(U.pVal[i]);
                break;
            }
        }
    }
...
unsigned
APInt::countTrailingZerosSlowCase() const
{
    unsigned Count = 0;
    unsigned i = 0;
    for (; i < getNumWords() && U.pVal[i] == 0; ++i)
        Count += APINT_BITS_PER_WORD;
    if (i < getNumWords())
        Count += unknown::countTrailingZeros(U.pVal[i]);
    return std::min(Count, BitWidth);
}

unsigned
APInt::countTrailingOnesSlowCase() const
{
    unsigned Count = 0;
    unsigned i = 0;
    for (; i < getNumWords() && U.pVal[i] == WORDTYPE_MAX; ++i)
        Count += APINT_BITS_PER_WORD;
    if (i < getNumWords())
        Count += unknown::countTrailingOnes(U.pVal[i]);
    assert(Count <= BitWidth);
    return Count;
}

unsigned
APInt::countPopulationSlowCase() const
{
    unsigned Count = 0;
    for (unsigned i = 0; i < getNumWords(); ++i)
        Count += unknown::countPopulation(U.pVal[i]);
    return Count;
}

bool
APInt::intersectsSlowCase(const APInt &RHS) const
{
...
bool
APInt::isSubsetOfSlowCase(const APInt &RHS) const
{
    for (unsigned i = 0, e = getNumWords(); i != e; ++i)
        if ((U.pVal[i] & ~RHS.U.pVal[i]) != 0)
            return false;
...
APInt::reverseBits() const
{
    ...
    {
    case 64:
        return APInt(BitWidth, unknown::reverseBits<uint64_t>(U.VAL));
    case 32:
        return APInt(BitWidth, unknown::reverseBits<uint32_t>(U.VAL));
    case 16:
        return APInt(BitWidth, unknown::reverseBits<uint16_t>(U.VAL));
    case 8:
        return APInt(BitWidth, unknown::reverseBits<uint8_t>(U.VAL));
    default:
        break;
    }
...
APInt
unknown::APIntOps::GreatestCommonDivisor(APInt A, APInt B)
{
...
APInt
unknown::APIntOps::RoundDoubleToAPInt(double Double, unsigned width)
{
    ...
    if (exp < 52)
        return isNeg ? -APInt(width, mantissa >> (52 - exp)) : APInt(width, mantissa >> (52 - exp));
...
///  --------------------------------------
double
APInt::roundToDouble(bool isSigned) const
{
...
APInt::sext(unsigned Width) const
{
    ...
    Result.U.pVal[getNumWords() - 1] =
        SignExtend64(Result.U.pVal[getNumWords() - 1], ((BitWidth - 1) % APINT_BITS_PER_WORD) + 1);

    // Fill with sign bits.
    std::memset(
        Result.U.pVal + getNumWords(), isNegative() ? -1 : 0, (Result.getNumWords() - getNumWords()) * APINT_WORD_SIZE);
    Result.clearUnusedBits();
    return Result;
}
...
APInt::zext(unsigned width) const
{
    ...
    std::memset(Result.U.pVal + getNumWords(), 0, (Result.getNumWords() - getNumWords()) * APINT_WORD_SIZE);
...
void
APInt::ashrInPlace(const APInt &shiftAmt)
{
...
/// Arithmetic right-shift function.
void
APInt::ashrSlowCase(unsigned ShiftAmt)
{
    ...
    if (WordsToMove != 0)
    {
        ...
        U.pVal[getNumWords() - 1] = SignExtend64(U.pVal[getNumWords() - 1], ((BitWidth - 1) % APINT_BITS_PER_WORD) + 1);
        ...
        else
        {
            ...
            U.pVal[WordsToMove - 1] = SignExtend64(U.pVal[WordsToMove - 1], APINT_BITS_PER_WORD - BitShift);
        }
    }

    // Fill in the remainder based on the original sign.
    std::memset(U.pVal + WordsToMove, Negative ? -1 : 0, WordShift * APINT_WORD_SIZE);
    clearUnusedBits();
}
...
void
APInt::lshrInPlace(const APInt &shiftAmt)
{
...
/// Logical right-shift function.
void
APInt::lshrSlowCase(unsigned ShiftAmt)
{
...
static unsigned
rotateModulo(unsigned BitWidth, const APInt &rotateAmt)
{
...
APInt::mu
APInt::magicu(unsigned LeadingZeros) const
{
    const APInt &d = *this;
    unsigned p;
    ...
    do
    {
        ...
        delta = d - 1 - r2;
    } while (p < d.getBitWidth() * 2 && (q1.ult(delta) || (q1 == delta && r1 == 0)));
...
static void
KnuthDiv(uint32_t *u, uint32_t *v, uint32_t *q, uint32_t *r, unsigned m, unsigned n)
{
...
#    define DEBUG_KNUTH(X) \
    ...
    do
    {
        ...
        for (unsigned i = 0; i < n; ++i)
        {
            ...
            DEBUG_KNUTH(dbgs() << "KnuthDiv: u[j+i] = " << u[j + i] << ", borrow = " << borrow << '\n');
        }
...
void
APInt::udivrem(const APInt &LHS, const APInt &RHS, APInt &Quotient, APInt &Remainder)
{
    ...
    divide(LHS.U.pVal, lhsWords, RHS.U.pVal, rhsWords, Quotient.U.pVal, Remainder.U.pVal);
    // Clear the rest of the Quotient and Remainder.
    std::memset(Quotient.U.pVal + lhsWords, 0, (getNumWords(BitWidth) - lhsWords) * APINT_WORD_SIZE);
    std::memset(Remainder.U.pVal + rhsWords, 0, (getNumWords(BitWidth) - rhsWords) * APINT_WORD_SIZE);
}

void
APInt::udivrem(const APInt &LHS, uint64_t RHS, APInt &Quotient, uint64_t &Remainder)
{
    assert(RHS != 0 && "Divide by zero?");
    unsigned BitWidth = LHS.BitWidth;
...
void
APInt::sdivrem(const APInt &LHS, const APInt &RHS, APInt &Quotient, APInt &Remainder)
{
    if (LHS.isNegative())
    {
        if (RHS.isNegative())
...
void
APInt::sdivrem(const APInt &LHS, int64_t RHS, APInt &Quotient, int64_t &Remainder)
{
...
{
    APInt Res = *this + RHS;
    Overflow = isNonNegative() == RHS.isNonNegative() && Res.isNonNegative() != isNonNegative();
    return Res;
}
...
APInt::sadd_sat(const APInt &RHS) const
{
    ...
    return isNegative() ? APInt::getSignedMinValue(BitWidth) : APInt::getSignedMaxValue(BitWidth);
}
...
void
APInt::fromString(unsigned numbits, StringRef str, uint8_t radix)
{
...
void
APInt::toString(SmallVectorImpl<char> &Str, unsigned Radix, bool Signed, bool formatAsCLiteral) const
{
    assert(
        (Radix == 10 || Radix == 8 || Radix == 16 || Radix == 2 || Radix == 36) &&
        "Radix should be 2, 8, 10, 16, or 36!");
...
std::string
APInt::toString(unsigned Radix = 10, bool Signed = true) const
{
    SmallString<40> S;
...
void
APInt::print(raw_ostream &OS, bool isSigned) const
{
    SmallString<40> S;
...
static_assert(APInt::APINT_BITS_PER_WORD % 2 == 0, "Part width must be divisible by 2!");

/* Some handy functions local to this file.  */

/* Returns the integer part with the least significant BITS set.
   BITS cannot be zero.  */
static inline APInt::WordType
...
void
APInt::tcSet(WordType *dst, WordType part, unsigned parts)
{
    assert(parts > 0);
...
void
APInt::tcAssign(WordType *dst, const WordType *src, unsigned parts)
{
...
bool
APInt::tcIsZero(const WordType *src, unsigned parts)
{
    for (unsigned i = 0; i < parts; i++)
        if (src[i])
            return false;
...
void
APInt::tcSetBit(WordType *parts, unsigned bit)
{
...
void
APInt::tcClearBit(WordType *parts, unsigned bit)
{
    parts[whichWord(bit)] &= ~maskBit(bit);
}
...
unsigned
APInt::tcLSB(const WordType *parts, unsigned n)
{
    for (unsigned i = 0; i < n; i++)
    {
        if (parts[i] != 0)
...
/* Returns the bit number of the most significant set bit of a number.
   If the input number has no bits set -1U is returned.  */
unsigned
APInt::tcMSB(const WordType *parts, unsigned n)
{
    do
    {
        --n;
...
/* Copy the bit vector of width srcBITS from SRC, starting at bit
   ...
   DST are zero-filled.  */
void
APInt::tcExtract(WordType *dst, unsigned dstCount, const WordType *src, unsigned srcBits, unsigned srcLSB)
{
...
APInt::WordType
APInt::tcAdd(WordType *dst, const WordType *rhs, WordType c, unsigned parts)
{
    assert(c <= 1);
...
APInt::WordType
APInt::tcAddPart(WordType *dst, WordType src, unsigned parts)
{
    for (unsigned i = 0; i < parts; ++i)
    {
        dst[i] += src;
...
/* DST -= RHS + C where C is zero or one.  Returns the carry flag.  */
APInt::WordType
APInt::tcSubtract(WordType *dst, const WordType *rhs, WordType c, unsigned parts)
{
    assert(c <= 1);
...
APInt::WordType
APInt::tcSubtractPart(WordType *dst, WordType src, unsigned parts)
{
    for (unsigned i = 0; i < parts; ++i)
    {
        WordType Dst = dst[i];
...
void
APInt::tcNegate(WordType *dst, unsigned parts)
{
...
/* DST = LHS * RHS, where DST has the same width as the operands and
   ...
   from both operands.  */
int
APInt::tcMultiply(WordType *dst, const WordType *lhs, const WordType *rhs, unsigned parts)
{
...
void
APInt::tcFullMultiply(WordType *dst, const WordType *lhs, const WordType *rhs, unsigned lhsParts, unsigned rhsParts)
{
...
*/
int
APInt::tcDivide(WordType *lhs, const WordType *rhs, WordType *remainder, WordType *srhs, unsigned parts)
{
...
void
APInt::tcShiftLeft(WordType *Dst, unsigned Words, unsigned Count)
{
    // Don't bother performing a no-op shift.
    if (!Count)
        return;
...
void
APInt::tcShiftRight(WordType *Dst, unsigned Words, unsigned Count)
{
    // Don't bother performing a no-op shift.
    if (!Count)
        return;
...
void
APInt::tcAnd(WordType *dst, const WordType *rhs, unsigned parts)
{
    for (unsigned i = 0; i < parts; i++)
        dst[i] &= rhs[i];
}
...
void
APInt::tcOr(WordType *dst, const WordType *rhs, unsigned parts)
{
    for (unsigned i = 0; i < parts; i++)
        dst[i] |= rhs[i];
}
...
void
APInt::tcXor(WordType *dst, const WordType *rhs, unsigned parts)
{
    for (unsigned i = 0; i < parts; i++)
        dst[i] ^= rhs[i];
}
...
void
APInt::tcComplement(WordType *dst, unsigned parts)
{
    for (unsigned i = 0; i < parts; i++)
        dst[i] = ~dst[i];
}
...
int
APInt::tcCompare(const WordType *lhs, const WordType *rhs, unsigned parts)
{
    while (parts)
    {
        parts--;
...
/* Set the least significant BITS bits of a bignum, clear the
   rest.  */
void
APInt::tcSetLeastSignificantBits(WordType *dst, unsigned parts, unsigned bits)
{
    unsigned i = 0;
...
APInt
unknown::APIntOps::RoundingUDiv(const APInt &A, const APInt &B, APInt::Rounding RM)
{
...
APInt
unknown::APIntOps::RoundingSDiv(const APInt &A, const APInt &B, APInt::Rounding RM)
{
    switch (RM)
    {
    case APInt::Rounding::DOWN:
...
Optional<APInt>
unknown::APIntOps::SolveQuadraticEquationWrap(APInt A, APInt B, APInt C, unsigned RangeWidth)
{
    unsigned CoeffWidth = A.getBitWidth();
    assert(CoeffWidth == B.getBitWidth() && CoeffWidth == C.getBitWidth());
    assert(RangeWidth <= CoeffWidth && "Value range width should be less than coefficient width");
    assert(RangeWidth > 1 && "Value range bit width should be > 1");

    LLVM_DEBUG(
        dbgs() << __func__ << ": solving " << A << "x^2 + " << B << "x + " << C << ", rw:" << RangeWidth << '\n');
    ...
    LLVM_DEBUG(
        dbgs() << __func__ << ": updated coefficients " << A << "x^2 + " << B << "x + " << C << ", rw:" << RangeWidth
               << '\n');
    ...
    bool SignChange = VX.isNegative() != VY.isNegative() || VX.isNullValue() != VY.isNullValue();
