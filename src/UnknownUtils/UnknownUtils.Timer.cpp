...
std::unique_ptr<raw_fd_ostream> unknown::CreateInfoOutputFile() {
  const std::string &OutputFilename = getLibSupportInfoOutputFilename();
  if (OutputFilename.empty())
    return unknown::make_unique<raw_fd_ostream>(2, false); // stderr.
  if (OutputFilename == "-")
    return unknown::make_unique<raw_fd_ostream>(1, false); // stdout.
  ...
  std::error_code EC;
  auto Result = unknown::make_unique<raw_fd_ostream>(
