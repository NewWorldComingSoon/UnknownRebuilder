...
#include "unknown/ADT/APFloat.h"
#include "unknown/ADT/APSInt.h"
#include "unknown/ADT/ArrayRef.h"
#include "unknown/ADT/FoldingSet.h"
#include "unknown/ADT/Hashing.h"
...
#include "unknown/Support/MathExtras.h"
...
/* These are legacy semantics for the fallback, inaccrurate implementation of
   ...
   semantics.  */
static const fltSemantics semPPCDoubleDoubleLegacy = {1023, -1022 + 53, 53 + 53, 128};
...
unsigned int
APFloatBase::semanticsPrecision(const fltSemantics &semantics)
{
...
unsigned int
APFloatBase::semanticsSizeInBits(const fltSemantics &semantics)
{
    return semantics.sizeInBits;
}

unsigned
APFloatBase::getSizeInBits(const fltSemantics &Sem)
{
...
static int
totalExponent(StringRef::iterator p, StringRef::iterator end, int exponentAdjustment)
{
...
skipLeadingZeroesAndAnyDot(StringRef::iterator begin, StringRef::iterator end, StringRef::iterator *dot)
{
    StringRef::iterator p = begin;
    *dot = end;
    while (p != end && *p == '0')
...
static void
interpretDecimal(StringRef::iterator begin, StringRef::iterator end, decimalInfo *D)
{
    ...
    if (p != D->firstSigDigit)
    {
        ...
        D->normalizedExponent =
            (D->exponent +
             static_cast<APFloat::ExponentType>((p - D->firstSigDigit) - (dot > D->firstSigDigit && dot < p)));
    }
...
/* Return the trailing fraction of a hexadecimal number.
   ...
   the next digit.  */
static lostFraction
trailingHexadecimalFraction(StringRef::iterator p, StringRef::iterator end, unsigned int digitValue)
{
...
static lostFraction
lostFractionThroughTruncation(const APFloatBase::integerPart *parts, unsigned int partCount, unsigned int bits)
{
    unsigned int lsb;
    ...
    if (bits <= partCount * APFloatBase::integerPartWidth && APInt::tcExtractBit(parts, bits - 1))
        return lfMoreThanHalf;
...
static lostFraction
combineLostFractions(lostFraction moreSignificant, lostFraction lessSignificant)
{
...
ulpsFromBoundary(const APFloatBase::integerPart *parts, unsigned int bits, bool isNearest)
{
...
/* Write out an integerPart in hexadecimal, starting with the most
   ...
   COUNT.  */
static unsigned int
partAsHex(char *dst, APFloatBase::integerPart part, unsigned int count, const char *hexDigitChars)
{
...
/* Constructors.  */
void
IEEEFloat::initialize(const fltSemantics *ourSemantics)
{
    unsigned int count;
...
void
IEEEFloat::assign(const IEEEFloat &rhs)
{
...
void
IEEEFloat::copySignificand(const IEEEFloat &rhs)
{
...
void
IEEEFloat::makeNaN(bool SNaN, bool Negative, const APInt *fill)
{
    category = fcNaN;
    sign = Negative;
    ...
    if (fill)
    {
        APInt::tcAssign(significand, fill->getRawData(), std::min(fill->getNumWords(), numParts));
...
bool
IEEEFloat::isSmallest() const
{
    // The smallest number by magnitude in our format will be the smallest
    // denormal, i.e. the floating point number with exponent being minimum
    // exponent and significand bitwise equal to 1 (i.e. with MSB equal to 0).
    return isFiniteNonZero() && exponent == semantics->minExponent && significandMSB() == 0;
}

bool
IEEEFloat::isSignificandAllOnes() const
{
    ...
    const unsigned NumHighBits = PartCount * integerPartWidth - semantics->precision + 1;
    assert(
        NumHighBits <= integerPartWidth && "Can not have more high bits to "
...
bool
IEEEFloat::isSignificandAllZeros() const
{
...
bool
IEEEFloat::isLargest() const
{
    // The largest number by magnitude in our format will be the floating point
    // number with maximum exponent and with significand that is all ones.
    return isFiniteNonZero() && exponent == semantics->maxExponent && isSignificandAllOnes();
}

bool
IEEEFloat::isInteger() const
{
...
bool
IEEEFloat::bitwiseIsEqual(const IEEEFloat &rhs) const
{
    if (this == &rhs)
        return true;
    if (semantics != rhs.semantics || category != rhs.category || sign != rhs.sign)
        return false;
    ...
    return std::equal(significandParts(), significandParts() + partCount(), rhs.significandParts());
}
...
IEEEFloat::IEEEFloat(const fltSemantics &ourSemantics, uninitializedTag tag) : IEEEFloat(ourSemantics) {}
...
void
IEEEFloat::incrementSignificand()
{
    integerPart carry;
...
IEEEFloat::integerPart
IEEEFloat::addSignificand(const IEEEFloat &rhs)
{
    integerPart *parts;
...
/* Multiply the significand of the RHS.  If ADDEND is non-NULL, add it
   ...
   lost fraction.  */
lostFraction
IEEEFloat::multiplySignificand(const IEEEFloat &rhs, const IEEEFloat *addend)
{
    ...
    APInt::tcFullMultiply(fullSignificand, lhsSignificand, rhs.significandParts(), partsCount, partsCount);
    ...
    if (addend && addend->isNonZero())
    {
        ...
        assert(lost_fraction == lfExactlyZero && "Lost precision while shifting addend for fused-multiply-add.");
...
lostFraction
IEEEFloat::divideSignificand(const IEEEFloat &rhs)
{
...
unsigned int
IEEEFloat::significandMSB() const
{
    return APInt::tcMSB(significandParts(), partCount());
}

unsigned int
IEEEFloat::significandLSB() const
{
...
void
IEEEFloat::shiftSignificandLeft(unsigned int bits)
{
...
IEEEFloat::compareAbsoluteValue(const IEEEFloat &rhs) const
{
    ...
    /* If exponents are equal, do an unsigned bignum comparison of the
       significands.  */
    if (compare == 0)
        compare = APInt::tcCompare(significandParts(), rhs.significandParts(), partCount());
...
IEEEFloat::handleOverflow(roundingMode rounding_mode)
{
    ...
    if (rounding_mode == rmNearestTiesToEven || rounding_mode == rmNearestTiesToAway ||
        (rounding_mode == rmTowardPositive && !sign) || (rounding_mode == rmTowardNegative && sign))
    {
        category = fcInfinity;
    ...
    APInt::tcSetLeastSignificantBits(significandParts(), partCount(), semantics->precision);
...
/* Returns TRUE if, when truncating the current number, with BIT the
   ...
   fcNormal numbers.  */
bool
IEEEFloat::roundAwayFromZero(roundingMode rounding_mode, lostFraction lost_fraction, unsigned int bit) const
{
...
IEEEFloat::opStatus
IEEEFloat::normalize(roundingMode rounding_mode, lostFraction lost_fraction)
{
    unsigned int omsb; /* One, not zero, based MSB.  */
    int exponentChange;
...
lostFraction
IEEEFloat::addOrSubtractSignificand(const IEEEFloat &rhs, bool subtract)
{
    integerPart carry;
    lostFraction lost_fraction;
    int bits;
...
IEEEFloat::opStatus
IEEEFloat::multiplySpecials(const IEEEFloat &rhs)
{
    switch (PackCategoriesIntoKey(category, rhs.category))
    {
    default:
...
IEEEFloat::opStatus
IEEEFloat::divideSpecials(const IEEEFloat &rhs)
{
    switch (PackCategoriesIntoKey(category, rhs.category))
    {
    default:
...
IEEEFloat::opStatus
IEEEFloat::modSpecials(const IEEEFloat &rhs)
{
    switch (PackCategoriesIntoKey(category, rhs.category))
    {
    default:
...
IEEEFloat::opStatus
IEEEFloat::addOrSubtract(const IEEEFloat &rhs, roundingMode rounding_mode, bool subtract)
{
    opStatus fs;
...
/* Normalized addition.  */
IEEEFloat::opStatus
IEEEFloat::add(const IEEEFloat &rhs, roundingMode rounding_mode)
{
...
IEEEFloat::opStatus
IEEEFloat::subtract(const IEEEFloat &rhs, roundingMode rounding_mode)
{
...
/* Normalized multiply.  */
IEEEFloat::opStatus
IEEEFloat::multiply(const IEEEFloat &rhs, roundingMode rounding_mode)
{
    opStatus fs;
...
/* Normalized divide.  */
IEEEFloat::opStatus
IEEEFloat::divide(const IEEEFloat &rhs, roundingMode rounding_mode)
{
    opStatus fs;
...
IEEEFloat::opStatus
IEEEFloat::remainder(const IEEEFloat &rhs)
{
    opStatus fs;
    IEEEFloat V = *this;
    ...
    fs = V.convertToInteger(
        makeMutableArrayRef(x, parts), parts * integerPartWidth, true, rmNearestTiesToEven, &ignored);
    if (fs == opInvalidOp)
    {
        delete[] x;
        return fs;
    ...
    fs = V.convertFromZeroExtendedInteger(x, parts * integerPartWidth, true, rmNearestTiesToEven);
...
IEEEFloat::opStatus
IEEEFloat::mod(const IEEEFloat &rhs)
{
    opStatus fs;
    fs = modSpecials(rhs);
    unsigned int origSign = sign;

    while (isFiniteNonZero() && rhs.isFiniteNonZero() && compareAbsoluteValue(rhs) != cmpLessThan)
    {
...
IEEEFloat::opStatus
IEEEFloat::fusedMultiplyAdd(const IEEEFloat &multiplicand, const IEEEFloat &addend, roundingMode rounding_mode)
{
    opStatus fs;
    ...
    if (isFiniteNonZero() && multiplicand.isFiniteNonZero() && addend.isFinite())
    {
        lostFraction lost_fraction;
...
IEEEFloat::cmpResult
IEEEFloat::compare(const IEEEFloat &rhs) const
{
    cmpResult result;
...
IEEEFloat::opStatus
IEEEFloat::convert(const fltSemantics &toSemantics, roundingMode rounding_mode, bool *losesInfo)
{
    ...
    if (&fromSemantics == &semX87DoubleExtended && &toSemantics != &semX87DoubleExtended && category == fcNaN &&
        (!(*significandParts() & 0x8000000000000000ULL) || !(*significandParts() & 0x4000000000000000ULL)))
    {
    ...
    *isExact = false;
    ...
    else
    {
        ...
        else
        {
            ...
            APInt::tcExtract(parts.data(), dstPartsCount, src, semantics->precision, 0);
            APInt::tcShiftLeft(parts.data(), dstPartsCount, bits - semantics->precision);
            truncatedBits = 0;
        }
    }
    ...
    if (truncatedBits)
    {
        ...
        if (lost_fraction != lfExactlyZero && roundAwayFromZero(rounding_mode, lost_fraction, truncatedBits))
        {
    ...
    if (sign)
    {
        ...
        else
        {
            ...
            if (omsb == width && APInt::tcLSB(parts.data(), dstPartsCount) + 1 != omsb)
                return opInvalidOp;
    ...
    bool *isExact) const
{
    ...
    fs = convertToSignExtendedInteger(parts, width, isSigned, rounding_mode, isExact);
...
IEEEFloat::opStatus
IEEEFloat::convertFromUnsignedParts(const integerPart *src, unsigned int srcCount, roundingMode rounding_mode)
{
...
IEEEFloat::opStatus
IEEEFloat::convertFromAPInt(const APInt &Val, bool isSigned, roundingMode rounding_mode)
{
...
IEEEFloat::opStatus
IEEEFloat::convertFromSignExtendedInteger(
    const integerPart *src,
...
IEEEFloat::opStatus
IEEEFloat::convertFromZeroExtendedInteger(
...
IEEEFloat::opStatus
IEEEFloat::convertFromHexadecimalString(StringRef s, roundingMode rounding_mode)
{
...
IEEEFloat::opStatus
IEEEFloat::roundSignificandWithExponent(
    ...
    roundingMode rounding_mode)
{
    ...
    isNearest = (rounding_mode == rmNearestTiesToEven || rounding_mode == rmNearestTiesToAway);
    ...
    for (;; parts *= 2)
    {
        ...
        HUerr = HUerrBound(calcLostFraction != lfExactlyZero, sigStatus != opOK, powHUerr);
        HUdistance = 2 * ulpsFromBoundary(decSig.significandParts(), excessPrecision, isNearest);
        ...
        if (HUdistance >= HUerr)
        {
            ...
            exponent = (decSig.exponent + semantics->precision - (calcSemantics.precision - excessPrecision));
            calcLostFraction =
                lostFractionThroughTruncation(decSig.significandParts(), decSig.partCount(), truncatedBits);
...
IEEEFloat::convertFromDecimalString(StringRef str, roundingMode rounding_mode)
{
    ...
    */
    ...
    else if (D.normalizedExponent - 1 > INT_MAX / 42039)
    {
        ...
        (D.normalizedExponent + 1) * 28738 <= 8651 * (semantics->minExponent - (int)semantics->precision))
    {
    ...
    else if ((D.normalizedExponent - 1) * 42039 >= 12655 * semantics->maxExponent)
    {
        /* Overflow and round.  */
        fs = handleOverflow(rounding_mode);
    }
    else
    {
        ...
        category = fcNormal;
        fs = roundSignificandWithExponent(decSignificand, partCount, D.exponent, rounding_mode);
...
bool
IEEEFloat::convertFromStringSpecials(StringRef str)
{
...
IEEEFloat::convertFromString(StringRef str, roundingMode rounding_mode)
{
    ...
    if (slen >= 2 && p[0] == '0' && (p[1] == 'x' || p[1] == 'X'))
    {
        ...
        return convertFromHexadecimalString(StringRef(p + 2, slen - 2), rounding_mode);
    }
...
*/
unsigned int
IEEEFloat::convertToHexString(char *dst, unsigned int hexDigits, bool upperCase, roundingMode rounding_mode) const
{
    char *p;
...
char *
IEEEFloat::convertNormalToHexString(char *dst, unsigned int hexDigits, bool upperCase, roundingMode rounding_mode) const
{
...
hash_code
hash_value(const IEEEFloat &Arg)
{
    ...
    return hash_combine(
        ...
        Arg.semantics->precision,
        Arg.exponent,
        hash_combine_range(Arg.significandParts(), Arg.significandParts() + Arg.partCount()));
}
...
{
    assert(semantics == (const unknown::fltSemantics *)&semX87DoubleExtended);
    assert(partCount() == 2);
...
{
    assert(semantics == (const unknown::fltSemantics *)&semPPCDoubleDoubleLegacy);
    assert(partCount() == 2);
...
{
    assert(semantics == (const unknown::fltSemantics *)&semIEEEquad);
    assert(partCount() == 2);
    ...
    uint64_t words[2];
    words[0] = mysignificand;
    words[1] = ((uint64_t)(sign & 1) << 63) | ((myexponent & 0x7fff) << 48) | (mysignificand2 & 0xffffffffffffLL);
...
{
    assert(semantics == (const unknown::fltSemantics *)&semIEEEdouble);
    assert(partCount() == 1);
    ...
    return APInt(
        64, ((((uint64_t)(sign & 1) << 63) | ((myexponent & 0x7ff) << 52) | (mysignificand & 0xfffffffffffffLL))));
}

APInt
IEEEFloat::convertFloatAPFloatToAPInt() const
{
    assert(semantics == (const unknown::fltSemantics *)&semIEEEsingle);
    assert(partCount() == 1);
    ...
    return APInt(32, (((sign & 1) << 31) | ((myexponent & 0xff) << 23) | (mysignificand & 0x7fffff)));
}

APInt
IEEEFloat::convertHalfAPFloatToAPInt() const
{
    assert(semantics == (const unknown::fltSemantics *)&semIEEEhalf);
    assert(partCount() == 1);
    ...
    return APInt(16, (((sign & 1) << 15) | ((myexponent & 0x1f) << 10) | (mysignificand & 0x3ff)));
}
...
{
    if (semantics == (const unknown::fltSemantics *)&semIEEEhalf)
        return convertHalfAPFloatToAPInt();

    if (semantics == (const unknown::fltSemantics *)&semIEEEsingle)
        return convertFloatAPFloatToAPInt();

    if (semantics == (const unknown::fltSemantics *)&semIEEEdouble)
        return convertDoubleAPFloatToAPInt();

    if (semantics == (const unknown::fltSemantics *)&semIEEEquad)
        return convertQuadrupleAPFloatToAPInt();

    if (semantics == (const unknown::fltSemantics *)&semPPCDoubleDoubleLegacy)
        return convertPPCDoubleDoubleAPFloatToAPInt();

    assert(semantics == (const unknown::fltSemantics *)&semX87DoubleExtended && "unknown format!");
    return convertF80LongDoubleAPFloatToAPInt();
}

float
IEEEFloat::convertToFloat() const
{
    assert(semantics == (const unknown::fltSemantics *)&semIEEEsingle && "Float semantics are not IEEEsingle");
    APInt api = bitcastToAPInt();
    return api.bitsToFloat();
}

double
IEEEFloat::convertToDouble() const
{
    assert(semantics == (const unknown::fltSemantics *)&semIEEEdouble && "Float semantics are not IEEEdouble");
    APInt api = bitcastToAPInt();
...
void
IEEEFloat::initFromF80LongDoubleAPInt(const APInt &api)
{
...
void
IEEEFloat::initFromPPCDoubleDoubleAPInt(const APInt &api)
{
...
void
IEEEFloat::initFromQuadrupleAPInt(const APInt &api)
{
    assert(api.getBitWidth() == 128);
    uint64_t i1 = api.getRawData()[0];
    uint64_t i2 = api.getRawData()[1];
    ...
    else if (myexponent == 0x7fff && (mysignificand == 0 && mysignificand2 == 0))
    {
        // exponent, significand meaningless
        category = fcInfinity;
    }
...
void
IEEEFloat::initFromDoubleAPInt(const APInt &api)
{
...
void
IEEEFloat::initFromFloatAPInt(const APInt &api)
{
...
void
IEEEFloat::initFromHalfAPInt(const APInt &api)
{
...
void
IEEEFloat::initFromAPInt(const fltSemantics *Sem, const APInt &api)
{
    if (Sem == &semIEEEhalf)
...
/// semantics.
void
IEEEFloat::makeLargest(bool Negative)
{
    ...
    const unsigned NumUnusedHighBits = PartCount * integerPartWidth - semantics->precision;
    significand[PartCount - 1] = (NumUnusedHighBits < integerPartWidth) ? (~integerPart(0) >> NumUnusedHighBits) : 0;
}

/// Make this number the smallest magnitude denormal number in the given
/// semantics.
void
IEEEFloat::makeSmallest(bool Negative)
{
    // We want (in interchange format):
    //   sign = {Negative}
    //   exponent = 0..0
...
void
IEEEFloat::makeSmallestNormalized(bool Negative)
{
    // We want (in interchange format):
    //   sign = {Negative}
    //   exponent = 0..0
...
namespace {
void
append(SmallVectorImpl<char> &Buffer, StringRef Str)
{
...
void
AdjustToPrecision(SmallVectorImpl<char> &buffer, int &exp, unsigned FormatPrecision)
{
...
void
IEEEFloat::toString(SmallVectorImpl<char> &Str, unsigned FormatPrecision, unsigned FormatMaxPadding, bool TruncateZero)
    const
{
    switch (category)
    {
    ...
    APInt significand(semantics->precision, makeArrayRef(significandParts(), partCountForBits(semantics->precision)));
    ...
    else
    {
        if (exp >= 0)
        {
            ...
            FormatScientific = ((unsigned)exp > FormatMaxPadding || NDigits + (unsigned)exp > FormatPrecision);
        }
        else
        {
...
bool
IEEEFloat::getExactInverse(APFloat *inv) const
{
...
IEEEFloat::next(bool nextDown)
{
    ...
    case fcNormal:
        ...
        if (isNegative())
        {
            ...
            bool WillCrossBinadeBoundary = exponent != semantics->minExponent && isSignificandAllZeros();
...
void
IEEEFloat::makeInf(bool Negative)
{
    category = fcInfinity;
    sign = Negative;
    exponent = semantics->maxExponent + 1;
...
void
IEEEFloat::makeZero(bool Negative)
{
    category = fcZero;
    sign = Negative;
...
IEEEFloat
scalbn(IEEEFloat X, int Exp, IEEEFloat::roundingMode RoundingMode)
{
...
IEEEFloat
frexp(const IEEEFloat &Val, int &Exp, IEEEFloat::roundingMode RM)
{
    Exp = ilogb(Val);
...
DoubleAPFloat::DoubleAPFloat(const fltSemantics &S) :
    Semantics(&S), Floats(new APFloat[2]{APFloat(semIEEEdouble), APFloat(semIEEEdouble)})
{
    assert(Semantics == &semPPCDoubleDouble);
}

DoubleAPFloat::DoubleAPFloat(const fltSemantics &S, uninitializedTag) :
    Semantics(&S), Floats(new APFloat[2]{APFloat(semIEEEdouble, uninitialized), APFloat(semIEEEdouble, uninitialized)})
{
    assert(Semantics == &semPPCDoubleDouble);
}

DoubleAPFloat::DoubleAPFloat(const fltSemantics &S, integerPart I) :
    Semantics(&S), Floats(new APFloat[2]{APFloat(semIEEEdouble, I), APFloat(semIEEEdouble)})
{
    assert(Semantics == &semPPCDoubleDouble);
}
...
DoubleAPFloat::DoubleAPFloat(const fltSemantics &S, APFloat &&First, APFloat &&Second) :
    Semantics(&S), Floats(new APFloat[2]{std::move(First), std::move(Second)})
{
    assert(Semantics == &semPPCDoubleDouble);
...
DoubleAPFloat::DoubleAPFloat(const DoubleAPFloat &RHS) :
    Semantics(RHS.Semantics),
    Floats(RHS.Floats ? new APFloat[2]{APFloat(RHS.Floats[0]), APFloat(RHS.Floats[1])} : nullptr)
{
    assert(Semantics == &semPPCDoubleDouble);
}

DoubleAPFloat::DoubleAPFloat(DoubleAPFloat &&RHS) : Semantics(RHS.Semantics), Floats(std::move(RHS.Floats))
{
    RHS.Semantics = &semBogus;
    assert(Semantics == &semPPCDoubleDouble);
}
...
APFloat::opStatus
DoubleAPFloat::addImpl(const APFloat &a, const APFloat &aa, const APFloat &c, const APFloat &cc, roundingMode RM)
{
    int Status = opOK;
    APFloat z = a;
    Status |= z.add(c, RM);
...
APFloat::opStatus
DoubleAPFloat::addWithSpecial(const DoubleAPFloat &LHS, const DoubleAPFloat &RHS, DoubleAPFloat &Out, roundingMode RM)
{
    ...
    if (LHS.getCategory() == fcInfinity && RHS.getCategory() == fcInfinity && LHS.isNegative() != RHS.isNegative())
    {
    ...
    APFloat A(LHS.Floats[0]), AA(LHS.Floats[1]), C(RHS.Floats[0]), CC(RHS.Floats[1]);
...
APFloat::opStatus
DoubleAPFloat::add(const DoubleAPFloat &RHS, roundingMode RM)
{
    return addWithSpecial(*this, RHS, *this, RM);
}

APFloat::opStatus
DoubleAPFloat::subtract(const DoubleAPFloat &RHS, roundingMode RM)
{
    changeSign();
    auto Ret = add(RHS, RM);
    changeSign();
...
APFloat::opStatus
DoubleAPFloat::multiply(const DoubleAPFloat &RHS, APFloat::roundingMode RM)
{
    const auto &LHS = *this;
    auto &Out = *this;
    ...
    */
    ...
    assert(LHS.getCategory() == fcNormal && RHS.getCategory() == fcNormal && "Special cases not handled exhaustively");
...
APFloat::opStatus
DoubleAPFloat::divide(const DoubleAPFloat &RHS, APFloat::roundingMode RM)
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
    APFloat Tmp(semPPCDoubleDoubleLegacy, bitcastToAPInt());
    auto Ret = Tmp.divide(APFloat(semPPCDoubleDoubleLegacy, RHS.bitcastToAPInt()), RM);
    *this = DoubleAPFloat(semPPCDoubleDouble, Tmp.bitcastToAPInt());
    return Ret;
}

APFloat::opStatus
DoubleAPFloat::remainder(const DoubleAPFloat &RHS)
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
    APFloat Tmp(semPPCDoubleDoubleLegacy, bitcastToAPInt());
    auto Ret = Tmp.remainder(APFloat(semPPCDoubleDoubleLegacy, RHS.bitcastToAPInt()));
    *this = DoubleAPFloat(semPPCDoubleDouble, Tmp.bitcastToAPInt());
    return Ret;
}

APFloat::opStatus
DoubleAPFloat::mod(const DoubleAPFloat &RHS)
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
    APFloat Tmp(semPPCDoubleDoubleLegacy, bitcastToAPInt());
...
APFloat::opStatus
DoubleAPFloat::fusedMultiplyAdd(
...
bool
DoubleAPFloat::isNegative() const
{
    return Floats[0].isNegative();
}

void
DoubleAPFloat::makeInf(bool Neg)
{
...
void
DoubleAPFloat::makeZero(bool Neg)
{
...
void
DoubleAPFloat::makeLargest(bool Neg)
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
...
void
DoubleAPFloat::makeSmallest(bool Neg)
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
...
void
DoubleAPFloat::makeSmallestNormalized(bool Neg)
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
...
void
DoubleAPFloat::makeNaN(bool SNaN, bool Neg, const APInt *fill)
{
...
bool
DoubleAPFloat::bitwiseIsEqual(const DoubleAPFloat &RHS) const
{
    return Floats[0].bitwiseIsEqual(RHS.Floats[0]) && Floats[1].bitwiseIsEqual(RHS.Floats[1]);
}

hash_code
hash_value(const DoubleAPFloat &Arg)
{
    if (Arg.Floats)
...
APFloat::opStatus
DoubleAPFloat::convertToInteger(
...
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
    return APFloat(semPPCDoubleDoubleLegacy, bitcastToAPInt()).convertToInteger(Input, Width, IsSigned, RM, IsExact);
}

APFloat::opStatus
DoubleAPFloat::convertFromAPInt(const APInt &Input, bool IsSigned, roundingMode RM)
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
    APFloat Tmp(semPPCDoubleDoubleLegacy);
...
unsigned int
DoubleAPFloat::convertToHexString(char *DST, unsigned int HexDigits, bool UpperCase, roundingMode RM) const
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
    return APFloat(semPPCDoubleDoubleLegacy, bitcastToAPInt()).convertToHexString(DST, HexDigits, UpperCase, RM);
}

bool
DoubleAPFloat::isDenormal() const
{
    return getCategory() == fcNormal && (Floats[0].isDenormal() || Floats[1].isDenormal() ||
                                         // (double)(Hi + Lo) == Hi defines a normal number.
                                         Floats[0].compare(Floats[0] + Floats[1]) != cmpEqual);
}

bool
DoubleAPFloat::isSmallest() const
{
...
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
    APFloat(semPPCDoubleDoubleLegacy, bitcastToAPInt()).toString(Str, FormatPrecision, FormatMaxPadding, TruncateZero);
}

bool
DoubleAPFloat::getExactInverse(APFloat *inv) const
{
    assert(Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
    APFloat Tmp(semPPCDoubleDoubleLegacy, bitcastToAPInt());
    if (!inv)
...
DoubleAPFloat
scalbn(DoubleAPFloat Arg, int Exp, APFloat::roundingMode RM)
{
    assert(Arg.Semantics == &semPPCDoubleDouble && "Unexpected Semantics");
    return DoubleAPFloat(semPPCDoubleDouble, scalbn(Arg.Floats[0], Exp, RM), scalbn(Arg.Floats[1], Exp, RM));
}
...
APFloat::Storage::Storage(IEEEFloat F, const fltSemantics &Semantics)
{
    ...
    {
        new (&Double) DoubleAPFloat(Semantics, APFloat(std::move(F), F.getSemantics()), APFloat(semIEEEdouble));
        return;
    }
    llvm_unreachable("Unexpected semantics");
}
...
APFloat::opStatus
APFloat::convertFromString(StringRef Str, roundingMode RM)
{
    APFLOAT_DISPATCH_ON_SEMANTICS(convertFromString(Str, RM));
}

hash_code
hash_value(const APFloat &Arg)
{
...
APFloat::APFloat(const fltSemantics &Semantics, StringRef S) : APFloat(Semantics)
{
    convertFromString(S, rmNearestTiesToEven);
}

APFloat::opStatus
APFloat::convert(const fltSemantics &ToSemantics, roundingMode RM, bool *losesInfo)
{
    ...
    if (usesLayout<IEEEFloat>(getSemantics()) && usesLayout<IEEEFloat>(ToSemantics))
        return U.IEEE.convert(ToSemantics, RM, losesInfo);
    if (usesLayout<IEEEFloat>(getSemantics()) && usesLayout<DoubleAPFloat>(ToSemantics))
    {
    ...
    if (usesLayout<DoubleAPFloat>(getSemantics()) && usesLayout<IEEEFloat>(ToSemantics))
    {
...
void
APFloat::print(raw_ostream &OS) const
{
...
void
APFloat::Profile(FoldingSetNodeID &NID) const
{
 ...
 */
APFloat::opStatus
APFloat::convertToInteger(APSInt &result, roundingMode rounding_mode, bool *isExact) const
{
    unsigned bitWidth = result.getBitWidth();
    SmallVector<uint64_t, 4> parts(result.getNumWords());
    opStatus status = convertToInteger(parts, bitWidth, result.isSigned(), rounding_mode, isExact);
