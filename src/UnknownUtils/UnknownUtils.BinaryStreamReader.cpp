...
BinaryStreamReader::BinaryStreamReader(ArrayRef<uint8_t> Data, endianness Endian) : Stream(Data, Endian) {}

BinaryStreamReader::BinaryStreamReader(StringRef Data, endianness Endian) : Stream(Data, Endian) {}
...
Error
BinaryStreamReader::readBytes(ArrayRef<uint8_t> &Buffer, uint32_t Size)
{
...
Error
BinaryStreamReader::readCString(StringRef &Dest)
{
...
Error
BinaryStreamReader::readWideString(ArrayRef<UTF16> &Dest)
{
    uint32_t Length = 0;
    uint32_t OriginalOffset = getOffset();
    const UTF16 *C;
...
Error
BinaryStreamReader::readFixedString(StringRef &Dest, uint32_t Length)
{
...
Error
BinaryStreamReader::readStreamRef(BinaryStreamRef &Ref)
{
    return readStreamRef(Ref, bytesRemaining());
}

Error
BinaryStreamReader::readStreamRef(BinaryStreamRef &Ref, uint32_t Length)
{
...
Error
BinaryStreamReader::readSubstream(BinarySubstreamRef &Stream, uint32_t Size)
{
