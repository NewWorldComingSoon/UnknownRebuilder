...
#include "unknown/ADT/DAGDeltaAlgorithm.h"
...
DAGDeltaAlgorithmImpl::DAGDeltaAlgorithmImpl(
  ...
  LLVM_DEBUG({
    unknown::errs() << "-- DAGDeltaAlgorithmImpl --\n";
    unknown::errs() << "Changes: [";
    ...
    unknown::errs() << "Predecessor Closure:\n";
    for (changeset_ty::const_iterator it = Changes.begin(), ie = Changes.end();
         it != ie; ++it) {
      unknown::errs() << format("  %-4d: [", *it);
    ...
    unknown::errs() << "Successor Closure:\n";
    for (changeset_ty::const_iterator it = Changes.begin(), ie = Changes.end();
         it != ie; ++it) {
      unknown::errs() << format("  %-4d: [", *it);
      for (succ_closure_iterator_ty it2 = succ_closure_begin(*it),
...
DAGDeltaAlgorithmImpl::Run() {
  ...
  while (!CurrentSet.empty()) {
    LLVM_DEBUG({
      unknown::errs() << "DAG_DD - " << CurrentSet.size() << " active changes, "
