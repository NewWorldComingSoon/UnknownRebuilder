...
void unknown::install_fatal_error_handler(fatal_error_handler_t handler,
                                       void *user_data) {
#if LLVM_ENABLE_THREADS == 1
...
void unknown::remove_fatal_error_handler() {
#if LLVM_ENABLE_THREADS == 1
  std::lock_guard<std::mutex> Lock(ErrorHandlerMutex);
#endif
...
void unknown::report_fatal_error(const char *Reason, bool GenCrashDiag) {
  report_fatal_error(Twine(Reason), GenCrashDiag);
}

void unknown::report_fatal_error(const std::string &Reason, bool GenCrashDiag) {
  report_fatal_error(Twine(Reason), GenCrashDiag);
}

void unknown::report_fatal_error(StringRef Reason, bool GenCrashDiag) {
  report_fatal_error(Twine(Reason), GenCrashDiag);
}

void unknown::report_fatal_error(const Twine &Reason, bool GenCrashDiag) {
...
void unknown::install_bad_alloc_error_handler(fatal_error_handler_t handler,
                                           void *user_data) {
#if LLVM_ENABLE_THREADS == 1
...
void unknown::remove_bad_alloc_error_handler() {
#if LLVM_ENABLE_THREADS == 1
  std::lock_guard<std::mutex> Lock(BadAllocErrorHandlerMutex);
#endif
...
void unknown::report_bad_alloc_error(const char *Reason, bool GenCrashDiag) {
...
void unknown::install_out_of_memory_new_handler() {
}
#else
...
void unknown::llvm_unreachable_internal(const char *msg, const char *file,
                                     unsigned line) {
...
std::error_code unknown::mapWindowsError(unsigned EV) {
  switch (EV) {
