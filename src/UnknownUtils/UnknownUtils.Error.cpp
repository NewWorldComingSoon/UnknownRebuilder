...
// FIXME: This class is only here to support the transition to unknown::Error. It
...
{
    return std::error_code(static_cast<int>(ErrorErrorCode::MultipleErrors), *ErrorErrorCat);
}

std::error_code
inconvertibleErrorCode()
{
    return std::error_code(static_cast<int>(ErrorErrorCode::InconvertibleError), *ErrorErrorCat);
}

std::error_code
FileError::convertToErrorCode() const
{
    return std::error_code(static_cast<int>(ErrorErrorCode::FileError), *ErrorErrorCat);
}
...
errorCodeToError(std::error_code EC)
{
    ...
    return Error(unknown::make_unique<ECError>(ECError(EC)));
}
...
{
    std::error_code EC;
    handleAllErrors(std::move(Err), [&](const ErrorInfoBase &EI) { EC = EI.convertToErrorCode(); });
...
void
Error::fatalUncheckedError() const
{
...
StringError::StringError(std::error_code EC, const Twine &S) : Msg(S.str()), EC(EC) {}

StringError::StringError(const Twine &S, std::error_code EC) : Msg(S.str()), EC(EC), PrintMsgOnly(true) {}

void
StringError::log(raw_ostream &OS) const
{
    if (PrintMsgOnly)
    {
        OS << Msg;
...
void
report_fatal_error(Error Err, bool GenCrashDiag)
{
