...
#include "unknown/ADT/SmallSet.h"
...
static std::unique_ptr<unknown::MemoryBuffer>
    LLVM_ATTRIBUTE_UNUSED getProcCpuinfoContent() {
  unknown::ErrorOr<std::unique_ptr<unknown::MemoryBuffer>> Text =
      unknown::MemoryBuffer::getFileAsStream("/proc/cpuinfo");
  if (std::error_code EC = Text.getError()) {
    unknown::errs() << "Can't read "
...
#define X86_CPU_SUBTYPE(ARCHNAME, ENUM) \
  if (Subtype == X86::ENUM) \
    return ARCHNAME;
#include "unknown/Support/X86TargetParser.def"
...
StringRef sys::getHostCPUName() {
  std::unique_ptr<unknown::MemoryBuffer> P = getProcCpuinfoContent();
