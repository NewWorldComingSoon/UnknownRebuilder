...
#include "unknown/Support/ItaniumManglingCanonicalizer.h"
...
using unknown::itanium_demangle::ForwardTemplateReference;
using unknown::itanium_demangle::Node;
using unknown::itanium_demangle::NodeKind;

namespace {
struct FoldingSetNodeIDBuilder {
  unknown::FoldingSetNodeID &ID;
  void operator()(const Node *P) { ID.AddPointer(P); }
  void operator()(StringView Str) {
    ID.AddString(unknown::StringRef(Str.begin(), Str.size()));
  }
  template<typename T>
...
template<typename ...T>
void profileCtor(unknown::FoldingSetNodeID &ID, Node::Kind K, T ...V) {
...
void profileNode(unknown::FoldingSetNodeID &ID, const Node *N) {
  N->visit(ProfileNode{ID});
}

class FoldingNodeAllocator {
  class alignas(alignof(Node *)) NodeHeader : public unknown::FoldingSetNode {
  public:
    ...
    void Profile(unknown::FoldingSetNodeID &ID) { profileNode(ID, getNode()); }
  };

  BumpPtrAllocator RawAlloc;
  unknown::FoldingSet<NodeHeader> Nodes;
...
class CanonicalizerAllocator : public FoldingNodeAllocator {
  ...
  unknown::SmallDenseMap<Node*, Node*, 32> Remappings;
