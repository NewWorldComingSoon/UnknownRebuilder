...
public:
    ...
    uint8_t *getBufferEnd() const override { return (uint8_t *)Buffer->data() + Buffer->size(); }
...
{
public:
    InMemoryBuffer(StringRef Path, MemoryBlock Buf, unsigned Mode) : FileOutputBuffer(Path), Buffer(Buf), Mode(Mode) {}

    uint8_t *getBufferStart() const override { return (uint8_t *)Buffer.base(); }

    uint8_t *getBufferEnd() const override { return (uint8_t *)Buffer.base() + Buffer.size(); }
    ...
    Error commit() override
    {
        ...
        int FD;
        std::error_code EC;
        if (auto EC = openFileForWrite(FinalPath, FD, CD_CreateAlways, OF_None, Mode))
...
{
    std::error_code EC;
    MemoryBlock MB = Memory::allocateMappedMemory(Size, nullptr, sys::Memory::MF_READ | sys::Memory::MF_WRITE, EC);
    if (EC)
        return errorCodeToError(EC);
    return unknown::make_unique<InMemoryBuffer>(Path, MB, Mode);
}

static Expected<std::unique_ptr<OnDiskBuffer>>
createOnDiskBuffer(StringRef Path, size_t Size, bool InitExisting, unsigned Mode)
{
    Expected<fs::TempFile> FileOrErr = fs::TempFile::create(Path + ".tmp%%%%%%%", Mode);
    if (!FileOrErr)
    ...
    std::error_code EC;
    auto MappedFile =
        unknown::make_unique<fs::mapped_file_region>(File.FD, fs::mapped_file_region::readwrite, Size, 0, EC);
    if (EC)
    {
        consumeError(File.discard());
        return errorCodeToError(EC);
    }
    return unknown::make_unique<OnDiskBuffer>(Path, std::move(File), std::move(MappedFile));
}
