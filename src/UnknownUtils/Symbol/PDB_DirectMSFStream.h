...
public:
    ...
    explicit DirectMSFStream(const void *data, uint32_t blockSize, const uint32_t *blockIndices, uint32_t streamSize)
        PDB_NO_EXCEPT;
    ...
    PDB_NO_DISCARD inline uint32_t GetBlockSize(void) const PDB_NO_EXCEPT { return m_blockSize; }

    // Returns the size of the stream.
    PDB_NO_DISCARD inline uint32_t GetSize(void) const PDB_NO_EXCEPT { return m_size; }
...
private:
    ...
    PDB_NO_DISCARD inline const void *GetData(void) const PDB_NO_EXCEPT { return m_data; }

    // Provides read-only access to the block indices.
    PDB_NO_DISCARD inline const uint32_t *GetBlockIndices(void) const PDB_NO_EXCEPT { return m_blockIndices; }
