...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
PDB::TPIStream::TPIStream(void) PDB_NO_EXCEPT : m_header(), m_stream(), m_records(nullptr), m_recordCount(0u) {}

// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
PDB::TPIStream::TPIStream(TPIStream &&other) PDB_NO_EXCEPT : m_header(PDB_MOVE(other.m_header)),
...
// ------------------------------------------------------------------------------------------------
PDB::TPIStream &
PDB::TPIStream::operator=(TPIStream &&other) PDB_NO_EXCEPT
{
    if (this != &other)
    {
      ...
      m_recordCount(GetLastTypeIndex() - GetFirstTypeIndex())
{
    ...
    // however, the index is not stored with types in the TPI stream directly, but has to be built while walking the
    // stream. similarly, because types are variable-length records, there are no direct offsets to access individual
    // types. we therefore walk the TPI stream once, and store pointers to the records for trivial O(N) array lookup by
    // index later.
...
// ------------------------------------------------------------------------------------------------
// ------------------------------------------------------------------------------------------------
PDB_NO_DISCARD PDB::TPIStream
PDB::CreateTPIStream(const RawFile &file) PDB_NO_EXCEPT
{
    DirectMSFStream stream = file.CreateMSFStream<DirectMSFStream>(TPIStreamIndex);
