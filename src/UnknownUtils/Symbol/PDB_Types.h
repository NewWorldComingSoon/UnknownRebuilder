...
namespace PDB {
// emulating std::byte from C++17 to make the intention clear that we're dealing with untyped data in certain cases,
// without actually requiring C++17
...
struct PDB_NO_DISCARD SuperBlock
{
    ...
    PDB_FLEXIBLE_ARRAY_MEMBER(
        uint32_t,
        directoryBlockIndices); // indices of the blocks that make up the directory indices
};
