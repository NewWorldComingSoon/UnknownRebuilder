...
PDB_NO_DISCARD inline size_t
ConvertBlockIndexToFileOffset(uint32_t blockIndex, uint32_t blockSize) PDB_NO_EXCEPT
{
...
PDB_NO_DISCARD inline uint32_t
ConvertSizeToBlockCount(uint32_t sizeInBytes, uint32_t blockSize) PDB_NO_EXCEPT
{
...
template <typename T>
PDB_NO_DISCARD inline uint32_t
GetCodeViewRecordSize(const T *record) PDB_NO_EXCEPT
{
...
PDB_NO_DISCARD inline size_t
GetNameLength(const Header &header, const T &record) PDB_NO_EXCEPT
{
