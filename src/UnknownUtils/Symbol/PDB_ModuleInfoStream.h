...
public:
    ...
    public:
        ...
        explicit Module(
            ...
            size_t objectNameLength) PDB_NO_EXCEPT;
    ...
    private:
        // the module info is stored in variable-length arrays inside the stream, so rather than store an array
        // directly, we need to store pointers to the individual data items inside the stream.
        ...
        // the name of the object file. either the same as the module name, or the path to the archive that contained
        // the module
    ...
    PDB_NO_DISCARD inline const Module &GetModule(uint32_t index) const PDB_NO_EXCEPT { return m_modules[index]; }
