...
public:
    ...
    explicit CoalescedMSFStream(const void *data, uint32_t blockSize, const uint32_t *blockIndices, uint32_t streamSize)
        PDB_NO_EXCEPT;
    ...
    PDB_NO_DISCARD inline size_t GetSize(void) const PDB_NO_EXCEPT { return m_size; }
    ...
    PDB_NO_DISCARD inline size_t GetPointerOffset(const T *pointer) const PDB_NO_EXCEPT
    {
        ...
        PDB_ASSERT(
            bytePointer >= m_data && bytePointer <= dataEnd,
            "Pointer 0x%016" PRIXPTR " not within stream range [0x%016" PRIXPTR ":0x%016" PRIXPTR "]",
            reinterpret_cast<uintptr_t>(bytePointer),
