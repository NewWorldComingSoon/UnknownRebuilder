...
#include "unknown/Support/NativeFormatting.h"
...
raw_ostream::~raw_ostream()
{
    ...
    assert(OutBufCur == OutBufStart && "raw_ostream destructor called with non-empty buffer!");
...
size_t
raw_ostream::preferred_buffer_size() const
{
...
{
    assert(
        ((Mode == Unbuffered && !BufferStart && Size == 0) || (Mode != Unbuffered && BufferStart && Size != 0)) &&
...
{
    unknown::write_hex(*this, N, HexPrintStyle::Lower);
    return *this;
}
...
raw_ostream &
raw_ostream::write_escaped(StringRef Str, bool UseHexEscapes)
{
...
{
    unknown::write_hex(*this, (uintptr_t)P, HexPrintStyle::PrefixLower);
    return *this;
}
...
{
    unknown::write_double(*this, N, FloatStyle::Exponent);
    return *this;
}

void
raw_ostream::flush_nonempty()
{
...
void
raw_ostream::copy_to_buffer(const char *Ptr, size_t Size)
{
...
raw_ostream::operator<<(const FormattedNumber &FN)
{
    if (FN.Hex)
    {
        ...
        unknown::write_hex(*this, FN.HexValue, Style, FN.Width);
    }
    else
    {
        unknown::SmallString<16> Buffer;
        unknown::raw_svector_ostream Stream(Buffer);
        unknown::write_integer(Stream, FN.DecValue, 0, IntegerStyle::Integer);
...
raw_ostream::operator<<(const FormattedBytes &FB)
{
    ...
    if (FB.FirstByteOffset.hasValue())
    {
        ...
        unsigned Power = 0;
        if (MaxOffset > 0)
            Power = unknown::Log2_64_Ceil(MaxOffset);
        OffsetWidth = std::max<uint64_t>(4, unknown::alignTo(Power, 4) / 4);
    }

    // The width of a block of data including all spaces for group separators.
    unsigned NumByteGroups = alignTo(FB.NumPerLine, FB.ByteGroupSize) / FB.ByteGroupSize;
    ...
    while (!Bytes.empty())
    {
        ...
        if (FB.FirstByteOffset.hasValue())
        {
            ...
            unknown::write_hex(*this, Offset + LineIndex, HPS, OffsetWidth);
            *this << ": ";
        }
        ...
        for (size_t I = 0; I < Line.size(); ++I, CharsPrinted += 2)
        {
            ...
            unknown::write_hex(*this, Line[I], HPS, 2);
        }
...
template <char C>
static raw_ostream &
write_padding(raw_ostream &OS, unsigned NumChars)
{
    static const char Chars[] = {C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C, C,
    ...
    while (NumChars)
    {
        unsigned NumToWrite = std::min(NumChars, (unsigned)array_lengthof(Chars) - 1);
...
{
    assert((Access & sys::fs::FA_Write) && "Cannot make a raw_ostream from a read-only descriptor!");
...
raw_fd_ostream::raw_fd_ostream(StringRef Filename, std::error_code &EC) :
    raw_fd_ostream(Filename, EC, sys::fs::CD_CreateAlways, sys::fs::FA_Write, sys::fs::OF_None)
{
}
...
raw_fd_ostream::raw_fd_ostream(StringRef Filename, std::error_code &EC, sys::fs::FileAccess Access) :
    raw_fd_ostream(Filename, EC, sys::fs::CD_CreateAlways, Access, sys::fs::OF_None)
{
}

raw_fd_ostream::raw_fd_ostream(StringRef Filename, std::error_code &EC, sys::fs::OpenFlags Flags) :
    raw_fd_ostream(Filename, EC, sys::fs::CD_CreateAlways, sys::fs::FA_Write, Flags)
{
}
    ...
    raw_pwrite_stream(unbuffered), FD(fd), ShouldClose(shouldClose)
{
    ...
    IsWindowsConsole = ::GetFileType((HANDLE)::_get_osfhandle(fd)) == FILE_TYPE_CHAR;
#endif
...
raw_fd_ostream::~raw_fd_ostream()
{
    ...
    if (has_error())
        report_fatal_error(
            "IO failure on output stream: " + error().message(),
            /*GenCrashDiag=*/false);
}
...
static bool
write_console_impl(int FD, StringRef Data)
{
    ...
    do
    {
        size_t WCharsToWrite = std::min(MaxWriteSize, WideText.size() - WCharsWritten);
        DWORD ActuallyWritten;
        bool Success = ::WriteConsoleW(
            (HANDLE)::_get_osfhandle(FD),
            &WideText[WCharsWritten],
            WCharsToWrite,
            &ActuallyWritten,
            /*Reserved=*/nullptr);
...
void
raw_fd_ostream::write_impl(const char *Ptr, size_t Size)
{
...
uint64_t
raw_fd_ostream::seek(uint64_t off)
{
...
void
raw_fd_ostream::pwrite_impl(const char *Ptr, size_t Size, uint64_t Offset)
{
    uint64_t Pos = tell();
    seek(Offset);
    write(Ptr, Size);
...
size_t
raw_fd_ostream::preferred_buffer_size() const
{
#if defined(_WIN32)
...
raw_ostream &
raw_fd_ostream::changeColor(enum Colors colors, bool bold, bool bg)
{
    if (sys::Process::ColorNeedsFlush())
        flush();
    const char *colorcode =
        (colors == SAVEDCOLOR) ? sys::Process::OutputBold(bg) : sys::Process::OutputColor(colors, bold, bg);
    if (colorcode)
    {
...
bool
raw_fd_ostream::is_displayed() const
{
...
void
raw_string_ostream::write_impl(const char *Ptr, size_t Size)
{
    OS.append(Ptr, Size);
}
...
uint64_t
raw_svector_ostream::current_pos() const
{
    return OS.size();
}
...
void
raw_svector_ostream::pwrite_impl(const char *Ptr, size_t Size, uint64_t Offset)
{
...
void
raw_null_ostream::write_impl(const char *Ptr, size_t Size)
{
}

uint64_t
raw_null_ostream::current_pos() const
{
    return 0;
}

void
raw_null_ostream::pwrite_impl(const char *Ptr, size_t Size, uint64_t Offset)
{
}

void
raw_pwrite_stream::anchor()
{
}

void
buffer_ostream::anchor()
{
}
