...
#include "unknown/Support/JSON.h"
...
unknown::Optional<std::nullptr_t> Object::getNull(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsNull();
  return unknown::None;
}
unknown::Optional<bool> Object::getBoolean(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsBoolean();
  return unknown::None;
}
unknown::Optional<double> Object::getNumber(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsNumber();
  return unknown::None;
}
unknown::Optional<int64_t> Object::getInteger(StringRef K) const {
  if (auto *V = get(K))
    return V->getAsInteger();
  return unknown::None;
}
unknown::Optional<unknown::StringRef> Object::getString(StringRef K) const {
  if (auto *V = get(K))
...
bool Parser::parseError(const char *Msg) {
  ...
  Err.emplace(
      unknown::make_unique<ParseError>(Msg, Line, P - StartOfLine, P - Start));
  return false;
}
} // namespace
...
bool isUTF8(unknown::StringRef S, size_t *ErrOffset) {
...
static void quote(unknown::raw_ostream &OS, unknown::StringRef S) {
  OS << '\"';
  for (unsigned char C : S) {
    ...
    default:
      OS << 'u';
      unknown::write_hex(OS, C, unknown::HexPrintStyle::Lower, 4);
      break;
    }
  }
  OS << '\"';
...
void unknown::json::Value::print(raw_ostream &OS, const Indenter &I) const {
  switch (Type) {
  case T_Null:
    OS << "null";
    break;
...
void unknown::format_provider<unknown::json::Value>::format(
    const unknown::json::Value &E, raw_ostream &OS, StringRef Options) {
  if (Options.empty()) {
    OS << E;
    return;
  }
...
unknown::raw_ostream &unknown::json::operator<<(raw_ostream &OS, const Value &E) {
