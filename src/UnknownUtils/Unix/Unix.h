...
#include "unknown/Config/config.h" // Get autoconf configuration settings
...
static inline bool
MakeErrMsg(std::string *ErrMsg, const std::string &prefix, int errnum = -1)
{
    if (!ErrMsg)
        return true;
    if (errnum == -1)
        errnum = errno;
    *ErrMsg = prefix + ": " + unknown::sys::StrError(errnum);
    return true;
}
...
toDuration(const struct timeval &TV)
{
...
toTimeVal(TimePoint<std::chrono::microseconds> TP)
{
    using namespace std::chrono;
