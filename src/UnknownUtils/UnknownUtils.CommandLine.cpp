...
#include "unknown/Support/StringSaver.h"
...
OptionCategory unknown::cl::GeneralCategory("General options");
...
ManagedStatic<SubCommand> unknown::cl::TopLevelSubCommand;

// A special subcommand that can be used to put an option into all subcommands.
ManagedStatic<SubCommand> unknown::cl::AllSubCommands;
    ...
    for (unsigned I = 0; I < NewArgv.size(); ++I)
      if (NewArgv[I]) {
        ...
        if (Arg.front() == '@') {
          ...
          if (unknown::sys::path::is_relative(FileName)) {
            SmallString<128> ResponseFile;
            ResponseFile.append(1, '@');
            if (unknown::sys::path::is_relative(FName)) {
              SmallString<128> curr_dir;
              unknown::sys::fs::current_path(curr_dir);
              ResponseFile.append(curr_dir.str());
            }
            unknown::sys::path::append(
                ResponseFile, unknown::sys::path::parent_path(FName), FileName);
...
void cl::ParseEnvironmentOptions(const char *progName, const char *envVar,
  ...
  unknown::Optional<std::string> envValue = sys::Process::GetEnv(StringRef(envVar));
  if (!envValue)
    return;
...
bool cl::ParseCommandLineOptions(int argc, const char *const *argv,
  ...
  if (EnvVar) {
    if (unknown::Optional<std::string> EnvValue =
...
void LLVMParseCommandLineOptions(int argc, const char *const *argv,
                                 const char *Overview) {
  unknown::cl::ParseCommandLineOptions(argc, argv, StringRef(Overview),
                                    &unknown::nulls());
}
