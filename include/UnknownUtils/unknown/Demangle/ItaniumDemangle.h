...
public:
    ...
    /* implicit */ NodeOrString(Node *N) : First(static_cast<const void *>(N)), Second(nullptr) {}
...
public:
    SpecialName(StringView Special_, const Node *Child_) : Node(KSpecialName), Special(Special_), Child(Child_) {}
...
public:
    PixelVectorType(NodeOrString Dimension_) : Node(KPixelVectorType), Dimension(Dimension_) {}
...
public:
    ClosureTypeName(NodeArray Params_, StringView Count_) : Node(KClosureTypeName), Params(Params_), Count(Count_) {}
...
public:
    IntegerCastExpr(const Node *Ty_, StringView Integer_) : Node(KIntegerCastExpr), Ty(Ty_), Integer(Integer_) {}
...
struct AbstractManglingParser
{
    ...
    {
        // gmh modified
        char *Stable = static_cast<char *>(DefaultAlloc.allocateBytes(Borrowed.size() + 1));
