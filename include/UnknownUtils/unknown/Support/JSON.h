...
bool isUTF8(unknown::StringRef S, size_t *ErrOffset = nullptr);
...
std::string fixUTF8(unknown::StringRef S);

class Array;
class ObjectKey;
class Value;
template <typename T> Value toJSON(const unknown::Optional<T> &Opt);
...
public:
  ...
  Value(const unknown::SmallVectorImpl<char> &V)
...
private:
  ...
  mutable unknown::AlignedCharArrayUnion<bool, double, int64_t, unknown::StringRef,
...
unknown::raw_ostream &operator<<(unknown::raw_ostream &, const Value &);
...
public:
  ...
  ObjectKey(const unknown::SmallVectorImpl<char> &V)
...
template <typename T> bool fromJSON(const Value &E, unknown::Optional<T> &Out) {
  if (E.getAsNull()) {
    Out = unknown::None;
    return true;
  }
...
public:
  ...
  template <typename T> bool map(StringRef Prop, unknown::Optional<T> &Out) {
    assert(*this && "Must check this is an object before calling map()");
    if (const Value *E = O->get(Prop))
      return fromJSON(*E, Out);
    Out = unknown::None;
...
unknown::Expected<Value> parse(unknown::StringRef JSON);
...
template <> struct format_provider<unknown::json::Value> {
