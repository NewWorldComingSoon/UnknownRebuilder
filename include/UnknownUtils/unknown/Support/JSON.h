...
#include "unknown/Support/FormatVariadic.h"
...
bool
isUTF8(unknown::StringRef S, size_t *ErrOffset = nullptr);
...
template <typename T>
Value
toJSON(const unknown::Optional<T> &Opt);

/// An Object is a JSON object, which maps strings to heterogenous JSON values.
/// It simulates DenseMap<ObjectKey, Value>. ObjectKey is a maybe-owned string.
class Object
{
    using Storage = DenseMap<ObjectKey, Value, unknown::DenseMapInfo<StringRef>>;
    Storage M;
...
public:
    ...
    unknown::Optional<std::nullptr_t> getNull(StringRef K) const;
    unknown::Optional<bool> getBoolean(StringRef K) const;
    unknown::Optional<double> getNumber(StringRef K) const;
    unknown::Optional<int64_t> getInteger(StringRef K) const;
    unknown::Optional<unknown::StringRef> getString(StringRef K) const;
...
inline bool
operator!=(const Array &L, const Array &R)
{
    return !(L == R);
}
...
public:
    ...
    Value(json::Array &&Elements) : Type(T_Array) { create<json::Array>(std::move(Elements)); }
    template <typename Elt>
    Value(const std::vector<Elt> &C) : Value(json::Array(C))
    {
    }
    Value(json::Object &&Properties) : Type(T_Object) { create<json::Object>(std::move(Properties)); }
    template <typename Elt>
    ...
    Value(const unknown::SmallVectorImpl<char> &V) : Value(std::string(V.begin(), V.end())){};
    Value(const unknown::formatv_object_base &V) : Value(V.str()){};
    // Strings: types with reference semantics. Must be valid UTF-8.
    Value(StringRef V) : Type(T_StringRef)
    {
        create<unknown::StringRef>(V);
        if (LLVM_UNLIKELY(!isUTF8(V)))
        {
            assert(false && "Invalid UTF-8 in value used as JSON");
    ...
    template <
        typename T,
        typename = typename std::enable_if<std::is_same<Value, decltype(toJSON(*(const T *)nullptr))>::value>,
        Value * = nullptr>
    ...
    unknown::Optional<std::nullptr_t> getAsNull() const
    {
        if (LLVM_LIKELY(Type == T_Null))
            return nullptr;
        return unknown::None;
    }
    unknown::Optional<bool> getAsBoolean() const
    {
        if (LLVM_LIKELY(Type == T_Boolean))
            return as<bool>();
        return unknown::None;
    }
    unknown::Optional<double> getAsNumber() const
    {
    ...
    unknown::Optional<int64_t> getAsInteger() const
    {
        if (LLVM_LIKELY(Type == T_Integer))
            return as<int64_t>();
        if (LLVM_LIKELY(Type == T_Double))
        {
            double D = as<double>();
            if (LLVM_LIKELY(
                    std::modf(D, &D) == 0.0 && D >= double(std::numeric_limits<int64_t>::min()) &&
                    D <= double(std::numeric_limits<int64_t>::max())))
                return D;
        }
        return unknown::None;
    }
    unknown::Optional<unknown::StringRef> getAsString() const
    {
        if (Type == T_String)
            return unknown::StringRef(as<std::string>());
        if (LLVM_LIKELY(Type == T_StringRef))
            return as<unknown::StringRef>();
        return unknown::None;
    }
    const json::Object *getAsObject() const { return LLVM_LIKELY(Type == T_Object) ? &as<json::Object>() : nullptr; }
    json::Object *getAsObject() { return LLVM_LIKELY(Type == T_Object) ? &as<json::Object>() : nullptr; }
    const json::Array *getAsArray() const { return LLVM_LIKELY(Type == T_Array) ? &as<json::Array>() : nullptr; }
    json::Array *getAsArray() { return LLVM_LIKELY(Type == T_Array) ? &as<json::Array>() : nullptr; }
    ...
    friend unknown::raw_ostream &operator<<(unknown::raw_ostream &, const Value &);
...
private:
    ...
    void print(unknown::raw_ostream &, const Indenter &) const;
    friend struct unknown::format_provider<unknown::json::Value>;
    ...
    mutable unknown::
        AlignedCharArrayUnion<bool, double, int64_t, unknown::StringRef, std::string, json::Array, json::Object>
            Union;
...
unknown::raw_ostream &
operator<<(unknown::raw_ostream &, const Value &);
...
public:
    ...
    ObjectKey(unknown::StringRef S) : Data(S)
    {
    ...
    ObjectKey(const unknown::SmallVectorImpl<char> &V) : ObjectKey(std::string(V.begin(), V.end())) {}
    ObjectKey(const unknown::formatv_object_base &V) : ObjectKey(V.str()) {}
    ...
    operator unknown::StringRef() const { return Data; }
...
private:
    ...
    unknown::StringRef Data;
};

inline bool
operator==(const ObjectKey &L, const ObjectKey &R)
{
    return unknown::StringRef(L) == unknown::StringRef(R);
}
inline bool
operator!=(const ObjectKey &L, const ObjectKey &R)
{
...
inline bool
fromJSON(const Value &E, std::string &Out)
{
...
inline bool
fromJSON(const Value &E, int &Out)
{
...
inline bool
fromJSON(const Value &E, int64_t &Out)
{
    if (auto S = E.getAsInteger())
    {
        Out = *S;
...
inline bool
fromJSON(const Value &E, double &Out)
{
...
inline bool
fromJSON(const Value &E, bool &Out)
{
...
template <typename T>
bool
fromJSON(const Value &E, unknown::Optional<T> &Out)
{
    if (E.getAsNull())
    {
        Out = unknown::None;
...
template <typename T>
bool
fromJSON(const Value &E, std::vector<T> &Out)
{
...
template <typename T>
bool
fromJSON(const Value &E, std::map<std::string, T> &Out)
{
    if (auto *O = E.getAsObject())
    {
        Out.clear();
        for (const auto &KV : *O)
            if (!fromJSON(KV.second, Out[unknown::StringRef(KV.first)]))
                return false;
        return true;
    }
    return false;
...
public:
    ...
    template <typename T>
    bool map(StringRef Prop, unknown::Optional<T> &Out)
    {
        assert(*this && "Must check this is an object before calling map()");
        if (const Value *E = O->get(Prop))
            return fromJSON(*E, Out);
...
parse(unknown::StringRef JSON);

class ParseError : public unknown::ErrorInfo<ParseError>
{
    const char *Msg;
...
public:
    ...
    {
        OS << unknown::formatv("[{0}:{1}, byte={2}]: {3}", Line, Column, Offset, Msg);
    }
...
template <>
struct format_provider<unknown::json::Value>
{
