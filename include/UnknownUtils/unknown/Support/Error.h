...
public:
    ...
    // ClassID.
    virtual bool isA(const void *const ClassID) const { return ClassID == classID(); }
...
private:
    ...
    {
        return reinterpret_cast<ErrorInfoBase *>(reinterpret_cast<uintptr_t>(Payload) & ~static_cast<uintptr_t>(0x1));
    }
    ...
    {
        Payload = reinterpret_cast<ErrorInfoBase *>(
            (reinterpret_cast<uintptr_t>(Payload) & ~static_cast<uintptr_t>(0x1)) | (V ? 0 : 1));
    }
...
Error
make_error(ArgTs &&...Args)
{
    return Error(unknown::make_unique<ErrT>(std::forward<ArgTs>(Args)...));
}
...
public:
    ...
    bool isA(const void *const ClassID) const override { return ClassID == classID() || ParentErrT::isA(ClassID); }
};
...
private:
    ErrorList(std::unique_ptr<ErrorInfoBase> Payload1, std::unique_ptr<ErrorInfoBase> Payload2)
    {
        assert(
            !Payload1->isA<ErrorList>() && !Payload2->isA<ErrorList>() &&
    ...
    static Error join(Error E1, Error E2)
    {
        ...
        return Error(std::unique_ptr<ErrorList>(new ErrorList(E1.takePayload(), E2.takePayload())));
    }
...
///   @endcode
inline void
cantFail(Error Err, const char *Msg = nullptr)
{
    if (Err)
    {
        if (!Msg)
...
class ErrorHandlerTraits : public ErrorHandlerTraits<decltype(&std::remove_reference<HandlerT>::type::operator())>
{
};
...
{
public:
    static bool appliesTo(const ErrorInfoBase &E) { return E.template isA<ErrT>(); }
...
class ErrorHandlerTraits<RetT (C::*)(ErrT &)> : public ErrorHandlerTraits<RetT (&)(ErrT &)>
{
};

// Specialization for member functions of the form 'RetT (const ErrT&) const'.
template <typename C, typename RetT, typename ErrT>
class ErrorHandlerTraits<RetT (C::*)(ErrT &) const> : public ErrorHandlerTraits<RetT (&)(ErrT &)>
{
};

// Specialization for member functions of the form 'RetT (const ErrT&)'.
template <typename C, typename RetT, typename ErrT>
class ErrorHandlerTraits<RetT (C::*)(const ErrT &)> : public ErrorHandlerTraits<RetT (&)(ErrT &)>
{
};

// Specialization for member functions of the form 'RetT (const ErrT&) const'.
template <typename C, typename RetT, typename ErrT>
class ErrorHandlerTraits<RetT (C::*)(const ErrT &) const> : public ErrorHandlerTraits<RetT (&)(ErrT &)>
{
};
...
Error
handleErrorImpl(std::unique_ptr<ErrorInfoBase> Payload, HandlerT &&Handler, HandlerTs &&...Handlers)
{
    if (ErrorHandlerTraits<HandlerT>::appliesTo(*Payload))
        return ErrorHandlerTraits<HandlerT>::apply(std::forward<HandlerT>(Handler), std::move(Payload));
...
template <typename... HandlerTs>
Error
handleErrors(Error E, HandlerTs &&...Hs)
{
    if (!E)
        return Error::success();
    ...
    if (Payload->isA<ErrorList>())
    {
        ...
        for (auto &P : List.Payloads)
            R = ErrorList::join(std::move(R), handleErrorImpl(std::move(P), std::forward<HandlerTs>(Hs)...));
        return R;
    }
...
template <typename... HandlerTs>
void
handleAllErrors(Error E, HandlerTs &&...Handlers)
{
...
Expected<T>
handleExpected(Expected<T> ValOrErr, RecoveryFtor &&RecoveryPath, HandlerTs &&...Handlers)
{
    if (ValOrErr)
        return ValOrErr;

    if (auto Err = handleErrors(ValOrErr.takeError(), std::forward<HandlerTs>(Handlers)...))
        return std::move(Err);
...
toString(Error E)
{
    ...
    handleAllErrors(std::move(E), [&Errors](const ErrorInfoBase &EI) { Errors.push_back(EI.message()); });
...
template <typename T>
Expected<T>
errorOrToExpected(ErrorOr<T> &&EO)
{
...
template <typename T>
ErrorOr<T>
expectedToErrorOr(Expected<T> &&E)
{
...
///   @code{.cpp}
///   Expected<int> foo() {
///      return unknown::make_error<PDBError>(pdb_error_code::dia_failed_loading,
...
Error
createStringError(std::error_code EC, char const *Fmt, const Ts &...Vals)
{
    std::string Buffer;
...
Error
createStringError(std::error_code EC, char const *Msg);
...
private:
    FileError(std::string F, std::unique_ptr<ErrorInfoBase> E)
    {
        ...
        assert(!F.empty() && "The file name provided to FileError must not be empty.");
        FileName = F;
        Err = std::move(E);
    }
...
/// Error is unchecked.
inline Error
createFileError(std::string F, Error E)
{
    return FileError::build(F, std::move(E));
}

Error
createFileError(std::string F, ErrorSuccess) = delete;
...
public:
    ...
    ExitOnError(std::string Banner = "", int DefaultErrorExitCode = 1) :
        Banner(std::move(Banner)), GetExitCode([=](const Error &) { return DefaultErrorExitCode; })
    {
    }
...
{
    return Error(std::unique_ptr<ErrorInfoBase>(reinterpret_cast<ErrorInfoBase *>(ErrRef)));
}
