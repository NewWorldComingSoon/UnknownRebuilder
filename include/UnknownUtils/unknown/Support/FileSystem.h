...
//===----------------------------------------------------------------------===//
//
// This file declares the unknown::sys::fs namespace. It is designed after
...
inline perms
operator|(perms l, perms r)
{
    return static_cast<perms>(static_cast<unsigned short>(l) | static_cast<unsigned short>(r));
}
inline perms
operator&(perms l, perms r)
{
...
operator~(perms x)
{
    ...
    return static_cast<perms>(static_cast<unsigned short>(~static_cast<unsigned short>(x)));
}
...
public:
    ...
    bool operator==(const UniqueID &Other) const { return Device == Other.Device && File == Other.File; }
    bool operator!=(const UniqueID &Other) const { return !(*this == Other); }
    bool operator<(const UniqueID &Other) const { return std::tie(Device, File) < std::tie(Other.Device, Other.File); }
...
public:
    ...
    uint64_t getSize() const { return (uint64_t(FileSizeHigh) << 32) + FileSizeLow; }
#endif
...
public:
    ...
    file_status(
        ...
        gid_t GID,
        off_t Size) :
        basic_file_status(Type, Perms, ATime, ATimeNSec, MTime, MTimeNSec, UID, GID, Size),
        fs_st_dev(Dev),
        fs_st_nlinks(Links),
        fs_st_ino(Ino)
    {
...
std::error_code
create_directories(const Twine &path, bool IgnoreExisting = true, perms Perms = owner_all | group_all);
...
///          error if the directory already existed.
std::error_code
create_directory(const Twine &path, bool IgnoreExisting = true, perms Perms = owner_all | group_all);
...
/// specific error_code.
std::error_code
create_link(const Twine &to, const Twine &from);
...
/// specific error_code.
std::error_code
create_hard_link(const Twine &to, const Twine &from);
...
///                     directory.
std::error_code
real_path(const Twine &path, SmallVectorImpl<char> &output, bool expand_tilde = false);
...
/// @param path The path to resolve.
void
expand_tilde(const Twine &path, SmallVectorImpl<char> &output);
...
///          otherwise a platform-specific error_code.
std::error_code
set_current_path(const Twine &path);
...
std::error_code
remove(const Twine &path, bool IgnoreNonExisting = true);
...
std::error_code
remove_directories(const Twine &path, bool IgnoreErrors = true);
...
std::error_code
rename(const Twine &from, const Twine &to);
...
///          platform-specific error_code.
std::error_code
access(const Twine &Path, AccessMode Mode);
...
///          platform-specific error_code.
std::error_code
equivalent(const Twine &A, const Twine &B, bool &result);

/// Simpler version of equivalent for clients that don't need to
///        differentiate between an error and false.
inline bool
equivalent(const Twine &A, const Twine &B)
{
    bool result;
...
file_type
get_file_type(const Twine &Path, bool Follow = true);
...
///        differentiate between an error and false.
inline bool
is_regular_file(const Twine &Path)
{
    bool Result;
...
///        differentiate between an error and false.
inline bool
is_symlink_file(const Twine &Path)
{
    bool Result;
...
///          platform-specific error_code.
std::error_code
status(const Twine &path, file_status &result, bool follow = true);
...
std::error_code
setPermissions(const Twine &Path, perms Permissions);
...
file_size(const Twine &Path, uint64_t &Result)
{
    file_status Status;
...
std::error_code
setLastAccessAndModificationTime(int FD, TimePoint<> AccessTime, TimePoint<> ModificationTime);

/// Simpler version that sets both file modification and access time to the same
/// time.
inline std::error_code
setLastAccessAndModificationTime(int FD, TimePoint<> Time)
{
...
///          platform-specific error_code.
std::error_code
status_known(const Twine &path, bool &result);
...
std::error_code
createUniqueFile(const Twine &Model, SmallVectorImpl<char> &ResultPath, unsigned Mode = all_read | all_write);
...
inline FileAccess
operator|(FileAccess A, FileAccess B)
{
...
///          platform-specific error_code.
Expected<file_t>
openNativeFile(const Twine &Name, CreationDisposition Disp, FileAccess Access, OpenFlags Flags, unsigned Mode = 0666);
...
inline Expected<file_t>
openNativeFileForWrite(const Twine &Name, CreationDisposition Disp, OpenFlags Flags, unsigned Mode = 0666)
{
...
///          platform-specific error_code.
inline std::error_code
openFileForReadWrite(const Twine &Name, int &ResultFD, CreationDisposition Disp, OpenFlags Flags, unsigned Mode = 0666)
{
...
///          otherwise an error object.
inline Expected<file_t>
openNativeFileForReadWrite(const Twine &Name, CreationDisposition Disp, OpenFlags Flags, unsigned Mode = 0666)
{
...
///          platform-specific error_code.
std::error_code
openFileForRead(const Twine &Name, int &ResultFD, OpenFlags Flags = OF_None, SmallVectorImpl<char> *RealPath = nullptr);
...
///          otherwise an error object.
Expected<file_t>
openNativeFileForRead(const Twine &Name, OpenFlags Flags = OF_None, SmallVectorImpl<char> *RealPath = nullptr);
...
std::string
getMainExecutable(const char *argv0, void *MainExecAddr);
...
public:
    explicit directory_entry(
        ...
        Path(Path.str()), Type(Type), FollowSymlinks(FollowSymlinks), Status(Status)
    {
    }
...
public:
    explicit directory_iterator(const Twine &path, std::error_code &ec, bool follow_symlinks = true) :
        FollowSymlinks(follow_symlinks)
    {
        State = std::make_shared<detail::DirIterState>();
        SmallString<128> path_storage;
        ec = detail::directory_iterator_construct(*State, path.toStringRef(path_storage), FollowSymlinks);
    }

    explicit directory_iterator(const directory_entry &de, std::error_code &ec, bool follow_symlinks = true) :
        FollowSymlinks(follow_symlinks)
    {
        State = std::make_shared<detail::DirIterState>();
        ec = detail::directory_iterator_construct(*State, de.path(), FollowSymlinks);
    }
...
public:
    ...
    explicit recursive_directory_iterator(const Twine &path, std::error_code &ec, bool follow_symlinks = true) :
        State(std::make_shared<detail::RecDirIterState>()), Follow(follow_symlinks)
    {
    ...
    recursive_directory_iterator &increment(std::error_code &ec)
    {
        ...
        while (!State->Stack.empty() && State->Stack.top().increment(ec) == end_itr)
        {
            State->Stack.pop();
            --State->Level;
        }
