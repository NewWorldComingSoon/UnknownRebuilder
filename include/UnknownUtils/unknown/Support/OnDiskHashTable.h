...
#include "unknown/Support/EndianStream.h"
...
class OnDiskChainedHashTableGenerator
{
    ...
    public:
        ...
        Item(typename Info::key_type_ref Key, typename Info::data_type_ref Data, Info &InfoObj) :
    ...
    unknown::SpecificBumpPtrAllocator<Item> BA;
...
private:
    ...
    {
        Bucket *NewBuckets = static_cast<Bucket *>(safe_calloc(NewSize, sizeof(Bucket)));
...
public:
    ...
    offset_type Emit(raw_ostream &Out, Info &InfoObj)
    {
        ...
        unsigned TargetNumBuckets = NumEntries <= 2 ? 1 : NextPowerOf2(NumEntries * 4 / 3);
        ...
        for (offset_type I = 0; I < NumBuckets; ++I)
        {
            ...
            for (Item *I = B.Head; I; I = I->Next)
            {
                ...
                const std::pair<offset_type, offset_type> &Len = InfoObj.EmitKeyDataLength(Out, I->Key, I->Data);
#ifdef NDEBUG
                ...
                assert(offset_type(DataStart - KeyStart) == Len.first && "key length does not match bytes written");
                assert(offset_type(End - DataStart) == Len.second && "data length does not match bytes written");
#endif
            }
        }
        ...
        uint64_t N = unknown::OffsetToAlignment(TableOff, alignof(offset_type));
        TableOff += N;
        while (N--)
            LE.write<uint8_t>(0);
...
public:
    ...
    {
        assert((reinterpret_cast<uintptr_t>(Buckets) & 0x3) == 0 && "'buckets' must have a 4-byte alignment");
    }
    ...
    {
        assert((reinterpret_cast<uintptr_t>(Buckets) & 0x3) == 0 && "buckets should be 4-byte aligned.");
        using namespace unknown::support;
        offset_type NumBuckets = endian::readNext<offset_type, little, aligned>(Buckets);
        offset_type NumEntries = endian::readNext<offset_type, little, aligned>(Buckets);
    ...
    iterator find_hashed(const internal_key_type &IKey, hash_value_type KeyHash, Info *InfoPtr = nullptr)
    {
        ...
        {
            // Read the hash.
            hash_value_type ItemHash = endian::readNext<hash_value_type, little, unaligned>(Items);

            // Determine the length of the key and the data.
            const std::pair<offset_type, offset_type> &L = Info::ReadKeyDataLength(Items);
            ...
            // Read the key.
            const internal_key_type &X = InfoPtr->ReadKey((const unsigned char *const)Items, L.first);
    ...
    /// table.
    static OnDiskChainedHashTable *
    Create(const unsigned char *Buckets, const unsigned char *const Base, const Info &InfoObj = Info())
    {
        assert(Buckets > Base);
        auto NumBucketsAndEntries = readNumBucketsAndEntries(Buckets);
        return new OnDiskChainedHashTable<Info>(
            NumBucketsAndEntries.first, NumBucketsAndEntries.second, Buckets, Base, InfoObj);
    }
};

/// Provides lookup and iteration over an on disk hash table.
///
/// \copydetails unknown::OnDiskChainedHashTable
template <typename Info>
...
private:
    ...
    public:
        ...
        iterator_base() : Ptr(nullptr), NumItemsInBucketLeft(0), NumEntriesLeft(0) {}
        ...
        void advance()
        {
            ...
            if (!NumItemsInBucketLeft)
            {
                // 'Items' starts with a 16-bit unsigned integer representing the
                // number of items in this bucket.
                NumItemsInBucketLeft = endian::readNext<uint16_t, little, unaligned>(Ptr);
            }
            Ptr += sizeof(hash_value_type); // Skip the hash.
            // Determine the length of the key and the data.
            const std::pair<offset_type, offset_type> &L = Info::ReadKeyDataLength(Ptr);
        ...
        const unsigned char *getItem() const { return Ptr + (NumItemsInBucketLeft ? 0 : 2) + sizeof(hash_value_type); }
    };
...
public:
    ...
    public:
        ...
        value_type operator*() const { return InfoObj->GetExternalKey(getInternalKey()); }
    };

    key_iterator key_begin() { return key_iterator(Payload, this->getNumEntries(), &this->getInfoObj()); }
    key_iterator key_end() { return key_iterator(); }

    iterator_range<key_iterator> keys() { return make_range(key_begin(), key_end()); }
    ...
    data_iterator data_begin() { return data_iterator(Payload, this->getNumEntries(), &this->getInfoObj()); }
    data_iterator data_end() { return data_iterator(); }

    iterator_range<data_iterator> data() { return make_range(data_begin(), data_end()); }
    ...
    /// based. This is offset 0 in the stream that was used when Emitting the
    /// table.
    static OnDiskIterableChainedHashTable *Create(
        const unsigned char *Buckets,
    ...
    {
        assert(Buckets > Base);
        auto NumBucketsAndEntries = OnDiskIterableChainedHashTable<Info>::readNumBucketsAndEntries(Buckets);
        return new OnDiskIterableChainedHashTable<Info>(
            NumBucketsAndEntries.first, NumBucketsAndEntries.second, Buckets, Payload, Base, InfoObj);
    }
};
