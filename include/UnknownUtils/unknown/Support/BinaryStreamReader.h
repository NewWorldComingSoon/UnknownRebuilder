...
#include "unknown/Support/BinaryStreamArray.h"
...
public:
    ...
    explicit BinaryStreamReader(ArrayRef<uint8_t> Data, unknown::support::endianness Endian);
    explicit BinaryStreamReader(StringRef Data, unknown::support::endianness Endian);

    BinaryStreamReader(const BinaryStreamReader &Other) : Stream(Other.Stream), Offset(Other.Offset) {}
    ...
    {
        static_assert(std::is_integral<T>::value, "Cannot call readInteger with non-integral value!");

        ArrayRef<uint8_t> Bytes;
        if (auto EC = readBytes(Bytes, sizeof(T)))
            return EC;

        Dest = unknown::support::endian::read<T, unknown::support::unaligned>(Bytes.data(), Stream.getEndian());
    ...
    {
        static_assert(std::is_enum<T>::value, "Cannot call readEnum with non-enum value!");
    ...
    Error readArray(ArrayRef<T> &Array, uint32_t NumElements)
    {
        ...
        if (NumElements > UINT32_MAX / sizeof(T))
            return make_error<BinaryStreamError>(stream_error_code::invalid_array_size);

        if (auto EC = readBytes(Bytes, NumElements * sizeof(T)))
            return EC;

        assert(alignmentAdjustment(Bytes.data(), alignof(T)) == 0 && "Reading at invalid alignment!");
