...
public:
    explicit Timer(StringRef Name, StringRef Description) { init(Name, Description); }
    Timer(StringRef Name, StringRef Description, TimerGroup &tg) { init(Name, Description, tg); }
    Timer(const Timer &RHS) { assert(!RHS.TG && "Can only copy uninitialized timers"); }
...
class TimerGroup
{
    struct PrintRecord
    {
        ...
        PrintRecord(const TimeRecord &Time, const std::string &Name, const std::string &Description) :
...
public:
    ...
    explicit TimerGroup(StringRef Name, StringRef Description, const StringMap<TimeRecord> &Records);
...
private:
    ...
    void printJSONValue(raw_ostream &OS, const PrintRecord &R, const char *suffix, double Value);
};
