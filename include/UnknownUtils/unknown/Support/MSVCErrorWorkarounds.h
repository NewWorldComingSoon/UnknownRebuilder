...
// A default-constructible unknown::Error that is suitable for use with MSVC's
...
// A default-constructible unknown::Expected that is suitable for use with MSVC's
// std::future implementation, which requires default constructible types.
template <typename T>
class MSVCPExpected : public Expected<T>
{
public:
    MSVCPExpected() : Expected<T>(make_error<StringError>("", inconvertibleErrorCode()))
    {
