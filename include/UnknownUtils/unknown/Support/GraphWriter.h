...
#include "unknown/Support/DOTGraphTraits.h"
...
bool
DisplayGraph(StringRef Filename, bool wait = true, GraphProgram::Name program = GraphProgram::DOT);
...
public:
    GraphWriter(raw_ostream &o, const GraphType &g, bool SN) : O(o), G(g) { DTraits = DOTTraits(SN); }
    ...
    void writeEdge(NodeRef Node, unsigned edgeidx, child_iterator EI)
    {
        if (NodeRef TargetNode = *EI)
        {
            ...
            emitEdge(
                ...
                edgeidx,
                static_cast<const void *>(TargetNode),
                DestPort,
    ...
    void
    emitEdge(const void *SrcNodeID, int SrcNodePort, const void *DestNodeID, int DestNodePort, const std::string &Attrs)
    {
        if (SrcNodePort > 64)
...
template <typename GraphType>
raw_ostream &
WriteGraph(raw_ostream &O, const GraphType &G, bool ShortNames = false, const Twine &Title = "")
{
...
std::string
createGraphFilename(const Twine &Name, int &FD);
    ...
    std::string Filename = "")
{
    ...
    unknown::WriteGraph(O, G, ShortNames, Title);
    errs() << " done. \n";
...
{
    std::string Filename = unknown::WriteGraph(G, Name, ShortNames, Title);
