...
private:
    struct SrcBuffer
    {
        ...
        /// size of Buffer.
        using VariableSizeOffsets = PointerUnion4<
            std::vector<uint8_t> *,
            std::vector<uint16_t> *,
            std::vector<uint32_t> *,
            std::vector<uint64_t> *>;
...
public:
    ...
    void setIncludeDirs(const std::vector<std::string> &Dirs) { IncludeDirectories = Dirs; }
    ...
    unsigned FindLineNumber(SMLoc Loc, unsigned BufferID = 0) const { return getLineAndColumn(Loc, BufferID).first; }
    ...
    /// Emits a diagnostic to unknown::errs().
    void PrintMessage(
        SMLoc Loc,
        DiagKind Kind,
        const Twine &Msg,
...
public:
    ...
    SMFixIt(SMLoc Loc, const Twine &Insertion) : Range(Loc, Loc), Text(Insertion.str()) { assert(Loc.isValid()); }

    // FIXME: Twine.str() is not very efficient.
    SMFixIt(SMRange R, const Twine &Replacement) : Range(R), Text(Replacement.str()) { assert(R.isValid()); }
