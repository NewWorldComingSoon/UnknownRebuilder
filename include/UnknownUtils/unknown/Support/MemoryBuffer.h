...
protected:
    ...
    static constexpr sys::fs::mapped_file_region::mapmode Mapmode = sys::fs::mapped_file_region::readonly;
...
public:
    ...
    StringRef getBuffer() const { return StringRef(BufferStart, getBufferSize()); }
    ...
    getFile(const Twine &Filename, int64_t FileSize = -1, bool RequiresNullTerminator = true, bool IsVolatile = false);
    ...
    static ErrorOr<std::unique_ptr<MemoryBuffer>>
    getOpenFileSlice(int FD, const Twine &Filename, uint64_t MapSize, int64_t Offset, bool IsVolatile = false);
    ...
    /// can change outside the user's control, e.g. when libclang tries to parse
    /// while the user is editing/updating the file or if the file is on an NFS.
    static ErrorOr<std::unique_ptr<MemoryBuffer>> getOpenFile(
        int FD,
...
protected:
    ...
    static constexpr sys::fs::mapped_file_region::mapmode Mapmode = sys::fs::mapped_file_region::priv;
...
public:
    ...
    char *getBufferStart() { return const_cast<char *>(MemoryBuffer::getBufferStart()); }
    char *getBufferEnd() { return const_cast<char *>(MemoryBuffer::getBufferEnd()); }
    MutableArrayRef<char> getBuffer() { return {getBufferStart(), getBufferEnd()}; }

    static ErrorOr<std::unique_ptr<WritableMemoryBuffer>>
    getFile(const Twine &Filename, int64_t FileSize = -1, bool IsVolatile = false);
...
protected:
    ...
    static constexpr sys::fs::mapped_file_region::mapmode Mapmode = sys::fs::mapped_file_region::readwrite;
...
public:
    ...
    MemoryBufferRef(MemoryBuffer &Buffer) : Buffer(Buffer.getBuffer()), Identifier(Buffer.getBufferIdentifier()) {}
    MemoryBufferRef(StringRef Buffer, StringRef Identifier) : Buffer(Buffer), Identifier(Identifier) {}
