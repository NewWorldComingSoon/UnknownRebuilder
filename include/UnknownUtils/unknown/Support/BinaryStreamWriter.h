...
public:
    ...
    explicit BinaryStreamWriter(MutableArrayRef<uint8_t> Data, unknown::support::endianness Endian);

    BinaryStreamWriter(const BinaryStreamWriter &Other) : Stream(Other.Stream), Offset(Other.Offset) {}
    ...
    {
        static_assert(std::is_integral<T>::value, "Cannot call writeInteger with non-integral value!");
        uint8_t Buffer[sizeof(T)];
        unknown::support::endian::write<T, unknown::support::unaligned>(Buffer, Value, Stream.getEndian());
    ...
    {
        static_assert(std::is_enum<T>::value, "Cannot call writeEnum with non-Enum type");
    ...
    Error writeObject(const T &Obj)
    {
        static_assert(
            ...
            "writeObject");
        return writeBytes(ArrayRef<uint8_t>(reinterpret_cast<const uint8_t *>(&Obj), sizeof(T)));
    }
    ...
    Error writeArray(ArrayRef<T> Array)
    {
        ...
        return writeBytes(ArrayRef<uint8_t>(reinterpret_cast<const uint8_t *>(Array.data()), Array.size() * sizeof(T)));
    }
