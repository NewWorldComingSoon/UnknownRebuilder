...
#include "unknown/Support/MemAlloc.h"
#include <algorithm>
#include <cassert>
#include <cstddef>
#include <cstdint>
...
#ifdef __clang__
        static_assert(
            static_cast<void (AllocatorBase::*)(const void *, size_t)>(&AllocatorBase::Deallocate) !=
                static_cast<void (DerivedT::*)(const void *, size_t)>(&DerivedT::Deallocate),
            "Class derives from AllocatorBase without implementing the "
...
public:
    ...
    LLVM_ATTRIBUTE_RETURNS_NONNULL void *Allocate(size_t Size, size_t /*Alignment*/) { return safe_malloc(Size); }

    // Pull in base class overloads.
    using AllocatorBase<MallocAllocator>::Allocate;

    void Deallocate(const void *Ptr, size_t /*Size*/) { free(const_cast<void *>(Ptr)); }
...
void
printBumpPtrAllocatorStats(unsigned NumSlabs, size_t BytesAllocated, size_t TotalMemory);
...
public:
    ...
    template <typename T>
    BumpPtrAllocatorImpl(T &&Allocator) : Allocator(std::forward<T &&>(Allocator))
    {
    }
    ...
    LLVM_ATTRIBUTE_RETURNS_NONNULL LLVM_ATTRIBUTE_RETURNS_NOALIAS void *Allocate(size_t Size, size_t Alignment)
    {
        ...
        assert(AlignedAddr + SizeToAllocate <= (uintptr_t)End && "Unable to allocate memory!");
        char *AlignedPtr = (char *)AlignedAddr;
        CurPtr = AlignedPtr + SizeToAllocate;
        __msan_allocated_memory(AlignedPtr, Size);
        __asan_unpoison_memory_region(AlignedPtr, Size);
    ...
    // after deallocation.
    void Deallocate(const void *Ptr, size_t Size) { __asan_poison_memory_region(Ptr, Size); }
    ...
    unknown::Optional<int64_t> identifyObject(const void *Ptr)
    {
    ...
    void PrintStats() const { detail::printBumpPtrAllocatorStats(Slabs.size(), BytesAllocated, getTotalMemory()); }
...
private:
    ...
    void DeallocateSlabs(SmallVectorImpl<void *>::iterator I, SmallVectorImpl<void *>::iterator E)
    {
        for (; I != E; ++I)
        {
...
public:
    ...
    SpecificBumpPtrAllocator(SpecificBumpPtrAllocator &&Old) : Allocator(std::move(Old.Allocator)) {}
    ...
    void DestroyAll()
    {
        ...
        for (auto I = Allocator.Slabs.begin(), E = Allocator.Slabs.end(); I != E; ++I)
        {
            size_t AllocatedSlabSize = BumpPtrAllocator::computeSlabSize(std::distance(Allocator.Slabs.begin(), I));
            char *Begin = (char *)alignAddr(*I, alignof(T));
            char *End = *I == Allocator.Slabs.back() ? Allocator.CurPtr : (char *)*I + AllocatedSlabSize;
...
void *
operator new(size_t Size, unknown::BumpPtrAllocatorImpl<AllocatorT, SlabSize, SizeThreshold> &Allocator)
{
    struct S
    {
        char c;
    ...
    return Allocator.Allocate(Size, std::min((size_t)unknown::NextPowerOf2(Size), offsetof(S, x)));
}

template <typename AllocatorT, size_t SlabSize, size_t SizeThreshold>
void
operator delete(void *, unknown::BumpPtrAllocatorImpl<AllocatorT, SlabSize, SizeThreshold> &)
{
}
