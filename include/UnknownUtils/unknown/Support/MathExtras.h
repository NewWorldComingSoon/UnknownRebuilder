...
countTrailingZeros(T Val, ZeroBehavior ZB = ZB_Width)
{
    static_assert(
        ...
        "Only unsigned integral types are allowed.");
    return unknown::detail::TrailingZerosCounter<T, sizeof(T)>::count(Val, ZB);
}
...
countLeadingZeros(T Val, ZeroBehavior ZB = ZB_Width)
{
    static_assert(
        std::numeric_limits<T>::is_integer && !std::numeric_limits<T>::is_signed,
        "Only unsigned integral types are allowed.");
    return unknown::detail::LeadingZerosCounter<T, sizeof(T)>::count(Val, ZB);
}
...
template <>
constexpr inline bool
isUInt<16>(uint64_t x)
{
    return static_cast<uint16_t>(x) == x;
}
template <>
constexpr inline bool
isUInt<32>(uint64_t x)
{
...
template <typename T>
std::size_t
countTrailingOnes(T Value, ZeroBehavior ZB = ZB_Width)
{
    static_assert(
        std::numeric_limits<T>::is_integer && !std::numeric_limits<T>::is_signed,
        "Only unsigned integral types are allowed.");
...
inline uint64_t
GreatestCommonDivisor64(uint64_t A, uint64_t B)
{
    while (B)
    {
        uint64_t T = B;
...
inline uintptr_t
alignAddr(const void *Addr, size_t Alignment)
{
...
/// bytes, rounding up.
inline size_t
alignmentAdjustment(const void *Ptr, size_t Alignment)
{
...
/// non-zero.
inline uint64_t
OffsetToAlignment(uint64_t Value, uint64_t Align)
{
...
/// Requires 0 < B < 32.
inline int32_t
SignExtend32(uint32_t X, unsigned B)
{
...
/// Requires 0 < B < 64.
inline int64_t
SignExtend64(uint64_t X, unsigned B)
{
    assert(B > 0 && "Bit width can't be 0.");
