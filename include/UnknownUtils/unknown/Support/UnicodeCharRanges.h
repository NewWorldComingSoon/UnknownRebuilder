...
inline bool
operator<(uint32_t Value, UnicodeCharRange Range)
{
    return Value < Range.Lower;
}
inline bool
operator<(UnicodeCharRange Range, uint32_t Value)
{
...
#else
    UnicodeCharSet(CharRanges Ranges) : Ranges(Ranges) { assert(rangesAreValid()); }
#endif
...
private:
    ...
    {
        uint32_t Prev = 0;
        for (CharRanges::const_iterator I = Ranges.begin(), E = Ranges.end(); I != E; ++I)
        {
