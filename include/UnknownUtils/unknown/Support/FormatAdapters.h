...
#include "unknown/Support/FormatCommon.h"
#include "unknown/Support/FormatVariadicDetails.h"
#include "unknown/Support/raw_ostream.h"

namespace unknown {
template <typename T>
class FormatAdapter : public detail::format_adapter
{
protected:
...
public:
    AlignAdapter(T &&Item, AlignStyle Where, size_t Amount, char Fill) :
        FormatAdapter<T>(std::forward<T>(Item)), Where(Where), Amount(Amount), Fill(Fill)
    {
    }

    void format(unknown::raw_ostream &Stream, StringRef Style)
    {
...
public:
    PadAdapter(T &&Item, size_t Left, size_t Right) : FormatAdapter<T>(std::forward<T>(Item)), Left(Left), Right(Right)
    {
    }
...
public:
    RepeatAdapter(T &&Item, size_t Count) : FormatAdapter<T>(std::forward<T>(Item)), Count(Count) {}
...
public:
    ...
    void format(unknown::raw_ostream &Stream, StringRef Style) { Stream << Item; }
};
} // namespace detail

template <typename T>
detail::AlignAdapter<T>
fmt_align(T &&Item, AlignStyle Where, size_t Amount, char Fill = ' ')
{
