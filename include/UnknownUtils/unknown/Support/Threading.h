...
#include "unknown/Config/llvm-config.h" // for LLVM_ON_UNIX
...
#elif defined(LLVM_ON_UNIX) && (defined(_LIBCPP_VERSION) || !(defined(__NetBSD__) || defined(__OpenBSD__) || \
...
#    include <mutex>
#else
#    include "unknown/Support/Atomic.h"
#endif
...
/// the thread stack.
void
llvm_execute_on_thread(void (*UserFn)(void *), void *UserData, unsigned RequestedStackSize = 0);
...
/// The unknown::once_flag structure
///
/// This type is modeled after std::once_flag to use with unknown::call_once.
...
void
call_once(once_flag &flag, Function &&F, Args &&...ArgList)
{
#if LLVM_THREADING_USE_STD_CALL_ONCE
    std::call_once(flag, std::forward<Function>(F), std::forward<Args>(ArgList)...);
#else
