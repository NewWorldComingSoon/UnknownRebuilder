...
protected:
    ...
    explicit BinaryStreamRefBase(StreamType &BorrowedImpl) : BorrowedImpl(&BorrowedImpl), ViewOffset(0)
    {
    ...
    BinaryStreamRefBase(std::shared_ptr<StreamType> SharedImpl, uint32_t Offset, Optional<uint32_t> Length) :
        SharedImpl(SharedImpl), BorrowedImpl(SharedImpl.get()), ViewOffset(Offset), Length(Length)
    {
    }
...
public:
    unknown::support::endianness getEndian() const { return BorrowedImpl->getEndian(); }
    ...
    /// removed.
    RefType drop_symmetric(uint32_t N) const { return drop_front(N).drop_back(N); }

    /// Return a new BinaryStreamRef with the first \p Offset elements removed,
    /// and retaining exactly \p Len elements.
    RefType slice(uint32_t Offset, uint32_t Len) const { return drop_front(Offset).keep_front(Len); }
...
class BinaryStreamRef : public BinaryStreamRefBase<BinaryStreamRef, BinaryStream>
{
    friend BinaryStreamRefBase<BinaryStreamRef, BinaryStream>;
    friend class WritableBinaryStreamRef;
    BinaryStreamRef(std::shared_ptr<BinaryStream> Impl, uint32_t ViewOffset, Optional<uint32_t> Length) :
...
public:
    ...
    explicit BinaryStreamRef(ArrayRef<uint8_t> Data, unknown::support::endianness Endian);
    explicit BinaryStreamRef(StringRef Data, unknown::support::endianness Endian);
...
struct BinarySubstreamRef
{
    ...
    BinarySubstreamRef drop_front(uint32_t N) const { return slice(N, size() - N); }
...
class WritableBinaryStreamRef : public BinaryStreamRefBase<WritableBinaryStreamRef, WritableBinaryStream>
{
...
public:
    ...
    explicit WritableBinaryStreamRef(MutableArrayRef<uint8_t> Data, unknown::support::endianness Endian);
