...
struct SemiNCAInfo {
    ...
    static ResultTy Get(NodePtr N, BatchUpdatePtr BUI) {
      ...
      for (auto ChildAndKind : FCIt->second) {
        ...
        if (UK == UpdateKind::Insert) {
          ...
          assert(unknown::find(Res, Child) != Res.end()
  ...
  TreeNodePtr getNodeForBlock(NodePtr BB, DomTreeT &DT) {
    ...
    return (DT.DomTreeNodes[BB] = IDomNode->addChild(
        unknown::make_unique<DomTreeNodeBase<NodeT>>(BB, IDomNode)))
        .get();
  }
  ...
  static void CalculateFromScratch(DomTreeT &DT, BatchUpdatePtr BUI) {
    ...
    DT.RootNode = (DT.DomTreeNodes[Root] =
                       unknown::make_unique<DomTreeNodeBase<NodeT>>(Root, nullptr))
        .get();
  ...
  void attachNewSubtree(DomTreeT& DT, const TreeNodePtr AttachTo) {
    ...
    for (size_t i = 1, e = NumToNode.size(); i != e; ++i) {
      ...
      DT.DomTreeNodes[W] = IDomNode->addChild(
          unknown::make_unique<DomTreeNodeBase<NodeT>>(W, IDomNode));
    }
  }
    ...
    if (!FromTN) {
      ...
      FromTN =
          (DT.DomTreeNodes[From] = VirtualRoot->addChild(
               unknown::make_unique<DomTreeNodeBase<NodeT>>(From, VirtualRoot)))
              .get();
