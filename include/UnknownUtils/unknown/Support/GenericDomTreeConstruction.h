...
#include "unknown/ADT/DepthFirstIterator.h"
#include "unknown/ADT/PointerIntPair.h"
#include "unknown/ADT/SmallPtrSet.h"
#include "unknown/Support/Debug.h"
#include "unknown/Support/GenericDomTree.h"
...
struct SemiNCAInfo
{
    ...
    struct ChildrenGetter
    {
        ...
        static ResultTy Get(NodePtr N, BatchUpdatePtr BUI)
        {
            ...
            auto &FutureChildren = (Inverse != IsPostDom) ? BUI->FuturePredecessors : BUI->FutureSuccessors;
            ...
            for (auto ChildAndKind : FCIt->second)
            {
                ...
                if (UK == UpdateKind::Insert)
                {
                    ...
                    assert(unknown::find(Res, Child) != Res.end() && "Expected child not found in the CFG");
                    Res.erase(std::remove(Res.begin(), Res.end(), Child), Res.end());
                    LLVM_DEBUG(
                        dbgs() << "\tHiding edge " << BlockNamePrinter(N) << " -> " << BlockNamePrinter(Child) << "\n");
                }
                else
                {
                    // If there's an deletion in the future, it means that the edge cannot
                    // exist in the current CFG, but existed in it before.
                    assert(unknown::find(Res, Child) == Res.end() && "Unexpected child found in the CFG");
                    LLVM_DEBUG(
                        dbgs() << "\tShowing virtual edge " << BlockNamePrinter(N) << " -> " << BlockNamePrinter(Child)
                               << "\n");
                    Res.push_back(Child);
                }
            }
    ...
    TreeNodePtr getNodeForBlock(NodePtr BB, DomTreeT &DT)
    {
        ...
        // IDomNode
        return (DT.DomTreeNodes[BB] = IDomNode->addChild(unknown::make_unique<DomTreeNodeBase<NodeT>>(BB, IDomNode)))
            .get();
    }
    ...
    static RootsT FindRoots(const DomTreeT &DT, BatchUpdatePtr BUI)
    {
        ...
        if (Total + 1 != Num)
        {
            ...
            for (const NodePtr I : nodes(DT.Parent))
            {
                if (SNCA.NodeToInfo.count(I) == 0)
                {
                    ...
                    LLVM_DEBUG(
                        dbgs() << "\t\t\tFound a new furthest away node "
                               << "(non-trivial root): " << BlockNamePrinter(FurthestAway) << "\n");
                    ConnectToExitBlock.insert(FurthestAway);
                    Roots.push_back(FurthestAway);
                    LLVM_DEBUG(
                        dbgs() << "\t\t\tPrev DFSNum: " << Num << ", new DFSNum: " << NewNum
                               << "\n\t\t\tRemoving DFS info\n");
        ...
        LLVM_DEBUG(
            for (size_t i = 0; i <= Num; ++i) dbgs() << i << ": " << BlockNamePrinter(SNCA.NumToNode[i]) << "\n");
        ...
        LLVM_DEBUG(for (auto *Root : Roots) dbgs() << BlockNamePrinter(Root) << " ");
        LLVM_DEBUG(dbgs() << "\n");
    ...
    static void RemoveRedundantRoots(const DomTreeT &DT, BatchUpdatePtr BUI, RootsT &Roots)
    {
        ...
        for (unsigned i = 0; i < Roots.size(); ++i)
        {
            ...
            for (unsigned x = 2; x <= Num; ++x)
            {
                ...
                // one.
                if (unknown::find(Roots, N) != Roots.end())
                {
                    LLVM_DEBUG(
                        dbgs() << "\tForward DFS walk found another root " << BlockNamePrinter(N)
    ...
    static void CalculateFromScratch(DomTreeT &DT, BatchUpdatePtr BUI)
    {
        ...
        DT.RootNode = (DT.DomTreeNodes[Root] = unknown::make_unique<DomTreeNodeBase<NodeT>>(Root, nullptr)).get();
    ...
    void attachNewSubtree(DomTreeT &DT, const TreeNodePtr AttachTo)
    {
        ...
        for (size_t i = 1, e = NumToNode.size(); i != e; ++i)
        {
            ...
            // IDomNode.
            DT.DomTreeNodes[W] = IDomNode->addChild(unknown::make_unique<DomTreeNodeBase<NodeT>>(W, IDomNode));
        }
    }
    ...
    {
        assert((From || IsPostDom) && "From has to be a valid CFG node or a virtual root");
        ...
        if (!FromTN)
        {
            ...
            FromTN = (DT.DomTreeNodes[From] =
                          VirtualRoot->addChild(unknown::make_unique<DomTreeNodeBase<NodeT>>(From, VirtualRoot)))
                         .get();
    ...
    static bool
    UpdateRootsBeforeInsertion(DomTreeT &DT, const BatchUpdatePtr BUI, const TreeNodePtr From, const TreeNodePtr To)
    {
        ...
        auto RIt = unknown::find(DT.Roots, To->getBlock());
        if (RIt == DT.Roots.end())
            return false; // To is not a root, nothing to update.

        LLVM_DEBUG(
            dbgs() << "\t\tAfter the insertion, " << BlockNamePrinter(To)
                   << " is no longer a root\n\t\tRebuilding the tree!!!\n");
    ...
    static void UpdateRootsAfterUpdate(DomTreeT &DT, const BatchUpdatePtr BUI)
    {
        ...
        if (std::none_of(
                DT.Roots.begin(), DT.Roots.end(), [BUI](const NodePtr N) { return HasForwardSuccessors(N, BUI); }))
            return;

        // Recalculate the set of roots.
        auto Roots = FindRoots(DT, BUI);
        if (DT.Roots.size() != Roots.size() || !std::is_permutation(DT.Roots.begin(), DT.Roots.end(), Roots.begin()))
        {
    ...
    {
        LLVM_DEBUG(
            dbgs() << "\tReachable " << BlockNamePrinter(From->getBlock()) << " -> " << BlockNamePrinter(To->getBlock())
                   << "\n");
        ...
        // the NCD manually.
        const NodePtr NCDBlock = (From->getBlock() && To->getBlock())
        ...
        InsertionInfo &II)
    {
        ...
        do
        {
            ...
            for (const NodePtr Succ : ChildrenGetter<IsPostDom>::Get(Next->getBlock(), BUI))
            {
                ...
                if (SuccLevel > RootLevel)
                {
                    ...
                    {
                        LLVM_DEBUG(
                            dbgs() << "\t\t\talready visited at level " << II.Visited[SuccTN]
                ...
                {
                    LLVM_DEBUG(
                        dbgs() << "\t\tMarking affected and adding " << BlockNamePrinter(Succ) << " to a Bucket\n");
    ...
    static void UpdateLevelsAfterInsertion(InsertionInfo &II)
    {
        ...
        {
            LLVM_DEBUG(
                dbgs() << "\tlevel(" << BlockNamePrinter(TN) << ") = (" << BlockNamePrinter(TN->getIDom()) << ") "
    ...
    static void InsertUnreachable(DomTreeT &DT, const BatchUpdatePtr BUI, const TreeNodePtr From, const NodePtr To)
    {
        ...
        LLVM_DEBUG(
            dbgs() << "Inserted " << BlockNamePrinter(From) << " -> (prev unreachable) " << BlockNamePrinter(To)
                   << "\n");
        ...
        {
            LLVM_DEBUG(
                dbgs() << "\tInserting discovered connecting edge " << BlockNamePrinter(Edge.first) << " -> "
        ...
        SmallVectorImpl<std::pair<NodePtr, TreeNodePtr>> &DiscoveredConnectingEdges)
    {
        ...
        auto UnreachableDescender = [&DT, &DiscoveredConnectingEdges](NodePtr From, NodePtr To) {
            const TreeNodePtr ToTN = DT.getNode(To);
            if (!ToTN)
                return true;
    ...
    static void DeleteEdge(DomTreeT &DT, const BatchUpdatePtr BUI, const NodePtr From, const NodePtr To)
    {
        ...
        auto IsSuccessor = [BUI](const NodePtr SuccCandidate, const NodePtr Of) {
            ...
            return unknown::find(Successors, SuccCandidate) != Successors.end();
        };
        (void)IsSuccessor;
        ...
        {
            LLVM_DEBUG(
                dbgs() << "\tTo (" << BlockNamePrinter(To) << ") already unreachable -- there is no edge to delete\n");
            return;
        }
    ...
    static void
    DeleteReachable(DomTreeT &DT, const BatchUpdatePtr BUI, const TreeNodePtr FromTN, const TreeNodePtr ToTN)
    {
        LLVM_DEBUG(
            dbgs() << "Deleting reachable " << BlockNamePrinter(FromTN) << " -> " << BlockNamePrinter(ToTN) << "\n");
        ...
        const NodePtr ToIDom = DT.findNearestCommonDominator(FromTN->getBlock(), ToTN->getBlock());
        ...
        auto DescendBelow = [Level, &DT](NodePtr, NodePtr To) { return DT.getNode(To)->getLevel() > Level; };
    ...
    static bool HasProperSupport(DomTreeT &DT, const BatchUpdatePtr BUI, const TreeNodePtr TN)
    {
        ...
        for (const NodePtr Pred : ChildrenGetter<!IsPostDom>::Get(TN->getBlock(), BUI))
        {
            LLVM_DEBUG(dbgs() << "\tPred " << BlockNamePrinter(Pred) << "\n");
            if (!DT.getNode(Pred))
                continue;

            const NodePtr Support = DT.findNearestCommonDominator(TN->getBlock(), Pred);
            LLVM_DEBUG(dbgs() << "\tSupport " << BlockNamePrinter(Support) << "\n");
            if (Support != TN->getBlock())
            {
                LLVM_DEBUG(
                    dbgs() << "\t" << BlockNamePrinter(TN) << " is reachable from support " << BlockNamePrinter(Support)
                           << "\n");
                return true;
            }
        }
    ...
    static void DeleteUnreachable(DomTreeT &DT, const BatchUpdatePtr BUI, const TreeNodePtr ToTN)
    {
        ...
        auto DescendAndCollect = [Level, &AffectedQueue, &DT](NodePtr, NodePtr To) {
            ...
            if (TN->getLevel() > Level)
                return true;
            if (unknown::find(AffectedQueue, To) == AffectedQueue.end())
        ...
        SemiNCAInfo SNCA(BUI);
        unsigned LastDFSNum = SNCA.runDFS(ToTN->getBlock(), 0, DescendAndCollect, 0);
        ...
        {
            const TreeNodePtr TN = DT.getNode(N);
            const NodePtr NCDBlock = DT.findNearestCommonDominator(TN->getBlock(), ToTN->getBlock());
            assert(NCDBlock || DT.isPostDominator());
            const TreeNodePtr NCD = DT.getNode(NCDBlock);
            assert(NCD);

            LLVM_DEBUG(
                dbgs() << "Processing affected node " << BlockNamePrinter(TN) << " with NCD = " << BlockNamePrinter(NCD)
    ...
    static void EraseNode(DomTreeT &DT, const TreeNodePtr TN)
    {
        ...
        auto ChIt = unknown::find(IDom->Children, TN);
    ...
    static void ApplyNextUpdate(DomTreeT &DT, BatchUpdateInfo &BUI)
    {
        ...
        assert(FS.back().getPointer() == CurrentUpdate.getTo() && FS.back().getInt() == CurrentUpdate.getKind());
        FS.pop_back();
        if (FS.empty())
            BUI.FutureSuccessors.erase(CurrentUpdate.getFrom());

        auto &FP = BUI.FuturePredecessors[CurrentUpdate.getTo()];
        assert(FP.back().getPointer() == CurrentUpdate.getFrom() && FP.back().getInt() == CurrentUpdate.getKind());
        FP.pop_back();
        if (FP.empty())
            BUI.FuturePredecessors.erase(CurrentUpdate.getTo());
    ...
    bool verifyRoots(const DomTreeT &DT)
    {
        ...
        if (DT.Roots.size() != ComputedRoots.size() ||
            !std::is_permutation(DT.Roots.begin(), DT.Roots.end(), ComputedRoots.begin()))
        {
    ...
    bool verifyReachability(const DomTreeT &DT)
    {
        ...
        for (const NodePtr N : NumToNode)
        {
            if (N && !DT.getNode(N))
            {
                errs() << "CFG node " << BlockNamePrinter(N) << " not found in the DomTree!\n";
                errs().flush();
    ...
    static bool VerifyLevels(const DomTreeT &DT)
    {
        for (auto &NodeToTN : DT.DomTreeNodes)
        {
            ...
            {
                errs() << "Node " << BlockNamePrinter(BB) << " has level " << TN->getLevel() << " while its IDom "
                       << BlockNamePrinter(IDom->getBlock()) << " has level " << IDom->getLevel() << "!\n";
                errs().flush();
    ...
    static bool VerifyDFSNumbers(const DomTreeT &DT)
    {
        ...
        auto PrintNodeAndDFSNums = [](const TreeNodePtr TN) {
            errs() << BlockNamePrinter(TN) << " {" << TN->getDFSNumIn() << ", " << TN->getDFSNumOut() << '}';
        };
        ...
        for (const auto &NodeToTN : DT.DomTreeNodes)
        {
            ...
            unknown::sort(Children, [](const TreeNodePtr Ch1, const TreeNodePtr Ch2) {
                return Ch1->getDFSNumIn() < Ch2->getDFSNumIn();
            });

            auto PrintChildrenError =
                [Node, &Children, PrintNodeAndDFSNums](const TreeNodePtr FirstCh, const TreeNodePtr SecondCh) {
                    assert(FirstCh);
    ...
    bool verifyParentProperty(const DomTreeT &DT)
    {
        for (auto &NodeToTN : DT.DomTreeNodes)
        {
            ...
            clear();
            doFullDFSWalk(DT, [BB](NodePtr From, NodePtr To) { return From != BB && To != BB; });

            for (TreeNodePtr Child : TN->getChildren())
                if (NodeToInfo.count(Child->getBlock()) != 0)
                {
                    errs() << "Child " << BlockNamePrinter(Child) << " reachable after its parent "
                           << BlockNamePrinter(BB) << " is removed!\n";
                    errs().flush();
    ...
    bool verifySiblingProperty(const DomTreeT &DT)
    {
        for (auto &NodeToTN : DT.DomTreeNodes)
        {
            ...
            for (const TreeNodePtr N : Siblings)
            {
                ...
                doFullDFSWalk(DT, [BBN](NodePtr From, NodePtr To) { return From != BBN && To != BBN; });
                ...
                for (const TreeNodePtr S : Siblings)
                {
                    ...
                    {
                        errs() << "Node " << BlockNamePrinter(S) << " not reachable when its sibling "
                               << BlockNamePrinter(N) << " is removed!\n";
                        errs().flush();
...
template <typename DomTreeT>
void
CalculateWithUpdates(DomTreeT &DT, ArrayRef<typename DomTreeT::UpdateType> Updates)
{
...
template <class DomTreeT>
void
InsertEdge(DomTreeT &DT, typename DomTreeT::NodePtr From, typename DomTreeT::NodePtr To)
{
...
template <class DomTreeT>
void
DeleteEdge(DomTreeT &DT, typename DomTreeT::NodePtr From, typename DomTreeT::NodePtr To)
{
    if (DT.isPostDominator())
        std::swap(From, To);
...
template <class DomTreeT>
void
ApplyUpdates(DomTreeT &DT, ArrayRef<typename DomTreeT::UpdateType> Updates)
{
...
template <class DomTreeT>
bool
Verify(const DomTreeT &DT, typename DomTreeT::VerificationLevel VL)
{
    ...
    if (!SNCA.verifyRoots(DT) || !SNCA.verifyReachability(DT) || !SNCA.VerifyLevels(DT) || !SNCA.VerifyDFSNumbers(DT))
        return false;

    // Extra checks depending on VerificationLevel. Up to O(N^3)
    if (VL == DomTreeT::VerificationLevel::Basic || VL == DomTreeT::VerificationLevel::Full)
