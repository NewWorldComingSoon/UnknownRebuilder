...
std::string
escape(StringRef Input, bool EscapePrintable = true);
...
public:
    ...
    void operator delete(void *Ptr, BumpPtrAllocator &Alloc, size_t Size) noexcept { Alloc.Deallocate(Ptr, Size); }
...
private:
    ...
    StringRef
    unescapeDoubleQuoted(StringRef UnquotedValue, StringRef::size_type Start, SmallVectorImpl<char> &Storage) const;
};
...
public:
    ...
    bool operator==(const basic_collection_iterator &Other) const
    {
        ...
        {
            assert(
                (Base->CurrentEntry == Other.Base->CurrentEntry) && "Equal Bases expected to point to equal Entries");
        }
...
public:
    ...
    template <class T>
    friend typename T::iterator yaml::begin(T &);
    template <class T>
...
public:
    AliasNode(std::unique_ptr<Document> &D, StringRef Val) : Node(NK_Alias, D, StringRef(), StringRef()), Name(Val) {}
