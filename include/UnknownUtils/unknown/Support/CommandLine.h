...
//
void
ParseEnvironmentOptions(const char *progName, const char *envvar, const char *Overview = "");
...
void
SetVersionPrinter(VersionPrinterTy func);
...
void
AddExtraVersionPrinter(VersionPrinterTy func);
...
void
AddLiteralOption(Option &O, StringRef Name);
...
public:
    OptionCategory(StringRef const Name, StringRef const Description = "") : Name(Name), Description(Description)
    {
        registerCategory();
    }
...
public:
    SubCommand(StringRef Name, StringRef Description = "") : Name(Name), Description(Description)
    {
        registerSubCommand();
    }
    SubCommand() = default;
...
class Option
{
    ...
    virtual enum ValueExpected getValueExpectedFlagDefault() const { return ValueOptional; }
...
public:
    ...
    inline enum NumOccurrencesFlag getNumOccurrencesFlag() const { return (enum NumOccurrencesFlag)Occurrences; }
    ...
    inline enum OptionHidden getOptionHiddenFlag() const { return (enum OptionHidden)HiddenFlag; }

    inline enum FormattingFlags getFormattingFlag() const { return (enum FormattingFlags)Formatting; }
    ...
    bool isConsumeAfter() const { return getNumOccurrencesFlag() == cl::ConsumeAfter; }

    bool isInAllSubCommands() const
    {
        return any_of(Subs, [](const SubCommand *SC) { return SC == &*AllSubCommands; });
    }
...
protected:
    explicit Option(enum NumOccurrencesFlag OccurrencesFlag, enum OptionHidden Hidden) :
...
public:
    ...
    bool error(const Twine &Message, StringRef ArgName = StringRef(), raw_ostream &Errs = unknown::errs());
    bool error(const Twine &Message, raw_ostream &Errs) { return error(Message, StringRef(), Errs); }
...
struct OptionValueBase : public GenericOptionValue
{
    ...
    bool compare(const GenericOptionValue & /*V*/) const override { return false; }
...
template <>
struct OptionValue<cl::boolOrDefault> final : OptionValueCopy<cl::boolOrDefault>
{
...
#define clEnumVal(ENUMVAL, DESC) \
    unknown::cl::OptionEnumValue { #ENUMVAL, int(ENUMVAL), DESC }
#define clEnumValN(ENUMVAL, FLAGNAME, DESC) \
    unknown::cl::OptionEnumValue { FLAGNAME, int(ENUMVAL), DESC }
...
public:
    ValuesClass(std::initializer_list<OptionEnumValue> Options) : Values(Options) {}
...
protected:
    ...
    {
    public:
        GenericOptionInfo(StringRef name, StringRef helpStr) : Name(name), HelpStr(helpStr) {}
        StringRef Name;
        StringRef HelpStr;
    };
...
public:
    ...
    StringRef getDescription(unsigned N) const override { return Values[N].HelpStr; }

    // getOptionValue - Return the value of option name N.
    const GenericOptionValue &getOptionValue(unsigned N) const override { return Values[N].V; }
...
public:
    ...
    enum ValueExpected getValueExpectedFlagDefault() const { return ValueRequired; }
...
public:
    ...
    enum ValueExpected getValueExpectedFlagDefault() const { return ValueOptional; }
...
//
template <>
class parser<unsigned> final : public basic_parser<unsigned>
{
public:
    parser(Option &O) : basic_parser(O) {}
...
{
    static void opt(NumOccurrencesFlag N, Option &O) { O.setNumOccurrencesFlag(N); }
};
...
void
apply(Opt *O, const Mod &M, const Mods &...Ms)
{
...
class opt_storage
{
    ...
    {
        assert(
            Location && "cl::location(...) not specified for a command "
...
class opt : public Option, public opt_storage<DataType, ExternalStorage, std::is_class<DataType>::value>
{
    ParserClass Parser;

    bool handleOccurrence(unsigned pos, StringRef ArgName, StringRef Arg) override
    {
        typename ParserClass::parser_data_type Val = typename ParserClass::parser_data_type();
    ...
    enum ValueExpected getValueExpectedFlagDefault() const override { return Parser.getValueExpectedFlagDefault(); }
    ...
    void printOptionInfo(size_t GlobalWidth) const override { Parser.printOptionInfo(*this, GlobalWidth); }

    void printOptionValue(size_t GlobalWidth, bool Force) const override
    {
        if (Force || this->getDefault().compare(this->getValue()))
        {
            cl::printOptionDiff<ParserClass>(*this, Parser, this->getValue(), this->getDefault(), GlobalWidth);
        }
    }
...
public:
    ...
    {
        assert(
            Location != 0 && "cl::location(...) not specified for a command "
                             "line option with external storage!");
...
public:
    ...
    iterator erase(iterator first, iterator last) { return Storage.erase(first, last); }

    iterator insert(const_iterator pos, const DataType &value) { return Storage.insert(pos, value); }
    iterator insert(const_iterator pos, DataType &&value) { return Storage.insert(pos, value); }

    iterator insert(iterator pos, const DataType &value) { return Storage.insert(pos, value); }
    iterator insert(iterator pos, DataType &&value) { return Storage.insert(pos, value); }
...
class bits_storage
{
    ...
    static unsigned Bit(const T &V)
    {
        ...
        assert(BitPos < sizeof(unsigned) * CHAR_BIT && "enum exceeds width of bit vector!");
        return 1 << BitPos;
    }
...
public:
    ...
    template <class... Mods>
    explicit bits(const Mods &...Ms) : Option(ZeroOrMore, NotHidden), Parser(*this)
    {
        apply(this, Ms...);
        done();
    }
...
class alias : public Option
{
    ...
    bool addOccurrence(unsigned pos, StringRef /*ArgName*/, StringRef Value, bool MultiArg = false) override
    {
    ...
    ValueExpected getValueExpectedFlagDefault() const override { return AliasFor->getValueExpectedFlag(); }
...
public:
    ...
    template <class... Mods>
    explicit alias(const Mods &...Ms) : Option(Optional, Hidden), AliasFor(nullptr)
    {
        apply(this, Ms...);
        done();
    }
...
void
PrintHelpMessage(bool Hidden = false, bool Categorized = false);
...
/// StringMap<unknown::cl::Option*> &opts = unknown::cl::getRegisteredOptions();
/// assert(opts.count("help") == 1)
/// opts["help"]->setDescription("Show alphabetical help information")
/// // More code
/// unknown::cl::ParseCommandLineOptions(argc,argv);
/// //More code
/// }
/// \endcode
///
...
/// unknown::cl::ParseCommandLineOptions().
///
...
/// main(int argc, char* argv[]) {
///   unknown::cl::ParseCommandLineOptions(argc, argv);
///   for (auto* S : unknown::cl::getRegisteredSubcommands()) {
///     if (*S) {
...
using TokenizerCallback =
    void (*)(StringRef Source, StringSaver &Saver, SmallVectorImpl<const char *> &NewArgv, bool MarkEOLs);
...
///
void
tokenizeConfigFile(StringRef Source, StringSaver &Saver, SmallVectorImpl<const char *> &NewArgv, bool MarkEOLs = false);
...
///
bool
readConfigFile(StringRef CfgFileName, StringSaver &Saver, SmallVectorImpl<const char *> &Argv);
...
void
HideUnrelatedOptions(cl::OptionCategory &Category, SubCommand &Sub = *TopLevelSubCommand);
...
/// option category to display in the -help output.
void
HideUnrelatedOptions(ArrayRef<const cl::OptionCategory *> Categories, SubCommand &Sub = *TopLevelSubCommand);
