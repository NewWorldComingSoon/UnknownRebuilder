...
#define ENCODINGS \
    ...
    ENUM_ENTRY( \
        ENCODING_RB, \
        "(AL..DIL, R8L..R15L) Register code added to " \
        "the opcode byte") \
    ...
    ENUM_ENTRY( \
        ENCODING_FP, \
        "Position on floating-point stack in ModR/M " \
        "byte.") \
\
    ...
    ENUM_ENTRY( \
        ENCODING_Rv, \
        "Register code of operand size added to the " \
        "opcode byte") \
    ENUM_ENTRY( \
        ENCODING_DUP, \
