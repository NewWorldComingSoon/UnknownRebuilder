...
public:
    ...
    uint64_t getAddress(uint32_t *offset_ptr) const { return getUnsigned(offset_ptr, AddressSize); }
    ...
    ///     otherwise.
    bool isValidOffsetForAddress(uint32_t offset) const { return isValidOffsetForDataOfSize(offset, AddressSize); }
};
