...
#include "unknown/Support/BinaryStream.h"
#include "unknown/Support/BinaryStreamError.h"
#include "unknown/Support/Error.h"
#include "unknown/Support/FileOutputBuffer.h"
...
public:
    ...
    BinaryByteStream(ArrayRef<uint8_t> Data, unknown::support::endianness Endian) : Endian(Endian), Data(Data) {}
    BinaryByteStream(StringRef Data, unknown::support::endianness Endian) :
        Endian(Endian), Data(Data.bytes_begin(), Data.bytes_end())
    {
    }

    unknown::support::endianness getEndian() const override { return Endian; }
...
{
public:
    MemoryBufferByteStream(std::unique_ptr<MemoryBuffer> Buffer, unknown::support::endianness Endian) :
        BinaryByteStream(Buffer->getBuffer(), Endian), MemBuffer(std::move(Buffer))
    {
    }
...
public:
    ...
    MutableBinaryByteStream(MutableArrayRef<uint8_t> Data, unknown::support::endianness Endian) :
        Data(Data), ImmutableStream(Data, Endian)
    {
    }

    unknown::support::endianness getEndian() const override { return ImmutableStream.getEndian(); }
...
class AppendingBinaryByteStream : public WritableBinaryStream
{
    ...
    unknown::support::endianness Endian = unknown::support::little;

public:
    AppendingBinaryByteStream() = default;
    AppendingBinaryByteStream(unknown::support::endianness Endian) : Endian(Endian) {}
    ...
    virtual BinaryStreamFlags getFlags() const override { return BSF_Write | BSF_Append; }
...
private:
    ...
    {
    public:
        StreamImpl(std::unique_ptr<FileOutputBuffer> Buffer, unknown::support::endianness Endian) :
            MutableBinaryByteStream(MutableArrayRef<uint8_t>(Buffer->getBufferStart(), Buffer->getBufferEnd()), Endian),
        ...
        Error commit() override
        {
            if (FileBuffer->commit())
                return make_error<BinaryStreamError>(stream_error_code::filesystem_error);
            return Error::success();
        }
...
public:
    FileBufferByteStream(std::unique_ptr<FileOutputBuffer> Buffer, unknown::support::endianness Endian) :
        Impl(std::move(Buffer), Endian)
    {
    }

    unknown::support::endianness getEndian() const override { return Impl.getEndian(); }
    ...
    Error writeBytes(uint32_t Offset, ArrayRef<uint8_t> Data) override { return Impl.writeBytes(Offset, Data); }
