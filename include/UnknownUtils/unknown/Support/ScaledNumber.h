...
getRounded(DigitsT Digits, int16_t Scale, bool ShouldRound)
{
...
getRounded32(uint32_t Digits, int16_t Scale, bool ShouldRound)
{
...
getRounded64(uint64_t Digits, int16_t Scale, bool ShouldRound)
{
    return getRounded(Digits, Scale, ShouldRound);
}
...
template <class DigitsT>
inline std::pair<DigitsT, int16_t>
getAdjusted(uint64_t Digits, int16_t Scale = 0)
{
    static_assert(!std::numeric_limits<DigitsT>::is_signed, "expected unsigned");
    ...
    return getRounded<DigitsT>(Digits >> Shift, Scale + Shift, Digits & (UINT64_C(1) << (Shift - 1)));
}

/// Convenience helper for adjusting to 32 bits.
inline std::pair<uint32_t, int16_t>
getAdjusted32(uint64_t Digits, int16_t Scale = 0)
{
...
inline std::pair<uint64_t, int16_t>
getAdjusted64(uint64_t Digits, int16_t Scale = 0)
{
...
template <class DigitsT>
std::pair<DigitsT, int16_t>
getQuotient(DigitsT Dividend, DigitsT Divisor)
{
    static_assert(!std::numeric_limits<DigitsT>::is_signed, "expected unsigned");
    static_assert(sizeof(DigitsT) == 4 || sizeof(DigitsT) == 8, "expected 32-bit or 64-bit digits");
...
inline std::pair<uint32_t, int16_t>
getQuotient32(uint32_t Dividend, uint32_t Divisor)
{
...
inline std::pair<uint64_t, int16_t>
getQuotient64(uint64_t Dividend, uint64_t Divisor)
{
    return getQuotient(Dividend, Divisor);
}
...
template <class DigitsT>
int
compare(DigitsT LDigits, int16_t LScale, DigitsT RDigits, int16_t RScale)
{
    static_assert(!std::numeric_limits<DigitsT>::is_signed, "expected unsigned");
...
template <class DigitsT>
int16_t
matchScales(DigitsT &LDigits, int16_t &LScale, DigitsT &RDigits, int16_t &RScale)
{
    static_assert(!std::numeric_limits<DigitsT>::is_signed, "expected unsigned");
...
template <class DigitsT>
std::pair<DigitsT, int16_t>
getSum(DigitsT LDigits, int16_t LScale, DigitsT RDigits, int16_t RScale)
{
    static_assert(!std::numeric_limits<DigitsT>::is_signed, "expected unsigned");
...
inline std::pair<uint32_t, int16_t>
getSum32(uint32_t LDigits, int16_t LScale, uint32_t RDigits, int16_t RScale)
{
...
inline std::pair<uint64_t, int16_t>
getSum64(uint64_t LDigits, int16_t LScale, uint64_t RDigits, int16_t RScale)
{
    return getSum(LDigits, LScale, RDigits, RScale);
}
...
template <class DigitsT>
std::pair<DigitsT, int16_t>
getDifference(DigitsT LDigits, int16_t LScale, DigitsT RDigits, int16_t RScale)
{
    static_assert(!std::numeric_limits<DigitsT>::is_signed, "expected unsigned");
...
inline std::pair<uint32_t, int16_t>
getDifference32(uint32_t LDigits, int16_t LScale, uint32_t RDigits, int16_t RScale)
{
...
inline std::pair<uint64_t, int16_t>
getDifference64(uint64_t LDigits, int16_t LScale, uint64_t RDigits, int16_t RScale)
{
    return getDifference(LDigits, LScale, RDigits, RScale);
}
...
template <class DigitsT>
class ScaledNumber : ScaledNumberBase
{
public:
    static_assert(!std::numeric_limits<DigitsT>::is_signed, "only unsigned floats supported");
...
public:
    ...
    constexpr ScaledNumber(DigitsType Digits, int16_t Scale) : Digits(Digits), Scale(Scale) {}

private:
    ScaledNumber(const std::pair<DigitsT, int16_t> &X) : Digits(X.first), Scale(X.second) {}

public:
    static ScaledNumber getZero() { return ScaledNumber(0, 0); }
    static ScaledNumber getOne() { return ScaledNumber(1, 0); }
    static ScaledNumber getLargest() { return ScaledNumber(DigitsLimits::max(), ScaledNumbers::MaxScale); }
    static ScaledNumber get(uint64_t N) { return adjustToWidth(N, 0); }
    static ScaledNumber getInverse(uint64_t N) { return get(N).invert(); }
    static ScaledNumber getFraction(DigitsType N, DigitsType D) { return getQuotient(N, D); }
    ...
    int32_t lgCeiling() const { return ScaledNumbers::getLgCeiling(Digits, Scale); }
    ...
    {
        std::tie(Digits, Scale) = ScaledNumbers::getSum(Digits, Scale, X.Digits, X.Scale);
    ...
    ScaledNumber &operator-=(const ScaledNumber &X)
    {
        std::tie(Digits, Scale) = ScaledNumbers::getDifference(Digits, Scale, X.Digits, X.Scale);
        return *this;
    }
    ScaledNumber &operator*=(const ScaledNumber &X);
    ScaledNumber &operator/=(const ScaledNumber &X);
...
public:
    ...
    int compare(const ScaledNumber &X) const { return ScaledNumbers::compare(Digits, Scale, X.Digits, X.Scale); }
    int compareTo(uint64_t N) const { return ScaledNumbers::compare<uint64_t>(Digits, Scale, N, 0); }
...
private:
    static ScaledNumber getProduct(DigitsType LHS, DigitsType RHS) { return ScaledNumbers::getProduct(LHS, RHS); }
    ...
    static ScaledNumber adjustToWidth(uint64_t N, int32_t Shift)
    {
        ...
        assert(Shift <= ScaledNumbers::MaxScale - 64 && "Shift should be close to 0");
...
#define SCALED_NUMBER_BOP(op, base) \
    template <class DigitsT> \
    ScaledNumber<DigitsT> operator op(const ScaledNumber<DigitsT> &L, const ScaledNumber<DigitsT> &R) \
    { \
...
operator<<(const ScaledNumber<DigitsT> &L, int16_t Shift)
{
...
template <class DigitsT>
raw_ostream &
operator<<(raw_ostream &OS, const ScaledNumber<DigitsT> &X)
{
...
template <class DigitsT>
uint64_t
ScaledNumber<DigitsT>::scale(uint64_t N) const
{
...
template <class IntT>
IntT
ScaledNumber<DigitsT>::toInt() const
{
...
template <class DigitsT>
void
ScaledNumber<DigitsT>::shiftLeft(int32_t Shift)
{
    if (!Shift || isZero())
        return;
...
template <class DigitsT>
void
ScaledNumber<DigitsT>::shiftRight(int32_t Shift)
{
    if (!Shift || isZero())
        return;
    assert(Shift != INT32_MIN);
