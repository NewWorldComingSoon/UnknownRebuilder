...
private:
    ...
    KnownBits(APInt Zero, APInt One) : Zero(std::move(Zero)), One(std::move(One)) {}
...
public:
    ...
    {
        assert(Zero.getBitWidth() == One.getBitWidth() && "Zero and One should have the same width!");
    ...
    KnownBits trunc(unsigned BitWidth) { return KnownBits(Zero.trunc(BitWidth), One.trunc(BitWidth)); }

    /// Zero extends the underlying known Zero and One bits. This is equivalent
    /// to zero extending the value we're tracking.
    KnownBits zext(unsigned BitWidth) { return KnownBits(Zero.zext(BitWidth), One.zext(BitWidth)); }

    /// Sign extends the underlying known Zero and One bits. This is equivalent
    /// to sign extending the value we're tracking.
    KnownBits sext(unsigned BitWidth) { return KnownBits(Zero.sext(BitWidth), One.sext(BitWidth)); }
    ...
    unsigned countMaxPopulation() const { return getBitWidth() - Zero.countPopulation(); }
