...
#include "unknown/Support/CFGUpdate.h"
...
public:
    ...
    void setIDom(DomTreeNodeBase *NewIDom)
    {
        ...
        assert(I != IDom->Children.end() && "Not in immediate dominator children set!");
...
template <class NodeT>
raw_ostream &
operator<<(raw_ostream &O, const DomTreeNodeBase<NodeT> *Node)
{
    if (Node->getBlock())
    ...
    O << " {" << Node->getDFSNumIn() << "," << Node->getDFSNumOut() << "} [" << Node->getLevel() << "]\n";
...
template <class NodeT>
void
PrintDomTree(const DomTreeNodeBase<NodeT> *N, raw_ostream &O, unsigned Lev)
{
    O.indent(2 * Lev) << "[" << Lev << "] " << N;
    for (typename DomTreeNodeBase<NodeT>::const_iterator I = N->begin(), E = N->end(); I != E; ++I)
...
public:
    static_assert(
        std::is_pointer<typename GraphTraits<NodeT *>::NodeRef>::value,
        "Currently DominatorTreeBase supports only pointer nodes");
    using NodeType = NodeT;
    using NodePtr = NodeT *;
    using ParentPtr = decltype(std::declval<NodeT *>()->getParent());
    static_assert(std::is_pointer<ParentPtr>::value, "Currently NodeT's parent must be a pointer type");
...
public:
    ...
    /// See getNode.
    DomTreeNodeBase<NodeT> *operator[](const NodeT *BB) const { return getNode(BB); }
    ...
    {
        assert(!this->isPostDominator() && "This is not implemented for post dominators");
    ...
    bool isVirtualRoot(const DomTreeNodeBase<NodeT> *A) const { return isPostDominator() && !A->getBlock(); }
    ...
    ///
    void applyUpdates(ArrayRef<UpdateType> Updates) { DomTreeBuilder::ApplyUpdates(*this, Updates); }
    ...
    DomTreeNodeBase<NodeT> *addNewBlock(NodeT *BB, NodeT *DomBB)
    {
        ...
        DFSInfoValid = false;
        return (DomTreeNodes[BB] = IDomNode->addChild(unknown::make_unique<DomTreeNodeBase<NodeT>>(BB, IDomNode)))
            .get();
    }
    ...
    {
        assert(getNode(BB) == nullptr && "Block already in dominator tree!");
        assert(!this->isPostDominator() && "Cannot change root of post-dominator tree");
        DFSInfoValid = false;
        DomTreeNodeBase<NodeT> *NewNode =
            (DomTreeNodes[BB] = unknown::make_unique<DomTreeNodeBase<NodeT>>(BB, nullptr)).get();
        if (Roots.empty())
        {
            addRoot(BB);
        }
    ...
    void changeImmediateDominator(NodeT *BB, NodeT *NewBB) { changeImmediateDominator(getNode(BB), getNode(NewBB)); }
    ...
    void eraseNode(NodeT *BB)
    {
        ...
        auto RIt = unknown::find(Roots, BB);
        if (RIt != Roots.end())
        {
...
public:
    ...
    void updateDFSNumbers() const
    {
        ...
        SmallVector<std::pair<const DomTreeNodeBase<NodeT> *, typename DomTreeNodeBase<NodeT>::const_iterator>, 32>
            WorkStack;
    ...
    bool verify(VerificationLevel VL = VerificationLevel::Full) const { return DomTreeBuilder::Verify(*this, VL); }
...
protected:
    ...
    void Split(typename GraphTraits<N>::NodeRef NewBB)
    {
        ...
        assert(
            std::distance(GraphT::child_begin(NewBB), GraphT::child_end(NewBB)) == 1 &&
        ...
        {
            if (Pred != NewBB && !dominates(NewBBSucc, Pred) && isReachableFromEntry(Pred))
            {
...
template <typename NodeT, bool IsPostDom>
bool
DominatorTreeBase<NodeT, IsPostDom>::dominates(const NodeT *A, const NodeT *B) const
{
    if (A == B)
        return true;
...
template <typename NodeT, bool IsPostDom>
bool
DominatorTreeBase<NodeT, IsPostDom>::properlyDominates(const NodeT *A, const NodeT *B) const
{
    if (A == B)
        return false;
