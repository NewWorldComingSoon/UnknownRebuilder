...
struct Metadata final
{
    ...
    bool notEmpty() const
    {
        return !mReqdWorkGroupSize.empty() || !mWorkGroupSizeHint.empty() || !mVecTypeHint.empty() ||
               !mRuntimeHandle.empty();
    }
};
...
constexpr char WavefrontPrivateSegmentOffsetSGPR[] = "WavefrontPrivateSegmentOffsetSGPR";
} // end namespace Key
...
std::error_code
fromString(std::string String, Metadata &HSAMetadata);

/// Converts \p HSAMetadata to \p String.
std::error_code
toString(Metadata HSAMetadata, std::string &String);
...
std::error_code
toString(const Metadata &PALMetadata, std::string &String);
