...
#include "unknown/ADT/UniqueVector.h"
...
public:
    ...
    static unsigned registerCounter(StringRef Name, StringRef Desc) { return instance().addCounter(Name, Desc); }
    ...
    static bool isCounterSet(unsigned ID) { return instance().Counters[ID].IsSet; }
    ...
    // Dump or print the current counter set into unknown::dbgs().
    LLVM_DUMP_METHOD void dump() const;

    void print(raw_ostream &OS) const;

    // Get the counter ID for a given named counter, or return 0 if none is found.
    unsigned getCounterId(const std::string &Name) const { return RegisteredCounters.idFor(Name); }
    ...
    CounterVector::const_iterator begin() const { return RegisteredCounters.begin(); }
