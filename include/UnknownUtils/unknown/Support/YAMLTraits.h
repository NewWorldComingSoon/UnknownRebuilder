...
///      static void output(const MyType &val, void*, unknown::raw_ostream &out) {
///        // stream out custom formatting
///        out << unknown::format("%x", val);
///      }
...
struct ScalarTraits
{
    ...
    // static void output(const T &value, void *ctxt, unknown::raw_ostream &out);
    //
...
///      static void output(const MyType &Value, void*, unknown::raw_ostream &Out)
///      {
///        // stream out custom formatting
///        Out << Value;
///      }
...
struct BlockScalarTraits
{
    ...
    //
    // Function to write the value as a string:
    // static void output(const T &Value, void *ctx, unknown::raw_ostream &Out);
    //
    // Function to convert a string to a value.  Returns the empty
    // StringRef on success or an error string if string is malformed:
    // static StringRef input(StringRef Scalar, void *ctxt, T &Value);
...
///      static void output(const MyType &Value, void*, unknown::raw_ostream
///      &ScalarOut, unknown::raw_ostream &TagOut)
///      {
...
struct TaggedScalarTraits
{
    ...
    // static void output(const T &Value, void *ctx, unknown::raw_ostream &ScalarOut,
    // unknown::raw_ostream &TagOut);
    //
    // Function to convert a string to a value.  Returns the empty
    // StringRef on success or an error string if string is malformed:
...
struct has_ScalarEnumerationTraits
{
    ...
    static bool const value = (sizeof(test<ScalarEnumerationTraits<T>>(nullptr)) == 1);
};
...
struct has_ScalarTraits
{
    ...
    template <typename U>
    static char test(
        SameType<Signature_input, &U::input> *,
    ...
    static bool const value = (sizeof(test<ScalarTraits<T>>(nullptr, nullptr, nullptr)) == 1);
};
...
struct has_BlockScalarTraits
{
    ...
    static bool const value = (sizeof(test<BlockScalarTraits<T>>(nullptr, nullptr)) == 1);
};
...
struct has_TaggedScalarTraits
{
    ...
    using Signature_output = void (*)(const T &, void *, raw_ostream &, raw_ostream &);
    ...
    static bool const value = (sizeof(test<TaggedScalarTraits<T>>(nullptr, nullptr, nullptr)) == 1);
};
...
struct has_MappingTraits
{
    ...
    static bool const value = (sizeof(test<MappingContextTraits<T, Context>>(nullptr)) == 1);
};
...
struct has_CustomMappingTraits
{
    ...
    static bool const value = (sizeof(test<CustomMappingTraits<T>>(nullptr)) == 1);
};
...
// Test if SequenceTraits<T> is defined on type T
template <typename T>
struct has_SequenceTraits : public std::integral_constant<bool, has_SequenceMethodTraits<T>::value>
{
};
...
inline bool
isNumeric(StringRef S)
{
    const static auto skipDigits = [](StringRef Input) {
        return Input.drop_front(std::min(Input.find_first_not_of("0123456789"), Input.size()));
    };
    ...
    if (S.startswith("0o"))
        return S.size() > 2 && S.drop_front(2).find_first_not_of("01234567") == StringRef::npos;

    if (S.startswith("0x"))
        return S.size() > 2 && S.drop_front(2).find_first_not_of("0123456789abcdefABCDEF") == StringRef::npos;
    ...
    // doesn't have one.
    if (S.startswith(".") && (S.equals(".") || (S.size() > 1 && std::strchr("0123456789", S[1]) == nullptr)))
        return false;
...
isNull(StringRef S)
{
    return S.equals("null") || S.equals("Null") || S.equals("NULL") || S.equals("~");
}

inline bool
isBool(StringRef S)
{
    return S.equals("true") || S.equals("True") || S.equals("TRUE") || S.equals("false") || S.equals("False") ||
           S.equals("FALSE");
}
...
struct missingTraits : public std::integral_constant<
                           ...
                           !has_ScalarEnumerationTraits<T>::value && !has_ScalarBitSetTraits<T>::value &&
                               ...
                               !has_TaggedScalarTraits<T>::value && !has_MappingTraits<T, Context>::value &&
                               !has_SequenceTraits<T>::value && !has_CustomMappingTraits<T>::value &&
                               !has_DocumentListTraits<T>::value && !has_PolymorphicTraits<T>::value>
{
};
...
struct validatedMappingTraits
    : public std::
          integral_constant<bool, has_MappingTraits<T, Context>::value && has_MappingValidateTraits<T, Context>::value>
{
};
...
public:
    ...
    {
        this->processKeyWithDefault(Key, Val, Optional<T>(), /*Required=*/false, Ctx);
    }
...
private:
    ...
    void processKeyWithDefault(
        ...
        Context &Ctx)
    {
        assert(DefaultValue.hasValue() == false && "Optional<T> shouldn't have a value!");
        void *SaveInfo;
        bool UseDefault = true;
        const bool sameAsDefault = outputting() && !Val.hasValue();
        if (!outputting() && !Val.hasValue())
            Val = T();
        if (Val.hasValue() && this->preflightKey(Key, Required, sameAsDefault, UseDefault, SaveInfo))
        {
    ...
    template <typename T, typename Context>
    void processKeyWithDefault(const char *Key, T &Val, const T &DefaultValue, bool Required, Context &Ctx)
    {
        void *SaveInfo;
        bool UseDefault;
        const bool sameAsDefault = outputting() && Val == DefaultValue;
        if (this->preflightKey(Key, Required, sameAsDefault, UseDefault, SaveInfo))
        {
...
template <typename T, typename Context>
void
doMapping(IO &io, T &Val, Context &Ctx)
{
...
template <typename T>
void
doMapping(IO &io, T &Val, EmptyContext &Ctx)
{
...
yamlize(IO &io, T &Val, bool, EmptyContext &Ctx)
{
    if (io.outputting())
    {
        ...
        TaggedScalarTraits<T>::output(Val, io.getContext(), ScalarBuffer, TagBuffer);
        io.scalarTag(TagBuffer.str());
        StringRef ScalarStr = ScalarBuffer.str();
        io.scalarString(ScalarStr, TaggedScalarTraits<T>::mustQuote(Val, ScalarStr));
    }
    else
    {
        std::string Tag;
        ...
        StringRef Result = TaggedScalarTraits<T>::input(Str, Tag, io.getContext(), Val);
        if (!Result.empty())
        {
            io.setError(Twine(Result));
        }
...
yamlize(IO &io, T &Val, bool, EmptyContext &Ctx)
{
    switch (io.outputting() ? PolymorphicTraits<T>::getKind(Val) : io.getNodeKind())
    {
...
struct ScalarTraits<support::detail::packed_endian_specific_integral<value_type, endian, alignment>>
{
    ...
    static QuotingType mustQuote(StringRef Str) { return ScalarTraits<value_type>::mustQuote(Str); }
};
...
{
    MappingNormalization(IO &i_o, TFinal &Obj) : io(i_o), BufPtr(nullptr), Result(Obj)
    {
        if (io.outputting())
        {
...
{
    MappingNormalizationHeap(IO &i_o, TFinal &Obj, BumpPtrAllocator *allocator) : io(i_o), Result(Obj)
    {
        if (io.outputting())
        {
            BufPtr = new (&Buffer) TNorm(io, Obj);
...
private:
    ...
    public:
        ...
        static bool classof(const HNode *n) { return MappingNode::classof(n->_node); }
    ...
    public:
        ...
        static bool classof(const HNode *n) { return SequenceNode::classof(n->_node); }
...
private:
    ...
    std::unique_ptr<unknown::yaml::Stream> Strm;
...
template <typename T>
inline typename std::enable_if<has_DocumentListTraits<T>::value, Input &>::type
...
template <typename T>
inline typename std::enable_if<has_MappingTraits<T, EmptyContext>::value, Input &>::type
...
// a document.
template <typename T>
inline typename std::enable_if<has_SequenceTraits<T>::value, Input &>::type
...
template <typename T>
inline typename std::enable_if<has_BlockScalarTraits<T>::value, Input &>::type
...
template <typename T>
inline typename std::enable_if<has_CustomMappingTraits<T>::value, Input &>::type
operator>>(Input &In, T &Val)
{
    EmptyContext Ctx;
    if (In.setCurrentDocument())
...
template <typename T>
inline typename std::enable_if<missingTraits<T, EmptyContext>::value, Input &>::type
operator>>(Input &yin, T &docSeq)
{
    char missing_yaml_trait_for_type[sizeof(MissingTrait<T>)];
    return yin;
...
template <typename T>
inline typename std::enable_if<has_DocumentListTraits<T>::value, Output &>::type
operator<<(Output &yout, T &docList)
{
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        {
            yamlize(yout, DocumentListTraits<T>::element(yout, docList, i), true, Ctx);
...
template <typename T>
inline typename std::enable_if<has_MappingTraits<T, EmptyContext>::value, Output &>::type
...
template <typename T>
inline typename std::enable_if<has_SequenceTraits<T>::value, Output &>::type
...
template <typename T>
inline typename std::enable_if<has_BlockScalarTraits<T>::value, Output &>::type
...
template <typename T>
inline typename std::enable_if<has_CustomMappingTraits<T>::value, Output &>::type
operator<<(Output &Out, T &Val)
{
    EmptyContext Ctx;
    Out.beginDocuments();
...
// Provide better error message about types missing a trait specialization
template <typename T>
inline typename std::enable_if<missingTraits<T, EmptyContext>::value, Output &>::type
operator<<(Output &yout, T &seq)
{
    char missing_yaml_trait_for_type[sizeof(MissingTrait<T>)];
    return yout;
...
struct SequenceTraits<
    SmallVector<T, N>,
    typename std::enable_if<CheckIsBool<SequenceElementTraits<T>::flow>::value>::type>
...
struct StdMapStringCustomMappingTraitsImpl
{
    ...
    static void inputOne(IO &io, StringRef key, map_type &v) { io.mapRequired(key.str().c_str(), v[key]); }
...
#define LLVM_YAML_IS_SEQUENCE_VECTOR_IMPL(TYPE, FLOW) \
    ...
    namespace yaml { \
    static_assert( \
        !std::is_fundamental<TYPE>::value && !std::is_same<TYPE, std::string>::value && \
            !std::is_same<TYPE, unknown::StringRef>::value, \
        "only use LLVM_YAML_IS_SEQUENCE_VECTOR for types you control"); \
    template <> \
    struct SequenceElementTraits<TYPE> \
    { \
...
#define LLVM_YAML_DECLARE_BITSET_TRAITS(Type) \
    ...
    namespace yaml { \
    template <> \
    struct ScalarBitSetTraits<Type> \
    { \
...
#define LLVM_YAML_IS_DOCUMENT_LIST_VECTOR(_type) \
    ...
    namespace yaml { \
    template <unsigned N> \
    struct DocumentListTraits<SmallVector<_type, N>> : public SequenceTraitsImpl<SmallVector<_type, N>, false> \
    { \
    }; \
    template <> \
    struct DocumentListTraits<std::vector<_type>> : public SequenceTraitsImpl<std::vector<_type>, false> \
    { \
    }; \
    } \
    }
...
#define LLVM_YAML_IS_STRING_MAP(_type) \
    ...
    namespace yaml { \
    template <> \
    struct CustomMappingTraits<std::map<std::string, _type>> : public StdMapStringCustomMappingTraitsImpl<_type> \
    { \
    }; \
    } \
    }
