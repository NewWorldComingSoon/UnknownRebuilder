...
raw_ostream &
operator<<(raw_ostream &OS, const HexNumber &Value);
const std::string
to_hexString(uint64_t Value, bool UpperCase = true);
...
public:
    ...
    void unindent(int Levels = 1) { IndentLevel = std::max(0, IndentLevel - Levels); }
        ...
        TFlag EnumMask3 = {})
    {
        ...
        for (const auto &Flag : Flags)
        {
            ...
            if ((!IsEnum && (Value & Flag.Value) == Flag.Value) || (IsEnum && (Value & EnumMask) == Flag.Value))
            {
                SetFlags.push_back(Flag);
            }
        }

        unknown::sort(SetFlags, &flagName<TFlag>);
    ...
    void printString(StringRef Label, const std::string &Value) { printString(Label, StringRef(Value)); }

    void printString(StringRef Label, const char *Value) { printString(Label, StringRef(Value)); }
    ...
    {
        auto V = makeArrayRef(reinterpret_cast<const uint8_t *>(Value.data()), Value.size());
        printBinaryImpl(Label, Str, V, false);
    }

    void printBinary(StringRef Label, ArrayRef<uint8_t> Value) { printBinaryImpl(Label, StringRef(), Value, false); }
...
private:
    ...
    void printBinaryImpl(StringRef Label, StringRef Str, ArrayRef<uint8_t> Value, bool Block, uint32_t StartOffset = 0);
...
template <>
inline void
ScopedPrinter::printHex<support::ulittle16_t>(StringRef Label, support::ulittle16_t Value)
{
    startLine() << Label << ": " << hex(Value) << "\n";
}
