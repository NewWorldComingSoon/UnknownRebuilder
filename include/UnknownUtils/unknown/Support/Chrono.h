...
inline std::time_t
toTimeT(TimePoint<> TP)
{
    using namespace std::chrono;
    return system_clock::to_time_t(time_point_cast<system_clock::time_point::duration>(TP));
}
...
{
    using namespace std::chrono;
    return time_point_cast<nanoseconds>(system_clock::from_time_t(T)) + nanoseconds(nsec);
}
...
{
    static void format(const sys::TimePoint<> &TP, unknown::raw_ostream &OS, StringRef Style);
};
...
private:
    ...
    typedef
        typename std::conditional<std::chrono::treat_as_floating_point<Rep>::value, double, intmax_t>::type InternalRep;
    ...
    static std::pair<InternalRep, StringRef> consumeUnit(StringRef &Style, const Dur &D)
    {
        using namespace std::chrono;
...
public:
    static void format(const Dur &D, unknown::raw_ostream &Stream, StringRef Style)
    {
        InternalRep count;
        StringRef unit;
