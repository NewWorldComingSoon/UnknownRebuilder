...
#include "unknown/Support/ThreadPool.h"
#include "unknown/Support/thread.h"
...
class TaskQueue
{
    ...
    struct Task
    {
        ...
        explicit Task(Callable C, TaskQueue &Parent) :
            C(std::move(C)), P(std::make_shared<std::promise<ResultTy>>()), Parent(&Parent)
        {
        }
...
public:
    ...
    std::future<typename std::result_of<Callable()>::type> async(Callable &&C)
    {
        ...
        static_assert(false, "TaskQueue requires building with LLVM_ENABLE_THREADS!");
#endif
