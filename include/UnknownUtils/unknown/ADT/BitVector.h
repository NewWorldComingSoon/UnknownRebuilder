...
public:
    const_set_bits_iterator_impl(const BitVectorT &Parent, int Current) : Parent(Parent), Current(Current) {}
...
public:
    ...
    iterator_range<const_set_bits_iterator> set_bits() const { return make_range(set_bits_begin(), set_bits_end()); }
    ...
    int find_last_unset_in(unsigned Begin, unsigned End) const
    {
        ...
        for (unsigned i = LastWord + 1; i >= FirstWord + 1; --i)
        {
            ...
            if (Copy != ~0UL)
            {
                unsigned Result = (CurrentWord + 1) * BITWORD_SIZE - countLeadingOnes(Copy) - 1;
                return Result < Size ? Result : -1;
            }
        }
        return -1;
    ...
    int find_prev_unset(unsigned PriorTo) { return find_last_unset_in(0, PriorTo); }
...
private:
    ...
    {
        BitWord *RawBits = static_cast<BitWord *>(safe_malloc(NumWords * sizeof(BitWord)));
    ...
    void grow(unsigned NewSize)
    {
        ...
        BitWord *NewBits = static_cast<BitWord *>(safe_realloc(Bits.data(), NewCapacity * sizeof(BitWord)));
...
inline size_t
capacity_in_bytes(const BitVector &X)
{
...
inline void
swap(unknown::BitVector &LHS, unknown::BitVector &RHS)
{
    LHS.swap(RHS);
}
} // end namespace std
