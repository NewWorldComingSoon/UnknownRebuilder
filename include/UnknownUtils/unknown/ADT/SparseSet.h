...
{
    static unsigned getValIndex(const ValueT &Val) { return Val.getSparseSetIndex(); }
};
...
{
    unsigned operator()(const ValueT &Val) const { return SparseSetValTraits<ValueT>::getValIndex(Val); }
};
...
public:
    ...
    const_iterator find(const KeyT &Key) const { return const_cast<SparseSet *>(this)->findIndex(KeyIndexOf(Key)); }

    /// count - Returns 1 if this set contains an element identified by Key,
    /// 0 otherwise.
    ///
    size_type count(const KeyT &Key) const { return find(Key) == end() ? 0 : 1; }
