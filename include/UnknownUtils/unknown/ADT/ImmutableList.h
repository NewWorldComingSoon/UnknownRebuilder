...
public:
    ...
    static inline void Profile(FoldingSetNodeID &ID, const T &H, const ImmutableListImpl *L)
    {
        ID.AddPointer(L);
        ID.Add(H);
    }
...
public:
    ...
    public:
        ...
        const typename std::remove_reference<value_type>::type *operator->() const { return &L->getHead(); }
...
public:
    ImmutableListFactory() : Allocator(reinterpret_cast<uintptr_t>(new BumpPtrAllocator())) {}

    ImmutableListFactory(BumpPtrAllocator &Alloc) : Allocator(reinterpret_cast<uintptr_t>(&Alloc) | 0x1) {}
...
{
    static inline ImmutableList<T> getEmptyKey() { return reinterpret_cast<ImmutableListImpl<T> *>(-1); }

    static inline ImmutableList<T> getTombstoneKey() { return reinterpret_cast<ImmutableListImpl<T> *>(-2); }
    ...
    static bool isEqual(ImmutableList<T> X1, ImmutableList<T> X2) { return X1 == X2; }
};
