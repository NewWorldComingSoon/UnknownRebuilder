...
template <typename NodeTy>
struct ilist_node_traits : ilist_alloc_traits<NodeTy>, ilist_callback_traits<NodeTy>
{
};
...
struct HasObsoleteCustomization
{
    static const bool value =
        HasGetNext<TraitsT, NodeT>::value || HasCreateSentinel<TraitsT>::value || HasCreateNode<TraitsT, NodeT>::value;
};
...
public:
    ...
    typedef typename base_list_type::const_reverse_iterator const_reverse_iterator;
...
public:
    ...
    iplist_impl(iplist_impl &&X) : TraitsT(std::move(X)), IntrusiveListT(std::move(X)) {}
    ...
    iterator insert(iterator where, const_reference New) { return this->insert(where, new value_type(New)); }
...
public:
    ...
    void splice(iterator where, iplist_impl &L2, reference N) { splice(where, L2, iterator(N)); }
    void splice(iterator where, iplist_impl &L2, pointer N) { splice(where, L2, iterator(N)); }
    ...
    /// Get the previous node, or \c nullptr for the list head.
    const_pointer getPrevNode(const_reference N) const { return getPrevNode(const_cast<reference>(N)); }
    ...
    /// Get the next node, or \c nullptr for the list tail.
    const_pointer getNextNode(const_reference N) const { return getNextNode(const_cast<reference>(N)); }
};
...
class iplist : public iplist_impl<simple_ilist<T, Options...>, ilist_traits<T>>
{
...
template <class Ty>
void
swap(unknown::iplist<Ty> &Left, unknown::iplist<Ty> &Right)
{
    Left.swap(Right);
}
