...
public:
    ...
    static ScopedHashTableVal *Create(
        ScopedHashTableVal *nextInScope,
        ScopedHashTableVal *nextForKey,
        const K &key,
        const V &val,
...
public:
    ...
    bool operator==(const ScopedHashTableIterator &RHS) const { return Node == RHS.Node; }
    bool operator!=(const ScopedHashTableIterator &RHS) const { return Node != RHS.Node; }
...
public:
    ...
    ~ScopedHashTable() { assert(!CurScope && TopLevelMap.empty() && "Scope imbalance!"); }
    ...
    void insertIntoScope(ScopeTy *S, const K &Key, const V &Val)
    {
        ...
        KeyEntry = ValTy::Create(S->getLastValInScope(), KeyEntry, Key, Val, Allocator);
...
ScopedHashTableScope<K, V, KInfo, Allocator>::ScopedHashTableScope(ScopedHashTable<K, V, KInfo, Allocator> &ht) : HT(ht)
{
...
ScopedHashTableScope<K, V, KInfo, Allocator>::~ScopedHashTableScope()
{
    ...
    while (ScopedHashTableVal<K, V> *ThisEntry = LastValInScope)
    {
        ...
        {
            assert(HT.TopLevelMap[ThisEntry->getKey()] == ThisEntry && "Scope imbalance!");
