...
public:
    ...
    {
        typename vector_type::iterator I = unknown::remove_if(vector_, TestAndEraseFromSet<UnaryPredicate>(P, set_));
        if (I == vector_.end())
            return false;
    ...
    bool set_union(const STy &S)
    {
        ...
        for (typename STy::const_iterator SI = S.begin(), SE = S.end(); SI != SE; ++SI)
            if (insert(*SI))
                Changed = true;
...
private:
    ...
    public:
        TestAndEraseFromSet(UnaryPredicate P, set_type &set_) : P(std::move(P)), set_(set_) {}
...
class SmallSetVector : public SetVector<T, SmallVector<T, N>, SmallDenseSet<T, N>>
{
public:
