...
public:
    ...
    void set(typename HelperT::template Lookup<N>::PointerT Pointer)
    {
        ...
        assert(
            (reinterpret_cast<uintptr_t>(V) & HelperT::TagMask) == 0 &&
    ...
    TagT getTag() const { return static_cast<TagT>(getOpaqueValue() & HelperT::TagMask); }
    ...
    /// pointer type.
    typename HelperT::template Lookup<HelperT::MinTag>::PointerT const *getAddrOfZeroTagPointer() const
    {
    ...
    /// stored, get the address of the stored value type-punned to the zero-tag's
    /// pointer type.
    typename HelperT::template Lookup<HelperT::MinTag>::PointerT *getAddrOfZeroTagPointer()
    {
        ...
        assert(InitialPtr == get<HelperT::MinTag>() && "Switching to typed storage changed the pointer returned!");
        // Now we can correctly return an address to typed storage.
        return &Storage.MinTagPointer;
    }

    explicit operator bool() const { return getOpaqueValue() & HelperT::PointerMask; }
    bool operator==(const PointerSumType &R) const { return getOpaqueValue() == R.getOpaqueValue(); }
    bool operator!=(const PointerSumType &R) const { return getOpaqueValue() != R.getOpaqueValue(); }
    bool operator<(const PointerSumType &R) const { return getOpaqueValue() < R.getOpaqueValue(); }
    bool operator>(const PointerSumType &R) const { return getOpaqueValue() > R.getOpaqueValue(); }
    bool operator<=(const PointerSumType &R) const { return getOpaqueValue() <= R.getOpaqueValue(); }
...
struct PointerSumTypeHelper : MemberTs...
{
    ...
    struct Min : std::integral_constant<uintptr_t, (V < Min<Vs...>::value ? V : Min<Vs...>::value)>
    {
    };
    ...
    enum
    {
        NumTagBits = Min<MemberTs::TraitsT::NumLowBitsAvailable...>::value
    };

    // Also compute the smallest discriminant and various masks for convenience.
    constexpr static TagT MinTag = static_cast<TagT>(Min<MemberTs::Tag...>::value);
    enum : uint64_t
    {
    ...
    {
        static_assert(MemberT::Tag < (1 << NumTagBits), "This discriminant value requires too many bits!");
    };
    ...
    static_assert(Checker<MemberTs...>::value, "Each member must pass the checker.");
};
...
struct DenseMapInfo<PointerSumType<TagT, MemberTs...>>
{
    ...
    enum
    {
        SomeTag = HelperT::MinTag
    };
    using SomePointerT = typename HelperT::template Lookup<HelperT::MinTag>::PointerT;
    using SomePointerInfo = DenseMapInfo<SomePointerT>;

    static inline SumType getEmptyKey() { return SumType::create<SomeTag>(SomePointerInfo::getEmptyKey()); }

    static inline SumType getTombstoneKey() { return SumType::create<SomeTag>(SomePointerInfo::getTombstoneKey()); }
    ...
    static bool isEqual(const SumType &LHS, const SumType &RHS) { return LHS == RHS; }
};
