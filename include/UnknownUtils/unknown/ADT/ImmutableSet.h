...
public:
    ...
    {
        // Compare the keys.
        if (!ImutInfo::isEqual(ImutInfo::KeyOfValue(getValue()), ImutInfo::KeyOfValue(V)))
            return false;

        // Also compare the data values.
        if (!ImutInfo::isDataEqual(ImutInfo::DataOfValue(getValue()), ImutInfo::DataOfValue(V)))
            return false;

        return true;
    }

    bool isElementEqual(const ImutAVLTree *RHS) const { return isElementEqual(RHS->getValue()); }
    ...
    unsigned validateTree() const
    {
        ...
        assert(getHeight() == (HL > HR ? HL : HR) + 1 && "Height calculation wrong");

        assert((HL > HR ? HL - HR : HR - HL) <= 2 && "Balancing invariant violated");

        assert(
            (!getLeft() ||
             ImutInfo::isLess(ImutInfo::KeyOfValue(getLeft()->getValue()), ImutInfo::KeyOfValue(getValue()))) &&
            "Value in left child is not less that current value");

        assert(
            !(getRight() ||
              ImutInfo::isLess(ImutInfo::KeyOfValue(getValue()), ImutInfo::KeyOfValue(getRight()->getValue()))) &&
...
private:
    ...
    ///   ImutAVLFactory.
    ImutAVLTree(Factory *f, ImutAVLTree *l, ImutAVLTree *r, value_type_ref v, unsigned height) :
        factory(f),
        left(l),
        right(r),
        height(height),
    ...
    static uint32_t computeDigest(ImutAVLTree *L, ImutAVLTree *R, value_type_ref V)
    {
        uint32_t digest = 0;
...
class ImutAVLFactory
{
    ...
    BumpPtrAllocator &getAllocator() const { return *reinterpret_cast<BumpPtrAllocator *>(Allocator & ~0x1); }
...
public:
    ImutAVLFactory() : Allocator(reinterpret_cast<uintptr_t>(new BumpPtrAllocator())) {}

    ImutAVLFactory(BumpPtrAllocator &Alloc) : Allocator(reinterpret_cast<uintptr_t>(&Alloc) | 0x1) {}
...
protected:
    ...
    TreeTy *remove_internal(key_type_ref K, TreeTy *T)
    {
        ...
        {
            return balanceTree(remove_internal(K, getLeft(T)), getValue(T), getRight(T));
        }
        else
        {
            return balanceTree(getLeft(T), getValue(T), remove_internal(K, getRight(T)));
        }
    }
    ...
    TreeTy *removeMinBinding(TreeTy *T, TreeTy *&Noderemoved)
    {
        ...
        return balanceTree(removeMinBinding(getLeft(T), Noderemoved), getValue(T), getRight(T));
    }
...
template <typename ImutInfo>
class ImutAVLTreeGenericIterator : public std::iterator<std::bidirectional_iterator_tag, ImutAVLTree<ImutInfo>>
{
...
public:
    ...
    bool atBeginning() const { return stack.size() == 1 && getVisitState() == VisitedNone; }
    ...
    bool operator==(const ImutAVLTreeGenericIterator &x) const { return stack == x.stack; }

    bool operator!=(const ImutAVLTreeGenericIterator &x) const { return !(*this == x); }
...
template <typename ImutInfo>
class ImutAVLTreeInOrderIterator : public std::iterator<std::bidirectional_iterator_tag, ImutAVLTree<ImutInfo>>
{
...
public:
    ...
    bool operator==(const ImutAVLTreeInOrderIterator &x) const { return InternalItr == x.InternalItr; }

    bool operator!=(const ImutAVLTreeInOrderIterator &x) const { return !(*this == x); }
    ...
    ImutAVLTreeInOrderIterator &operator++()
    {
        do
            ++InternalItr;
        while (!InternalItr.atEnd() && InternalItr.getVisitState() != InternalIteratorTy::VisitedLeft);
    ...
    ImutAVLTreeInOrderIterator &operator--()
    {
        do
            --InternalItr;
        while (!InternalItr.atBeginning() && InternalItr.getVisitState() != InternalIteratorTy::VisitedLeft);
...
struct ImutAVLValueIterator : iterator_adaptor_base<
                                  ...
                                  typename T::TreeTy::iterator,
                                  typename std::iterator_traits<typename T::TreeTy::iterator>::iterator_category,
                                  const typename T::value_type>
{
    ImutAVLValueIterator() = default;
    explicit ImutAVLValueIterator(typename T::TreeTy *Tree) : ImutAVLValueIterator::iterator_adaptor_base(Tree) {}

    typename ImutAVLValueIterator::reference operator*() const { return this->I->getValue(); }
};
...
struct ImutProfileInfo
{
    ...
    static void Profile(FoldingSetNodeID &ID, value_type_ref X) { FoldingSetTrait<T>::Profile(X, ID); }
};
...
struct ImutProfileInteger
{
    ...
    static void Profile(FoldingSetNodeID &ID, value_type_ref X) { ID.AddInteger(X); }
};

#define PROFILE_INTEGER_INFO(X) \
    template <> \
    struct ImutProfileInfo<X> : ImutProfileInteger<X> \
    { \
    };
...
struct ImutProfileInfo<bool>
{
    ...
    static void Profile(FoldingSetNodeID &ID, value_type_ref X) { ID.AddBoolean(X); }
};
...
struct ImutProfileInfo<T *>
{
    ...
    static void Profile(FoldingSetNodeID &ID, value_type_ref X) { ID.AddPointer(X); }
};
...
struct ImutContainerInfo : public ImutProfileInfo<T>
{
    ...
    static bool isEqual(key_type_ref LHS, key_type_ref RHS) { return std::equal_to<key_type>()(LHS, RHS); }

    static bool isLess(key_type_ref LHS, key_type_ref RHS) { return std::less<key_type>()(LHS, RHS); }
...
public:
    ...
    public:
        ...
        Factory(BumpPtrAllocator &Alloc, bool canonicalize = true) : F(Alloc), Canonicalize(canonicalize) {}
        ...
        typename TreeTy::Factory *getTreeFactory() const { return const_cast<typename TreeTy::Factory *>(&F); }
    };

    friend class Factory;

    /// Returns true if the set contains the specified value.
    bool contains(value_type_ref V) const { return Root ? Root->contains(V) : false; }
    ...
    static void Profile(FoldingSetNodeID &ID, const ImmutableSet &S) { ID.AddPointer(S.Root); }
...
public:
    ...
    /// invoking the constructor, but there are cases where make this
    /// constructor public is useful.
    explicit ImmutableSetRef(TreeTy *R, FactoryTy *F) : Root(R), Factory(F)
    {
        if (Root)
        {
            Root->retain();
    ...
    ImmutableSetRef(const ImmutableSetRef &X) : Root(X.Root), Factory(X.Factory)
    {
        if (Root)
        {
            Root->retain();
    ...
    static ImmutableSetRef getEmptySet(FactoryTy *F) { return ImmutableSetRef(0, F); }

    ImmutableSetRef add(value_type_ref V) { return ImmutableSetRef(Factory->add(Root, V), Factory); }

    ImmutableSetRef remove(value_type_ref V) { return ImmutableSetRef(Factory->remove(Root, V), Factory); }
    ...
    {
        return ImmutableSet<ValT>(canonicalize ? Factory->getCanonicalTree(Root) : Root);
    }
    ...
    static void Profile(FoldingSetNodeID &ID, const ImmutableSetRef &S) { ID.AddPointer(S.Root); }
