...
#include "unknown/ADT/PointerUnion.h"
...
class unique_function<ReturnT(ParamTs...)>
{
    ...
    using AdjustedParamT = typename std::conditional<
        !std::is_reference<T>::value && unknown::is_trivially_copy_constructible<T>::value &&
            unknown::is_trivially_move_constructible<T>::value && IsSizeLessThanThresholdT<T>::value,
        T,
        T &>::type;

    // The type of the erased function pointer we use as a callback to dispatch to
    // the stored callable when it is trivial to move and destroy.
    using CallPtrT = ReturnT (*)(void *CallableAddr, AdjustedParamT<ParamTs>... Params);
    ...
    // destroy pointers.
    using CallbackPointerUnionT = PointerUnion<TrivialCallback *, NonTrivialCallbacks *>;
    ...
    union StorageUnionT
    {
        ...
        typename std::aligned_storage<InlineStorageSize, alignof(void *)>::type InlineStorage;
    } StorageUnion;
    ...
    {
        return CallbackAndInlineFlag.getPointer().template get<NonTrivialCallbacks *>();
    }

    void *getInlineStorage() { return &StorageUnion.InlineStorage; }

    void *getOutOfLineStorage() { return StorageUnion.OutOfLineStorage.StoragePtr; }
    ...
    {
        return (*reinterpret_cast<CallableT *>(CallableAddr))(std::forward<ParamTs>(Params)...);
    }
...
public:
    ...
    ~unique_function()
    {
        ...
        if (!isTrivialCallback())
            getNonTrivialCallbacks()->DestroyPtr(IsInlineStorage ? getInlineStorage() : getOutOfLineStorage());

        if (!IsInlineStorage)
            deallocate_buffer(getOutOfLineStorage(), getOutOfLineStorageSize(), getOutOfLineStorageAlignment());
    }
    ...
    unique_function(CallableT Callable)
    {
        ...
        if (sizeof(CallableT) > InlineStorageSize || alignof(CallableT) > alignof(decltype(StorageUnion.InlineStorage)))
        {
        ...
        if (unknown::is_trivially_move_constructible<CallableT>::value &&
        ...
        static NonTrivialCallbacks Callbacks = {&CallImpl<CallableT>, &MoveImpl<CallableT>, &DestroyImpl<CallableT>};
    ...
    {
        void *CallableAddr = isInlineStorage() ? getInlineStorage() : getOutOfLineStorage();

        return (isTrivialCallback() ? getTrivialCallback() : getNonTrivialCallbacks()->CallPtr)(
            CallableAddr, Params...);
    }
