...
class PointerEmbeddedInt
{
    ...
    static_assert(Bits < sizeof(uintptr_t) * CHAR_BIT, "Cannot embed more bits than we have in a pointer!");
...
public:
    ...
    {
        assert(
            (std::is_signed<IntT>::value ? isInt<Bits>(I) : isUInt<Bits>(I)) &&
...
struct PointerLikeTypeTraits<PointerEmbeddedInt<IntT, Bits>>
{
    ...
    static inline void *getAsVoidPointer(const T &P) { return reinterpret_cast<void *>(P.Value); }

    static inline T getFromVoidPointer(void *P) { return T(reinterpret_cast<uintptr_t>(P), typename T::RawValueTag()); }
...
struct DenseMapInfo<PointerEmbeddedInt<IntT, Bits>>
{
    ...
    static unsigned getHashValue(const T &Arg) { return IntInfo::getHashValue(Arg); }
