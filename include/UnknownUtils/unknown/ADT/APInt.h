...
private:
    ...
    uint64_t getWord(unsigned bitPosition) const { return isSingleWord() ? U.VAL : U.pVal[whichWord(bitPosition)]; }
...
public:
    ...
    APInt(unsigned numBits, uint64_t val, bool isSigned = false) : BitWidth(numBits)
    {
    ...
    uint64_t getLimitedValue(uint64_t Limit = UINT64_MAX) const { return ugt(Limit) ? Limit : getZExtValue(); }
    ...
    bool isMask(unsigned numBits) const
    {
        ...
        return (numBits == Ones) && ((Ones + countLeadingZerosSlowCase()) == BitWidth);
    }
    ...
    static APInt getSignMask(unsigned BitWidth) { return getSignedMinValue(BitWidth); }

    /// Get the all-ones value.
    ///
    /// \returns the all-ones value for an APInt of the specified bit-width.
    static APInt getAllOnesValue(unsigned numBits) { return APInt(numBits, WORDTYPE_MAX, true); }
    ...
    {
        return (!isSingleWord() && getMinSignedBits() > 64) ? isNegative() : getSExtValue() < RHS;
    }
    ...
    unsigned countLeadingZeros() const
    {
        if (isSingleWord())
        {
            ...
            return unknown::countLeadingZeros(U.VAL) - unusedBits;
        }
    ...
    {
        if (isSingleWord())
            return unknown::countLeadingOnes(U.VAL << (APINT_BITS_PER_WORD - BitWidth));
    ...
    {
        if (isSingleWord())
            return std::min(unsigned(unknown::countTrailingZeros(U.VAL)), BitWidth);
    ...
    {
        if (isSingleWord())
            return unknown::countTrailingOnes(U.VAL);
    ...
    {
        if (isSingleWord())
            return unknown::countPopulation(U.VAL);
...
inline bool
operator==(uint64_t V1, const APInt &V2)
{
    return V2 == V1;
}
...
inline APInt
operator&(APInt a, const APInt &b)
{
    a &= b;
    return a;
}
...
inline APInt
operator&(const APInt &a, APInt &&b)
{
    b &= a;
    return std::move(b);
}
...
inline raw_ostream &
operator<<(raw_ostream &OS, const APInt &I)
{
    I.print(OS, true);
    return OS;
}
...
inline const APInt &
smin(const APInt &A, const APInt &B)
{
    return A.slt(B) ? A : B;
}
...
inline const APInt &
smax(const APInt &A, const APInt &B)
{
    return A.sgt(B) ? A : B;
}
...
/// Determine the smaller of two APInts considered to be signed.
inline const APInt &
umin(const APInt &A, const APInt &B)
{
    return A.ult(B) ? A : B;
}
...
inline const APInt &
umax(const APInt &A, const APInt &B)
{
    return A.ugt(B) ? A : B;
}
...
APInt
GreatestCommonDivisor(APInt A, APInt B);

/// Converts the given APInt to a double value.
///
/// Treats the APInt as an unsigned value for conversion purposes.
inline double
RoundAPIntToDouble(const APInt &APIVal)
{
...
inline double
RoundSignedAPIntToDouble(const APInt &APIVal)
{
...
inline float
RoundSignedAPIntToFloat(const APInt &APIVal)
{
...
APInt
RoundDoubleToAPInt(double Double, unsigned width);

/// Converts a float value into a APInt.
///
/// Converts a float value into an APInt value.
inline APInt
RoundFloatToAPInt(float Float, unsigned width)
{
...
APInt
RoundingUDiv(const APInt &A, const APInt &B, APInt::Rounding RM);

/// Return A sign-divided by B, rounded by the given rounding mode.
APInt
RoundingSDiv(const APInt &A, const APInt &B, APInt::Rounding RM);
