...
#include "unknown/ADT/ilist_node.h"
#include "unknown/ADT/iterator.h"
#include "unknown/ADT/simple_ilist.h"
...
class AllocatorList : AllocatorT
{
    struct Node : ilist_node<Node>
    {
        ...
        Node(const T &V) : V(V) {}
        template <class... Ts>
        Node(Ts &&...Vs) : V(std::forward<Ts>(Vs)...)
        {
        }
        T V;
    };
...
private:
    ...
    class IteratorImpl : public iterator_adaptor_base<
                             ...
                             ValueT>
    {
        ...
        using base_type = iterator_adaptor_base<
            IteratorImpl<ValueT, IteratorBase>,
            IteratorBase,
            std::bidirectional_iterator_tag,
            ValueT>;
    ...
    public:
        ...
        friend bool operator==(const IteratorImpl &L, const IteratorImpl &R) { return L.wrapped() == R.wrapped(); }
        friend bool operator!=(const IteratorImpl &L, const IteratorImpl &R) { return !(L == R); }
    };
...
public:
    ...
    AllocatorList(AllocatorList &&X) : AllocatorT(std::move(X.getAlloc())), List(std::move(X.List)) {}

    AllocatorList(const AllocatorList &X) { List.cloneFrom(X.List, Cloner(*this), Disposer(*this)); }
    ...
    const_reverse_iterator rbegin() const { return const_reverse_iterator(List.rbegin()); }
    const_reverse_iterator rend() const { return const_reverse_iterator(List.rend()); }
    ...
    iterator insert(iterator I, T &&V) { return iterator(List.insert(I.wrapped(), *create(std::move(V)))); }
    iterator insert(iterator I, const T &V) { return iterator(List.insert(I.wrapped(), *create(V))); }
    ...
    iterator erase(iterator I) { return iterator(List.eraseAndDispose(I.wrapped(), Disposer(*this))); }

    iterator erase(iterator First, iterator Last)
    {
        return iterator(List.eraseAndDispose(First.wrapped(), Last.wrapped(), Disposer(*this)));
    }
