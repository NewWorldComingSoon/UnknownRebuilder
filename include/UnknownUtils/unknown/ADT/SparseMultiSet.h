...
#include "unknown/ADT/SparseSet.h"
#include <cassert>
#include <cstdint>
#include <cstdlib>
#include <iterator>
...
{
    static_assert(
        std::numeric_limits<SparseT>::is_integer && !std::numeric_limits<SparseT>::is_signed,
    ...
    {
        assert(ValIndexOf(Val) < Universe && "Invalid key in set. Did object mutate?");
...
public:
    ...
    class iterator_base : public std::iterator<std::bidirectional_iterator_tag, ValueT>
    {
    ...
    public:
        ...
        {
            assert(
                isKeyed() && SMS->sparseIndex(SMS->Dense[Idx].Data) == SparseIdx &&
        ...
        bool operator==(const iterator_base &RHS) const
        {
            ...
            {
                assert((isEnd() || SparseIdx == RHS.SparseIdx) && "Same dense entry, but different keys?");
                return true;
            }
        ...
        { // predecrement - Back up
            ...
            assert((isEnd() || !SMS->isHead(SMS->Dense[Idx])) && "Decrementing head of list");
    ...
    const_iterator end() const { return const_iterator(this, SMSNode::INVALID, SMSNode::INVALID); }
    ...
    {
        assert(I.isKeyed() && !I.isEnd() && !Dense[I.Idx].isTombstone() && "erasing invalid/end/tombstone iterator");
