...
SmallVectorTemplateBase<T, isPodLike>::grow(size_t MinSize)
{
    ...
    T *NewElts = static_cast<T *>(unknown::safe_malloc(NewCapacity * sizeof(T)));
...
template <typename T>
inline void
swap(unknown::SmallVectorImpl<T> &LHS, unknown::SmallVectorImpl<T> &RHS)
{
    LHS.swap(RHS);
}
...
template <typename T, unsigned N>
inline void
swap(unknown::SmallVector<T, N> &LHS, unknown::SmallVector<T, N> &RHS)
{
    LHS.swap(RHS);
}
