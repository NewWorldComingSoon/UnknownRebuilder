...
#include "unknown/ADT/EpochTracker.h"
#include "unknown/Support/AlignOf.h"
#include "unknown/Support/Compiler.h"
#include "unknown/Support/MathExtras.h"
#include "unknown/Support/ReverseIteration.h"
#include "unknown/Support/type_traits.h"
#include <algorithm>
#include <cassert>
#include <cstddef>
#include <cstring>
...
struct DenseMapPair : public std::pair<KeyT, ValueT>
{
    ...
    DenseMapPair(const KeyT &Key, const ValueT &Value) : std::pair<KeyT, ValueT>(Key, Value) {}

    DenseMapPair(KeyT &&Key, ValueT &&Value) : std::pair<KeyT, ValueT>(std::move(Key), std::move(Value)) {}
    ...
    DenseMapPair(
        ...
        typename std::enable_if<
            std::is_convertible<AltKeyT, KeyT>::value && std::is_convertible<AltValueT, ValueT>::value>::type * = 0) :
        std::pair<KeyT, ValueT>(std::forward<AltKeyT>(AltKey), std::forward<AltValueT>(AltValue))
    {
    }
...
template <
    ...
    typename Bucket = unknown::detail::DenseMapPair<KeyT, ValueT>,
    bool IsConst = false>
...
public:
    ...
    inline iterator end() { return makeIterator(getBucketsEnd(), getBucketsEnd(), *this, true); }
    ...
    inline const_iterator end() const { return makeConstIterator(getBucketsEnd(), getBucketsEnd(), *this, true); }
    ...
    // value.
    std::pair<iterator, bool> insert(const std::pair<KeyT, ValueT> &KV) { return try_emplace(KV.first, KV.second); }
    ...
    std::pair<iterator, bool> insert_as(std::pair<KeyT, ValueT> &&KV, const LookupKeyT &Val)
    {
        ...
        // Otherwise, insert the new element.
        TheBucket = InsertIntoBucketWithLookup(TheBucket, std::move(KV.first), std::move(KV.second), Val);
        return std::make_pair(makeIterator(TheBucket, getBucketsEnd(), *this, true), true);
    }
...
protected:
    ...
    void copyFrom(const DenseMapBase<OtherBaseT, KeyT, ValueT, KeyInfoT, BucketT> &other)
    {
        ...
        if (isPodLike<KeyT>::value && isPodLike<ValueT>::value)
            memcpy(reinterpret_cast<void *>(getBuckets()), other.getBuckets(), getNumBuckets() * sizeof(BucketT));
        else
...
private:
    iterator makeIterator(BucketT *P, BucketT *E, DebugEpochBase &Epoch, bool NoAdvance = false)
    {
        if (shouldReverseIterate<KeyT>())
        {
    ...
    const_iterator
    makeConstIterator(const BucketT *P, const BucketT *E, const DebugEpochBase &Epoch, const bool NoAdvance = false)
        const
    {
        if (shouldReverseIterate<KeyT>())
        {
    ...
    BucketT *InsertIntoBucket(BucketT *TheBucket, KeyArg &&Key, ValueArgs &&...Values)
    {
    ...
    template <typename LookupKeyT>
    BucketT *InsertIntoBucketWithLookup(BucketT *TheBucket, KeyT &&Key, ValueT &&Value, LookupKeyT &Lookup)
    {
...
template <
    ...
    typename ValueT,
    typename KeyInfoT = DenseMapInfo<KeyT>,
    typename BucketT = unknown::detail::DenseMapPair<KeyT, ValueT>>
class DenseMap : public DenseMapBase<DenseMap<KeyT, ValueT, KeyInfoT, BucketT>, KeyT, ValueT, KeyInfoT, BucketT>
{
                          ...
                          BucketT>
{
    ...
    static_assert(isPowerOf2_64(InlineBuckets), "InlineBuckets must be a power of 2.");
...
private:
    ...
    LargeRep allocateBuckets(unsigned Num)
    {
        ...
        LargeRep Rep = {static_cast<BucketT *>(operator new(sizeof(BucketT) * Num)), Num};
        return Rep;
    }
};
...
public:
    ...
    bool operator==(const ConstIterator &RHS) const
    {
        ...
        assert(getEpochAddress() == RHS.getEpochAddress() && "comparing incomparable iterators!");
        return Ptr == RHS.Ptr;
    }
    bool operator!=(const ConstIterator &RHS) const
    {
...
private:
    ...
    void RetreatPastEmptyBuckets()
    {
        ...
        while (Ptr != End &&
               (KeyInfoT::isEqual(Ptr[-1].getFirst(), Empty) || KeyInfoT::isEqual(Ptr[-1].getFirst(), Tombstone)))
            --Ptr;
    }
};
