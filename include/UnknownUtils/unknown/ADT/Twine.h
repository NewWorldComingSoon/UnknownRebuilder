...
public:
    ...
    /*implicit*/ Twine(const SmallVectorImpl<char> &Str) : LHSKind(SmallStringKind)
    {
        LHS.smallString = &Str;
        assert(isValid() && "Invalid twine!");
    }
    ...
    /*implicit*/ Twine(const formatv_object_base &Fmt) : LHSKind(FormatvObjectKind)
    {
    ...
    explicit Twine(unsigned char Val) : LHSKind(CharKind) { LHS.character = static_cast<char>(Val); }
    ...
    /*implicit*/ Twine(const char *LHS, const StringRef &RHS) : LHSKind(CStringKind), RHSKind(StringRefKind)
    {
    ...
    /*implicit*/ Twine(const StringRef &LHS, const char *RHS) : LHSKind(StringRefKind), RHSKind(CStringKind)
    {
...
inline Twine
operator+(const Twine &LHS, const Twine &RHS)
{
...
inline Twine
operator+(const char *LHS, const StringRef &RHS)
{
...
inline Twine
operator+(const StringRef &LHS, const char *RHS)
{
    return Twine(LHS, RHS);
}

inline raw_ostream &
operator<<(raw_ostream &OS, const Twine &RHS)
{
    RHS.print(OS);
    return OS;
}
