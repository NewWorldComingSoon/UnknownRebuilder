...
public:
    ...
    void updateMax(unsigned V)
    {
        ...
        while (V > PrevMax && !Value.compare_exchange_weak(PrevMax, V, std::memory_order_relaxed))
        {
        }
        init();
    }
...
#define STATISTIC(VARNAME, DESC) static unknown::Statistic VARNAME = {DEBUG_TYPE, #VARNAME, DESC, {0}, {false}}

/// Enable the collection and printing of statistics.
void
EnableStatistics(bool PrintOnExit = true);
...
void
PrintStatistics(raw_ostream &OS);
...
/// PrintStatisticsJSON().
void
PrintStatisticsJSON(raw_ostream &OS);
...
/// completes.
const std::vector<std::pair<StringRef, unsigned>>
GetStatistics();
