...
template <
    class GraphT,
    class SetType = bf_iterator_default_set<typename GraphTraits<GraphT>::NodeRef>,
    class GT = GraphTraits<GraphT>>
class bf_iterator : public std::iterator<std::forward_iterator_tag, typename GT::NodeRef>,
...
public:
    ...
    static bf_iterator begin(const GraphT &G) { return bf_iterator(GT::getEntryNode(G)); }

    static bf_iterator end(const GraphT &G) { return bf_iterator(); }

    bool operator==(const bf_iterator &RHS) const { return VisitQueue == RHS.VisitQueue; }
...
template <class T>
iterator_range<bf_iterator<T>>
breadth_first(const T &G)
{
