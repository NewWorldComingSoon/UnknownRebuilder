...
#include "unknown/Support/RecyclingAllocator.h"
#include <algorithm>
#include <cassert>
#include <cstdint>
#include <iterator>
...
public:
    ...
    void erase(unsigned i, unsigned j, unsigned Size) { moveLeft(j, i, Size - j); }
...
void
adjustSiblingSizes(NodeT *Node[], unsigned Nodes, unsigned CurSize[], const unsigned NewSize[])
{
    // Move elements right.
...
class NodeRef
{
    struct CacheAlignedPointerTraits
    {
        ...
        enum
        {
            NumLowBitsAvailable = Log2CacheLine
        };
    };
...
public:
    ...
    /// member.
    NodeRef &subtree(unsigned i) const { return reinterpret_cast<NodeRef *>(pip.getPointer())[i]; }
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
unsigned
LeafNode<KeyT, ValT, N, Traits>::insertFrom(unsigned &Pos, unsigned Size, KeyT a, KeyT b, ValT y)
{
    unsigned i = Pos;
...
class Path
{
    ...
    struct Entry
    {
        ...
        NodeRef &subtree(unsigned i) const { return reinterpret_cast<NodeRef *>(node)[i]; }
    };
...
public:
    ...
    NodeRef &subtree(unsigned Level) const { return path[Level].subtree(path[Level].offset); }

    /// reset - Reset cached information about node(Level) from subtree(Level -1).
    /// @param Level 1..height. THe node to update after parent node changed.
    void reset(unsigned Level) { path[Level] = Entry(subtree(Level - 1), offset(Level)); }

    /// push - Add entry to path.
    /// @param Node Node to add, should be subtree(path.size()-1).
    /// @param Offset Offset into Node.
    void push(NodeRef Node, unsigned Offset) { path.push_back(Entry(Node, Offset)); }
    ...
    bool atLastEntry(unsigned Level) const { return path[Level].offset == path[Level].size - 1; }
...
private:
    ...
    LLVM_ALIGNAS(RootLeaf) LLVM_ALIGNAS(RootBranchData) AlignedCharArrayUnion<RootLeaf, RootBranchData> data;
...
public:
    ...
    {
        assert((uintptr_t(data.buffer) & (alignof(RootLeaf) - 1)) == 0 && "Insufficient alignment");
        new (&rootLeaf()) RootLeaf();
    }
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
ValT
IntervalMap<KeyT, ValT, N, Traits>::treeSafeLookup(KeyT x, ValT NotFound) const
{
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::visitNodes(void (IntervalMap::*f)(IntervalMapImpl::NodeRef, unsigned Height))
{
    if (!branched())
        return;
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::deleteNode(IntervalMapImpl::NodeRef Node, unsigned Level)
{
    if (Level)
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::clear()
{
    if (branched())
    {
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
class IntervalMap<KeyT, ValT, N, Traits>::const_iterator : public std::iterator<std::bidirectional_iterator_tag, ValT>
{
protected:
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::const_iterator::pathFillFind(KeyT x)
{
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::const_iterator::treeFind(KeyT x)
{
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::const_iterator::treeAdvanceTo(KeyT x)
{
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
bool
IntervalMap<KeyT, ValT, N, Traits>::iterator::canCoalesceLeft(KeyT Start, ValT Value)
{
    using namespace IntervalMapImpl;
    Path &P = this->path;
    if (!this->branched())
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
bool
IntervalMap<KeyT, ValT, N, Traits>::iterator::canCoalesceRight(KeyT Stop, ValT Value)
{
    using namespace IntervalMapImpl;
    Path &P = this->path;
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::iterator::setNodeStop(unsigned Level, KeyT Stop)
{
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::iterator::setStart(KeyT a)
{
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::iterator::setStop(KeyT b)
{
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::iterator::setValue(ValT x)
{
    setValueUnchecked(x);
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
bool
IntervalMap<KeyT, ValT, N, Traits>::iterator::insertNode(unsigned Level, IntervalMapImpl::NodeRef Node, KeyT Stop)
{
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::iterator::insert(KeyT a, KeyT b, ValT y)
{
    if (this->branched())
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::iterator::treeInsert(KeyT a, KeyT b, ValT y)
{
    using namespace IntervalMapImpl;
    Path &P = this->path;
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::iterator::erase()
{
    IntervalMap &IM = *this->map;
    IntervalMapImpl::Path &P = this->path;
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::iterator::treeErase(bool UpdateRoot)
{
    IntervalMap &IM = *this->map;
    IntervalMapImpl::Path &P = this->path;
    Leaf &Node = P.leaf<Leaf>();
...
template <typename KeyT, typename ValT, unsigned N, typename Traits>
void
IntervalMap<KeyT, ValT, N, Traits>::iterator::eraseNode(unsigned Level)
{
...
template <typename NodeT>
bool
IntervalMap<KeyT, ValT, N, Traits>::iterator::overflow(unsigned Level)
{
    using namespace IntervalMapImpl;
    Path &P = this->path;
    unsigned CurSize[4];
