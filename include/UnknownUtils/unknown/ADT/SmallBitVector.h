...
class SmallBitVector
{
    ...
    static_assert(NumBaseBits == 64 || NumBaseBits == 32, "Unsupported word size");
...
private:
    ...
    {
        setSmallRawBits((NewBits & ~(~uintptr_t(0) << getSmallSize())) | (getSmallSize() << SmallNumDataBits));
    }
...
public:
    ...
    SmallBitVector &set(unsigned Idx)
    {
        if (isSmall())
        {
            assert(Idx <= static_cast<unsigned>(std::numeric_limits<uintptr_t>::digits) && "undefined behavior");
...
inline void
swap(unknown::SmallBitVector &LHS, unknown::SmallBitVector &RHS)
{
    LHS.swap(RHS);
}
