...
public:
    ...
    function_ref(
        ...
        typename std::enable_if<
            !std::is_same<typename std::remove_reference<Callable>::type, function_ref>::value>::type * = nullptr) :
...
auto
adl_begin(ContainerTy &&container) -> decltype(begin(std::forward<ContainerTy>(container)))
{
...
template <typename ContainerTy>
auto
adl_end(ContainerTy &&container) -> decltype(end(std::forward<ContainerTy>(container)))
{
...
template <typename ContainerTy>
auto
adl_begin(ContainerTy &&container) -> decltype(adl_detail::adl_begin(std::forward<ContainerTy>(container)))
{
...
template <typename ContainerTy>
auto
adl_end(ContainerTy &&container) -> decltype(adl_detail::adl_end(std::forward<ContainerTy>(container)))
{
...
template <typename T>
void
adl_swap(T &&lhs, T &&rhs) noexcept(noexcept(adl_detail::adl_swap(std::declval<T>(), std::declval<T>())))
{
...
class mapped_iterator : public iterator_adaptor_base<
...
template <typename ContainerTy>
auto
reverse(ContainerTy &&C, typename std::enable_if<has_rbegin<ContainerTy>::value>::type * = nullptr)
...
auto
reverse(ContainerTy &&C, typename std::enable_if<!has_rbegin<ContainerTy>::value>::type * = nullptr)
    -> decltype(make_range(unknown::make_reverse_iterator(std::end(C)), unknown::make_reverse_iterator(std::begin(C))))
{
    return make_range(unknown::make_reverse_iterator(std::end(C)), unknown::make_reverse_iterator(std::begin(C)));
}
...
{
    using BaseT = filter_iterator_base<WrappedIteratorT, PredicateT, std::bidirectional_iterator_tag>;
    void findPrevValid()
    {
...
make_early_inc_range(RangeT &&Range)
{
    ...
    return make_range(
        EarlyIncIteratorT(std::begin(std::forward<RangeT>(Range))),
...
using zip_traits = iterator_facade_base<
    ZipType,
    typename std::
        common_type<std::bidirectional_iterator_tag, typename std::iterator_traits<Iters>::iterator_category...>::type,
    // ^ TODO: Implement random access methods.
    typename ZipTupleType<Iters...>::type,
    typename std::iterator_traits<typename std::tuple_element<0, std::tuple<Iters...>>::type>::difference_type,
...
public:
    ...
    const value_type operator*() const { return deref(index_sequence_for<Iters...>{}); }
...
class zip_shortest : public zip_common<zip_shortest<Iters...>, Iters...>
{
    ...
    {
        return all_of(
            std::initializer_list<bool>{std::get<Ns>(this->iterators) != std::get<Ns>(other.iterators)...},
            identity<bool>{});
    }
...
public:
    ...
    bool operator==(const zip_shortest<Iters...> &other) const { return !test(other, index_sequence_for<Iters...>{}); }
};
...
deref_or_none(const Iter &I, const Iter &End)
    -> unknown::Optional<typename std::remove_const<typename std::remove_reference<decltype(*I)>::type>::type>
{
    if (I == End)
        return None;
    return *I;
...
{
    using type = unknown::Optional<
        typename std::remove_const<typename std::remove_reference<decltype(*std::declval<Iter>())>::type>::type>;
};
...
class zip_longest_iterator
    : public iterator_facade_base<
          ...
          typename std::
              common_type<std::forward_iterator_tag, typename std::iterator_traits<Iters>::iterator_category...>::type,
...
private:
    ...
    template <size_t... Ns>
    bool test(const zip_longest_iterator<Iters...> &other, index_sequence<Ns...>) const
    {
        return unknown::any_of(
            std::initializer_list<bool>{std::get<Ns>(this->iterators) != std::get<Ns>(other.iterators)...},
            identity<bool>{});
    }
...
private:
    ...
    iterator begin_impl(index_sequence<Ns...>) const
    {
        return iterator(std::make_pair(adl_begin(std::get<Ns>(ts)), adl_end(std::get<Ns>(ts)))...);
    }

    template <size_t... Ns>
    iterator end_impl(index_sequence<Ns...>) const
    {
        return iterator(std::make_pair(adl_end(std::get<Ns>(ts)), adl_end(std::get<Ns>(ts)))...);
    }
...
/// until all iterators reach the end. The unknown::Optional only contains a value
...
{
    static const bool value = std::is_same<T, U>::value || is_one_of<T, Ts...>::value;
};
...
{
    static const bool value = std::is_base_of<T, U>::value && are_base_of<T, Ts...>::value;
};
...
sort(Container &&C)
{
    unknown::sort(adl_begin(C), adl_end(C));
}
...
{
    unknown::sort(adl_begin(C), adl_end(C), Comp);
}
...
template <typename Container>
void
DeleteContainerPointers(Container &C)
{
    for (auto V : C)
        delete V;
    C.clear();
...
size(
    ...
    typename std::enable_if<
        ...
        void>::type * = nullptr) -> decltype(std::distance(Range.begin(), Range.end()))
{
...
auto
find(R &&Range, const T &Val) -> decltype(adl_begin(Range))
{
...
template <typename R, typename UnaryPredicate>
auto
find_if(R &&Range, UnaryPredicate P) -> decltype(adl_begin(Range))
{
...
template <typename R, typename UnaryPredicate>
auto
find_if_not(R &&Range, UnaryPredicate P) -> decltype(adl_begin(Range))
{
...
template <typename R, typename UnaryPredicate>
auto
remove_if(R &&Range, UnaryPredicate P) -> decltype(adl_begin(Range))
{
...
template <typename R, typename E>
auto
count(R &&Range, const E &Element) -> typename std::iterator_traits<decltype(adl_begin(Range))>::difference_type
{
...
template <typename R, typename UnaryPredicate>
auto
count_if(R &&Range, UnaryPredicate P) -> typename std::iterator_traits<decltype(adl_begin(Range))>::difference_type
{
...
template <typename R, typename UnaryPredicate>
auto
partition(R &&Range, UnaryPredicate P) -> decltype(adl_begin(Range))
{
...
auto
lower_bound(R &&Range, ForwardIt I) -> decltype(adl_begin(Range))
{
...
auto
lower_bound(R &&Range, ForwardIt I, Compare C) -> decltype(adl_begin(Range))
{
...
template <typename R, typename ForwardIt>
auto
upper_bound(R &&Range, ForwardIt I) -> decltype(adl_begin(Range))
{
...
template <typename R, typename ForwardIt, typename Compare>
auto
upper_bound(R &&Range, ForwardIt I, Compare C) -> decltype(adl_begin(Range))
{
...
is_splat(R &&Range)
{
    ...
    return range_size != 0 && (range_size == 1 || std::equal(adl_begin(Range) + 1, adl_end(Range), adl_begin(Range)));
}
...
template <typename R>
class enumerator_iter : public iterator_facade_base<
                            enumerator_iter<R>,
...
auto
apply_tuple_impl(F &&f, Tuple &&t, index_sequence<I...>)
...
auto
apply_tuple(F &&f, Tuple &&t) -> decltype(detail::apply_tuple_impl(
    std::forward<F>(f),
    std::forward<Tuple>(t),
    build_index_impl<std::tuple_size<typename std::decay<Tuple>::type>::value>{}))
{
    using Indices = build_index_impl<std::tuple_size<typename std::decay<Tuple>::type>::value>;
...
hasNItems(
    ...
    typename std::enable_if<
        !std::is_same<
            typename std::iterator_traits<typename std::remove_reference<decltype(Begin)>::type>::iterator_category,
            std::random_access_iterator_tag>::value,
        void>::type * = nullptr)
{
    for (; N; --N, ++Begin)
